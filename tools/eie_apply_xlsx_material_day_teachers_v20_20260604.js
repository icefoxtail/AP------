// EIE 26.04 Excel 湲곗? 援먯옱紐??붿씪蹂??대떦留?諛섏쁺?섎뒗 ?ㅽ뻾 ?ㅽ겕由쏀듃 v20
// 湲곗? ?뚯씪: ?곸뼱26.04 (1).xlsx / sheet: 26.04
// ?ㅽ뻾 ?꾩튂: ?ㅼ젣 EIE ?쒓컙???붾㈃ > DevTools Console
// 諛섏쁺 ??? 援먯옱紐? ??????紐?湲??대떦
// ?덈? 嫄대뱶由ъ? ?딆쓬: ?곷떒 ?좎깮???? ?숈깮 諛곗젙, cell id, 援먯떆/?쒓컙, 異쒓껐/?숈젣/?곷떞
...
(async () => {
  ...
  const ok = window.confirm(`v20: ?곸뼱26.04 ?묒? 湲곗? 援먯옱紐낃낵 ??????紐?湲??대떦留?${mappings.length}媛?移대뱶??諛섏쁺?⑸땲??
??????紐?湲??쒓린???묒? 癒몃━湲???붿닔湲??붾ぉ/?붾ぉ湲?洹쒖튃???곸슜?⑸땲??
?곷떒 ?좎깮???? ?숈깮 諛곗젙, 援먯떆/?쒓컙? 嫄대뱶由ъ? ?딆뒿?덈떎.
吏꾪뻾?좉퉴??`);
  if (!ok) {
