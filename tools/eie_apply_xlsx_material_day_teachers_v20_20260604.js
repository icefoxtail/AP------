// EIE 26.04 Excel 기준 교재명/요일별 담당만 반영하는 실행 스크립트 v20
// 기준 파일: 영어26.04 (1).xlsx / sheet: 26.04
// 실행 위치: 실제 EIE 시간표 화면 > DevTools Console
// 반영 대상: 교재명, 월/화/수/목/금 담당
// 절대 건드리지 않음: 상단 선생님 열, 학생 배정, cell id, 교시/시간, 출결/숙제/상담
...
(async () => {
  ...
  function rawMetaWithTruth(row, truth) {
    ...
    return {
      ...
      excel_truth: {
        ...
        scope: 'v20_material_text_and_weekday_teachers_only_no_homeroom_change'
      }
    };
  }
  ...
  function payloadForRow(row, truth) {
    return {
      ...
      // 상단 선생님 열과 기존 row 담당 정보는 바꾸지 않는다.
    };
  }
  ...
  const ok = window.confirm(`v20: 영어26.04 엑셀 기준 교재명과 월/화/수/목/금 담당만 ${mappings.length}개 카드에 반영합니다.
월/화/수/목/금 표기는 엑셀 머리글의 월수금/화목/화목금 규칙을 적용합니다.
상단 선생님 열, 학생 배정, 교시/시간은 건드리지 않습니다.
진행할까요?`);
  if (!ok) {
    console.log('취소했습니다.');
    return;
