...
const SCOPED=JSON.parse(fs.readFileSync(path.join(REPORT,'654_scoped_candidate_bank_manifest_r44.json')));
const REGISTRY_REF=JSON.parse(fs.readFileSync(path.join(REPORT,'61_source_exam_id_registry_ref_r10.json')));
const OUTPUT=path.join(REPORT,'657_current_v2_preparation_r44.json');
const WORK_BATCH_ID='hs-quadratic-r44f';
const BUILDER_ID='codex-builder-r44f';
const BUILDER_SESSION_ID='codex-session-r44f';
const RUN_ROOT='archive/_generated/hs-quadratic-svg-upgrade-20260908/pipeline-r44f/runs';
const runs=[]; const errors=[];
for(let index=0;index<SCOPED.candidateFiles.length;index+=1){
  const file=SCOPED.candidateFiles[index];
  const runId=`hsquadratic-r44f-${String(index+1).padStart(3,'0')}`;
...
const output={schemaVersion:'HS_QUADRATIC_CURRENT_V2_PREPARATION_R44',status:errors.length?'V2_PREPARATION_PARTIAL_OR_FAIL':'V2_PREPARATION_DRAFTS_READY_NO_PASS',productionAuthorized:false,workBatchId:WORK_BATCH_ID,builderId:BUILDER_ID,builderSessionId:BUILDER_SESSION_ID,sourceRegistryRef:'reports/hs-quadratic-svg-upgrade-20260908/61_source_exam_id_registry_ref_r10.json',sourceRegistryAuthority:'PENDING',scopedCandidateManifest:'reports/hs-quadratic-svg-upgrade-20260908/654_scoped_candidate_bank_manifest_r44.json',targetQuestionCount:430,runCount:runs.length,preparedQuestionCount:runs.reduce((sum,run)=>sum+run.questionCount,0),errors,runs,note:'Fresh r44 current v2 drafts. No semantic review, render capture, whole-job freeze, provider launch, or final PASS is fabricated.'};
