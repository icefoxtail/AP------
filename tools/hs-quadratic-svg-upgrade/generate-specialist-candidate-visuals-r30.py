...
FACTS = REPORT / '410_specialist_v1_expected_facts_r30.json'
OUT_ROOT = ROOT / 'archive' / '_generated' / 'hs-quadratic-svg-upgrade-20260908' / 'candidate-r30' / 'assets'
MANIFEST = REPORT / '411_specialist_candidate_visual_manifest_r30.json'
BASE_PATH = Path(__file__).with_name('generate-deterministic-candidate-visuals-r11.py')
spec = importlib.util.spec_from_file_location('base_visuals_r30', BASE_PATH)
...
def render(case_id: str, item: dict) -> str:
    ...
    if item['expectedVisualType'] == 'number-line':
        ...
        svg = base.cartesian(case_id, fact, '이차함수 그래프와 핵심 조건')
    return svg.replace('candidate_builder_python_r11', 'candidate_builder_python_r30')


data = json.loads(FACTS.read_text(encoding='utf-8'))
rows = []
for index, item in enumerate(data['rows'], 1):
    case_id = f'hs-r30-{index:03d}'
...
output = {'schemaVersion': 'HS_QUADRATIC_SPECIALIST_CANDIDATE_VISUAL_MANIFEST_R30', 'status': 'CANDIDATE_GENERATED_NO_PASS', 'productionAuthorized': False, 'v1Facts': FACTS.relative_to(ROOT).as_posix(), 'rows': rows, 'note': 'Generated from 5 fresh source-only function/cartesian facts and 3 fresh source-only inequality number-line facts; no semantic or final PASS is claimed.'}
