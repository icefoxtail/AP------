...
const BASE = JSON.parse(fs.readFileSync(path.join(REPORT, '83_specialist_candidate_bank_manifest_r12.json'), 'utf8'));
...
const OUT = path.join(ROOT, 'archive', '_generated', 'hs-quadratic-svg-upgrade-20260908', 'candidate-r13', 'exams');
const OUTPUT = path.join(REPORT, '103_specialist_candidate_bank_manifest_r13.json');
...
for (let index = 0; index < BASE.candidateFiles.length; index += 1) {
  ...
  for (const question of candidate.questionBank) {
    const visual = visualByKey.get(key(file.sourcePath, question.id));
    if (!visual) continue;
    const fact = factByKey.get(key(file.sourcePath, question.id));
    ...
    question.solutionImageAlt = '이차부등식 해집합과 구간 최댓값·최솟값을 확인하는 후보 해설 시각자료';
    question.solutionImageCaption = '후보 SVG: 독립 계산으로 확정한 해집합·끝점·핵심 값을 그림에서 확인한다.';
  ...
  actualVisualCount += candidate.questionBank.filter((question) => question.solutionImage).length;
  ...
  candidateFiles.push({ sourcePath: file.sourcePath, candidatePath: `archive/_generated/hs-quadratic-svg-upgrade-20260908/candidate-r13/exams/${outputName}`, sourceQuestionCount: source.questionBank.length, candidateQuestionCount: candidate.questionBank.length, candidateFileSha256: sha(fs.readFileSync(outputPath)) });
}
...
const output = {
  schemaVersion: 'HS_QUADRATIC_SPECIALIST_CANDIDATE_BANK_MANIFEST_R13',
  status: errors.length ? 'SPECIALIST_CANDIDATE_BANK_FAIL' : 'SPECIALIST_CANDIDATE_BANK_ATTACHED_NO_PASS',
  productionAuthorized: false,
  baseCandidateManifest: 'reports/hs-quadratic-svg-upgrade-20260908/83_specialist_candidate_bank_manifest_r12.json',
  v1Facts: 'reports/hs-quadratic-svg-upgrade-20260908/101_specialist_v1_expected_facts_r13.json',
  visualManifest: 'reports/hs-quadratic-svg-upgrade-20260908/102_specialist_candidate_visual_manifest_r13.json',
  ...
  rows,
  errors,
  note: 'Candidate-r13 carries the validated r12 bank and adds 18 fresh specialist candidate visuals. Unique visual count is used; production source/assets and final PASS remain untouched.',
};
