...
const OUTPUT = path.join(REPORT, '19_legacy_evidence_alignment.json');
const LEGACY = [
...
function main() {
  ...
  const legacyRows = LEGACY.flatMap(file => parseCsv(fs.readFileSync(path.join(ROOT, file), 'utf8'))).filter(row => current.some(item => item.questionUid === row.questionUid));
  const legacyByUid = new Map(legacyRows.map(row => [row.questionUid, row]));
  const expectedRows = EXPECTED.flatMap(file => fs.readFileSync(path.join(ROOT, file), 'utf8').split(/\r?\n/).filter(Boolean).map(line => JSON.parse(line))).filter(row => current.some(item => item.questionUid === row.questionUid));
  const expectedByUid = new Map(expectedRows.map(row => [row.questionUid, row]));
  ...
  for (const item of current) {
    ...
    const old = legacyByUid.get(item.questionUid) || null;
    const fieldMatches = old ? Object.fromEntries(Object.keys(currentHashes).map(field => [field, currentHashes[field] === old[field]])) : {};
    rows.push({ questionUid: item.questionUid, currentSourceFileSha256: item.sourceFileSha256, legacyInventoryPresent: Boolean(old), legacyExpectedFactPresent: expectedByUid.has(item.questionUid), currentHashes, legacyHashes: old ? Object.fromEntries(Object.keys(currentHashes).map(field => [field, old[field]])) : null, fieldMatches, legacySourceStatus: old?.sourceValidityStatus || null, legacyVisualCandidate: old?.visualRequirement || null, currentVisualDecision: item.visualDecision, status: old && Object.values(fieldMatches).every(Boolean) && expectedByUid.has(item.questionUid) ? 'LEGACY_ALIGNMENT_ONLY' : 'CURRENT_REVIEW_REQUIRED' });
  }
  const output = { schemaVersion: 'HS_QUADRATIC_LEGACY_EVIDENCE_ALIGNMENT_V1', status: 'DIAGNOSTIC_ONLY_NO_PASS', scope: { currentTargetCount: current.length, legacyInventoryRowsMatched: legacyRows.length, legacyExpectedRowsMatched: expectedRows.length }, counts: { allFourHashesMatch: rows.filter(row => row.status === 'LEGACY_ALIGNMENT_ONLY').length, anyCurrentFieldDrift: rows.filter(row => row.legacyInventoryPresent && Object.values(row.fieldMatches).some(match => !match)).length, missingLegacyInventory: rows.filter(row => !row.legacyInventoryPresent).length, missingLegacyExpectedFacts: rows.filter(row => !row.legacyExpectedFactPresent).length }, rows, note: 'Legacy inventory/expected facts are diagnostic alignment only. Even exact raw field matches do not create current V1/A1/V2/V3 evidence or solution freeze.' };
