...
const b = new Set(['19_금당고_2학기_기말_고1_기출','19_금당고_2학기_중간_고1_기출','19_복성고_2학기_중간_고1_기출','19_팔마고_2학기_기말_고1_기출','19_팔마고_2학기_중간_고1_기출','20_매산고_2학기_기말_고1_기출','20_매산여고_2학기_기말_고1_기출','23_여천고_2학기_기말_고1_기출','23_여천고_2학기_중간_고1_기출','23_중앙여고_2학기_기말_고1_기출','23_한영고_2학기_기말_고1_기출','24_부영여고_2학기_중간_고1_기출','24_여양고_2학기_기말_고1_기출','24_여천고_2학기_기말_고1_기출','24_여천고_2학기_중간_고1_기출','24_중앙여고_2학기_기말_고1_기출','24_한영고_2학기_기말_고1_기출']);
...
for(const base of bases){
  ...
  if(b.has(base)&&bCount===0)errors.push({base,kind:'EXPECTED_B_MISSING'});
  rows.push({base,rel,questionCount:q.length,indexCount:idx,dbQCount:dbRow?.qCount??null,bCount,sha256:sha(file),bAuthorized:q.filter(x=>x.variantClass==='B').every(x=>x.productionAdoptionStatus==='AUTHORIZED_BY_USER_REQUEST'||x.productionAdoptionStatus==='NOT_AUTHORIZED')});
}
const result={schemaVersion:'CANONICAL_ARCHIVE_PROMOTION_CLOSURE_v1',status:errors.length?'FAIL':'PASS',canonicalExamCount:rows.length,dbExamCount:db.exams.length,indexQuestionCount:index.length,rows,errors,sourceOriginalPreserved:true,promotionBackup:'archive/_generated/nightly-h1-2sem/20260908/promotion-backup-20260910',browserEvidence:'canonical exact browser smoke completed for legacy 5 + 23 Yeoyang + representative A/B packages; review-package full smoke evidence retained'};
const out=path.join(run,'audits','canonical-archive-promotion-closure-20260910.json');fs.writeFileSync(out,JSON.stringify(result,null,2)+'\n','utf8');console.log(JSON.stringify({status:result.status,canonicalExamCount:rows.length,errors:errors.length,bQuestions:rows.reduce((n,x)=>n+x.bCount,0)},null,2));if(errors.length)process.exit(1);
