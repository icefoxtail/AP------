...
foreach ($item in $items) {
  ...
  if (Test-Path -LiteralPath $item.manifestPath) {
    try { $existing = Get-Content -LiteralPath $item.manifestPath -Raw -Encoding utf8 | ConvertFrom-Json } catch { }
  }
  if ($existing -and [string]$existing.status -eq 'DONE') {
    $batch.results += [ordered]@{ key = $item.key; status = 'SKIPPED_ALREADY_DONE'; manifestPath = $item.manifestPath }
    continue
  }
  $jobPath = Join-Path (Split-Path -Parent $ManifestPath) ('print_job_' + [string]$item.key + '.json')
  [ordered]@{
  ...
  $args = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $ConverterPath, '-JobFile', $jobPath, '-SecurityDll', $SecurityDll)
  ...
  if (!$finished) {
    ...
    if ($null -eq $childResult) {
      ...
      $batch.results += [ordered]@{ key = $item.key; status = [string]$childResult.status; exitCode = $child.ExitCode; manifestPath = $item.manifestPath; outputPdf = [string]$childResult.outputPdf; error = [string]$childResult.error }
    }
  }
}

$batch.finishedAt = (Get-Date).ToString('o')
$batch | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath ($ManifestPath + '.print-result.json') -Encoding utf8
