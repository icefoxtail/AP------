...
const work = path.join(root, "work/23_hanyeong_2mid");
...
if (sandbox.window.questionBank.length !== 21) throw new Error("included count mismatch");
if (sandbox.window.questionBank.some((question) => !question.answer || !question.solution)) throw new Error("answer/solution blank");
...
if (!assets.length) throw new Error("no PNG assets");
await fs.writeFile(path.join(packageDir, "reports/BUILD_SANITY.md"), `# BUILD SANITY — ${title}\n\n- native HWP PDF generated from the original HWP\n- source question count: 21\n- included independently solved questions: 21\n- excluded questions: none\n- q11 answer-key conflict retained as review metadata; direct derivation included\n- JS syntax: PASS\n- Node VM load: PASS\n- answers/solutions blank among included: 0\n- native PDF and original HWP copied: PASS\n- PNG assets: ${assets.length}\n- ZIP fresh extraction: checked after archive creation\n`, "utf8");

const files = (await walk(packageDir)).filter((file) => path.basename(file) !== "sha256_manifest.txt").sort();
await fs.writeFile(path.join(packageDir, "reports/sha256_manifest.txt"), files.map((file) => `${sha256(file)}  ${rel(file, packageDir)}`).join("\n") + "\n", "utf8");
...
if (freshCtx.window.questionBank.length !== 21) throw new Error("fresh extracted count mismatch");
const pngCheck = `import sys\nfrom pathlib import Path\nfrom PIL import Image\nroot=Path(sys.argv[1])\nfiles=list(root.rglob('*.png'))\nassert files\nfor p in files:\n    with Image.open(p) as im: im.verify()\nprint(len(files))`;
await execFileAsync("python", ["-c", pngCheck, fresh]);
console.log(JSON.stringify({ status: "PASS", zipPath, zipSha256: sha256(zipPath), included: 21, excluded: [], pngCount: assets.length }, null, 2));
