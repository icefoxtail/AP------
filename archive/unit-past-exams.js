(function () {
  ...
  function renderConfigStep(unit) {
    ...
    const existingPapers = state.sourceMode === 'archive' && unit.papers.length ? `<section class="unit-existing-section"><h3>기존 시험지</h3><div class="unit-existing-list">${unit.papers.map(paper => renderExistingPaperCompact(paper, unit)).join('')}</div></section>` : '';
