...
function build() {
    ...
    const blindFreezeStatus = ledger.legacyCompare?.status === 'COMPLETE_RECHECK_PENDING'
        ? 'BLIND_FREEZE_LEGACY_COMPARE_COMPLETE_RECHECK_PENDING'
        : 'BLIND_FIRST_PASS_HEURISTIC_CANDIDATE_PENDING_INDEPENDENT_GPT_RECHECK';
    fs.writeFileSync(path.join(evidenceDir, 'difficulty_blind_freeze_928.json'), JSON.stringify({ schemaVersion: 'middle-geometry-difficulty-blind-freeze-v1', status: blindFreezeStatus, denominator: 928, bucketCounts, confidenceCounts, compatibilityCounts, routeOutUnknownCount: ledger.records.filter(record => record.difficultyBucket === 'UNKNOWN').length, independentRecheckTargetCount: recheckTargets.length, targetQuestionUids: recheckTargets.map(record => record.questionUid), blindRule: 'blind first pass is candidate-only; legacy compare is post-freeze evidence; no reviewed_pass or eligibility is granted until independent recheck/adjudication', productionPromotion: 'NOT_ATTEMPTED' }, null, 2) + '\n');
