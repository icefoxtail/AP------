...
const validateRepair = (patch, current) => {
  ...
  const after = { ...current, ...patch.after };
  ...
  const l3 = after.problemTypeKey ? byUniqueKey(problemRows, "problemTypeKey", after.problemTypeKey) : null;
  const l4 = after.templateKey ? byUniqueKey(templateRows, "templateKey", after.templateKey) : null;
  if (l4 && (!l3 || l4.parentProblemTypeKey !== l3.problemTypeKey)) throw new Error(`invalid L4 parent ${l4.templateKey} -> ${l4.parentProblemTypeKey}; expected ${l3.problemTypeKey}`);
  ...
  if (l3 && matching.length !== 1) throw new Error(`curriculum/L1/L2/L3 binding must resolve to one ACTIVE row (${matching.length}) for ${patch.questionUid}`);
  const binding = matching[0] || { curriculum: after.curriculum || after.curriculumKey, standardCourse: after.standardCourse || after.courseKey };
...
for (const patch of packet.metaPatches) {
  ...
  if (patch.status === "REPAIR") {
    ...
    if (!sameApply) {
      ...
      nextRecord.L3 = canonical.l3?.canonicalLabelKo || null;
      nextRecord.L4 = canonical.l4?.canonicalLabelKo || null;
      nextRecord.conceptClusterKey = patch.after.conceptClusterKey || patch.after.subUnitKey;
      nextRecord.crossConceptKeys = [...(patch.after.crossConceptKeys || current.crossConceptKeys || [])];
