...
for (const optional of ['CONSENSUS_PLAN.json','SOURCE_QUALITY_PLAN.json','DIFFICULTY_DRAFT.jsonl','C_QUALITY_DEFECTS.json','DIFFICULTY_INPUT_PRE_L4_REUSE.jsonl','B11_L4_REUSE_INPUT_DELTA.json','DIFFICULTY_BLIND_INPUT_Q19.jsonl','Q19_SHARED_MATERIAL_REF.json','DIFFICULTY_BLIND_CORRECTION_Q19_RAW.jsonl','DIFFICULTY_BLIND_CORRECTION_Q19.jsonl','B11_Q19_SCHEMA_NORMALIZATION.json','B11_DIFFICULTY_QUALITY_REJECTION.json','DIFFICULTY_RECHECK_QUEUE_PRE_ADJUDICATION.jsonl','B11_RECHECK_TRIGGER_ADJUDICATION.json']) if (fs.existsSync(path.join(dir, optional))) artifactNames.push(optional);
...
const lines = [
  ...
  `- Worker-quality affected UIDs with targeted closure: **${validation.counts.workerQualityRejectedUidCount || 0}** (C evidence rejected: ${validation.counts.cQualityRejectedUidCount || 0})`,
