...
if (batchNo >= 10) requiredChecked.push('l1L2BaselineAuthority');
...
const l1L2Baseline = batchNo >= 10 ? JSON.parse(fs.readFileSync(path.join(dir, 'L1_L2_BASELINE_SUMMARY.json'), 'utf8')) : null;
...
for (const optional of ['CONSENSUS_PLAN.json','SOURCE_QUALITY_PLAN.json','DIFFICULTY_DRAFT.jsonl','C_QUALITY_DEFECTS.json','AB_QUALITY_DEFECTS.json','B13_WORKER_QUALITY_REJECTION_A.json','B13_WORKER_QUALITY_REJECTION_B.json','LUNA_A_PRE_SCHEMA_CORRECTION.jsonl','LUNA_A_SCHEMA_CORRECTION_RECEIPT.json','LUNA_A_PRE_TAXONOMY_ARRAY_REVIEW.jsonl','LUNA_A_ARRAY_REVIEW_RECEIPT.json','LUNA_B_PRE_TAXONOMY_ARRAY_REVIEW.jsonl','LUNA_B_ARRAY_REVIEW_RECEIPT.json','A_REVIEW_INPUT_Q06.jsonl','LUNA_A_PRE_Q06_QUALITY_CORRECTION.jsonl','LUNA_A_REVISION_Q06.jsonl','LUNA_A_REVISION_Q06_NORMALIZED.jsonl','A_Q06_SCHEMA_NORMALIZATION.json','B13_A_Q06_QUALITY_REJECTION.json','CONFLICT_C_PRE_SCHEMA_CORRECTION.jsonl','CONFLICT_C_SCHEMA_PARENT_REVIEW_RECEIPT.json','DIFFICULTY_INPUT_PRE_L4_REUSE.jsonl','B11_L4_REUSE_INPUT_DELTA.json','DIFFICULTY_BLIND_INPUT_Q19.jsonl','Q19_SHARED_MATERIAL_REF.json','DIFFICULTY_BLIND_CORRECTION_Q19_RAW.jsonl','DIFFICULTY_BLIND_CORRECTION_Q19.jsonl','B11_Q19_SCHEMA_NORMALIZATION.json','B11_DIFFICULTY_QUALITY_REJECTION.json','DIFFICULTY_BLIND_INPUT_Q12.jsonl','DIFFICULTY_BLIND_CORRECTION_Q12.jsonl','DIFFICULTY_BLIND_CORRECTION_Q12_POLICY_REVIEW_RAW.jsonl','DIFFICULTY_BLIND_CORRECTION_Q12_POLICY_REVIEW.jsonl','B12_Q12_POLICY_REVIEW_RECEIPT.json','B12_Q12_POLICY_FIELD_NORMALIZATION.json','B12_DIFFICULTY_QUALITY_REJECTION.json','DIFFICULTY_RECHECK_QUEUE_PRE_ADJUDICATION.jsonl','B11_RECHECK_TRIGGER_ADJUDICATION.json']) if (fs.existsSync(path.join(dir, optional))) artifactNames.push(optional);
...
for (const name of fs.readdirSync(dir).filter(name => name.startsWith('ROOT_')).sort()) if (!artifactNames.includes(name) && fs.statSync(path.join(dir, name)).isFile()) artifactNames.push(name);
...
const lines = [
  ...
  ...(l1L2Baseline ? [`- Existing L1/L2 baseline used UIDs: **${l1L2Baseline.baselineUsedUidCount}/${exam.questionRowCount}**`, `- Confirmed L1_L2_CONFLICT UIDs: **${l1L2Baseline.l1L2ConflictCount}**`] : []),
  ...(l1L2Baseline?.rootOnlyCandidateCount !== undefined ? [`- Root-opened source-grounded L1/L2 candidates not flagged by A/B: **${l1L2Baseline.rootOnlyCandidateCount}**`] : []),
  ...
  `- Worker-quality affected UIDs with targeted closure: **${validation.counts.workerQualityRejectedUidCount || 0}** (C evidence rejected: ${validation.counts.cQualityRejectedUidCount || 0}; A/B evidence rejected: ${validation.counts.abQualityRejectedUidCount || 0}; A targeted replacement: ${validation.counts.aQualityRejectedUidCount || 0})`,
  `- Rejected pre-ledger worker attempts (accepted UID 0): **${validation.counts.workerQualityRejectedAttemptCount || 0}**`,
  `- Worker A schema / taxonomy-array corrected UIDs before consensus: **${validation.counts.aSchemaCorrectedUidCount || 0}/${validation.counts.aTaxonomyArrayRevisedUidCount || 0}**`,
  `- Worker B taxonomy-array corrected UIDs before consensus: **${validation.counts.bTaxonomyArrayRevisedUidCount || 0}**`,
  `- Conflict C schema-corrected / parent-rejudged UIDs before consensus: **${validation.counts.cSchemaCorrectedUidCount || 0}/${validation.counts.cParentRejudgedUidCount || 0}**`,
  ...
  ...(batchNo >= 13 ? [`- Independent difficulty recheck runtime model verified: **${validation.counts.difficultyRecheckModelVerified === true}**`] : []),
