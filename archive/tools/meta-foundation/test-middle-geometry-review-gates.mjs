...
assert.equal(audit.gates.aliasAuthorityParity.compiledAliasCount, 177);
assert.equal(audit.gates.aliasAuthorityParity.compiledDistinctAliasCount, 177);
assert.equal(audit.gates.aliasAuthorityParity.rawAliasDistinct, 177);
assert.equal(audit.gates.aliasAuthorityParity.rawToCompiledDistinctParity, true);
assert.equal(audit.candidateCounts.autoEligible, 0);
assert.equal(runtime.counts.autoEligible, 0);
assert.ok(ledger.records.filter(record => record.problemTypeKey).every(record => record.difficultyBoundaryFlag === 'UNKNOWN' && record.legacyLevelCompatibility === 'UNKNOWN'));
