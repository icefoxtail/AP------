...
export function auditSemanticKernel(root, run, freshnessRows = null) {
  ...
  for (const q of run.questions) {
    ...
    if (run.schemaVersion === RUN_VERSION_V2 && policy.scope === 'QUESTION_QUALITY') {
      ...
      if (!a2 || !nonempty(a2.payload?.independentAnswer) || !nonempty(a2.payload?.independentDerivation) || a2.payload?.allChoicesChecked !== true || a2.payload?.answerUnique !== true || a2.priorReviewVisibility !== 'NONE') findings.push('MATH_A2_INDEPENDENT_CANDIDATE_REVIEW_REQUIRED');
      if (a1 && a2 && a1.payload?.independentAnswer !== a2.payload?.independentAnswer) findings.push('REVIEW_CONFLICT:MATH_ANSWER');
    }
    ...
    if (policy.visual) {
      ...
      findings.push(...validateVisualBenefitPair(triage, adjudication, { independent: run.schemaVersion === RUN_VERSION_V2, question: candidateQuestions.get(q.questionUid), visual: q.visual, ruleRefs: run.inputs.filter(ref => ref.role === 'rule') }).errors);
    ...
    if (needed && policy.visual) {
      ...
      if (v1 && v2 && v3) {
        ...
        if (v1.inputVisibilityProfile !== 'SOURCE_ONLY' || v2.inputVisibilityProfile !== 'ARTIFACT_ONLY' || v3.inputVisibilityProfile !== (run.schemaVersion === RUN_VERSION_V2 ? 'CANDIDATE_ONLY' : 'FROZEN_V1_V2') || [v1, v2].some(e => e.priorReviewVisibility !== 'NONE' || e.payload?.freshBlind !== true) || run.schemaVersion === RUN_VERSION_V2 && v3.priorReviewVisibility !== 'NONE') findings.push('BLIND_VISIBILITY_INVALID');
