...
export function fixture(pipeline = 'logic-visual', { visual = true } = {}) {
  ...
  if (visual && profiles.pipelines[pipeline].visual) {
    ...
    const observed = add('v2', { freshBlind: true, specSha: VISUAL_SPEC_SHA, fact: fact(uid), observedFacts: [{ id: 'intersection', status: 'OBSERVED', observation: 'Artifact shows the intersection extrema.' }], inputBundle: v2Bundle, artifactPath: 'assets/visual.svg', artifactSha: fileRef(root, 'assets/visual.svg').sha256, structureSha: structureFingerprint(fact(uid)) }, { inputVisibilityProfile: 'ARTIFACT_ONLY' });
    add('v3', { v1EvidenceSha: expected.sha256, v2EvidenceSha: observed.sha256, finalVisualRequirement: run.questions[0].visual.requirement, cDenominatorInputSha: run.denominator.inputSha, EXPECTED_CRITICAL_FACT_COUNT: 1, OBSERVED_CRITICAL_FACT_COUNT: 1, CRITICAL_FACT_COVERAGE: 1, EXPECTED_OBSERVED_FACT_PARITY: 'PASS', checks: Object.fromEntries(['necessity', 'decisiveStep', 'completeness', 'mediumFit', 'solutionParity', 'altCaptionParity', 'semanticsLocks', 'staticContract'].map(k => [k, 'PASS'])) }, { inputVisibilityProfile: 'FROZEN_V1_V2', startedAt: '2026-09-06T01:02:00Z', frozenAt: '2026-09-06T01:03:00Z' });
  }
  if (!visual && profiles.pipelines[pipeline].visual) {
