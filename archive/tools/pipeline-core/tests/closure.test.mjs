...
const mutations = {
  ...
  'mobile width substituted desktop': f => f.rewriteEvidence('solution-mobile', e => { e.payload.viewport.width = 1280; }),
  'missing screenshot file': f => { fs.unlinkSync(path.join(f.root, 'screens/solution-mobile.png')); },
  'PNG truncated but rehashed': f => f.rewriteEvidence('solution-mobile', e => { e.payload.screenshot = f.write('screens/solution-mobile.png', Buffer.from('89504e470d0a1a0a', 'hex')); }),
  'asset association wrong UID': f => f.rewriteEvidence('solution-mobile', e => { e.payload.assetAssociations[0].questionUid = 'other'; }),
