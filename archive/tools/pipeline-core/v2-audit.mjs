...
import { RUN_VERSION_V2, CORE_SHA, profiles, runInputSha, auditSemanticKernel, createAuditSnapshot, loadBoundQuestionBanks } from './closure.mjs';
...
const load = (root, ref, snapshot = null) => snapshot?.json(ref) || JSON.parse(readBoundFile(root, ref));
...
export function computeV2AxisInputShas(root, run) {
  ...
  return Object.fromEntries(loadBoundQuestionBanks(root, run, { snapshot }).map(q => {
    const declared = run.questions.find(row => row.questionUid === q.questionUid);
    return [q.questionUid, Object.fromEntries(requiredAxesForQuestion(profiles.pipelines[run.pipeline], declared, run).map(axis => [axis, axisInputSha(q, axis, projectionContext(root, run, q, axis, snapshot))]))];
  }));
}

function projectionContext(root, run, question, axis, snapshot = null) {
  ...
  const dependencies = run.inputs.filter(ref => !['source', 'candidate', 'rule', 'verifier'].includes(ref.role)).filter(ref => {
    ...
    if (snapshot) snapshot.read(ref); else readBoundFile(root, ref);
    ...
    if (scoped) {
      const classification = load(root, declared.authorityRef, snapshot);
  ...
  for (const ref of [...rules, ...verifiers]) if (snapshot) snapshot.read(ref); else readBoundFile(root, ref);
...
function bindAuthority(root, run, snapshot = null) {
  ...
  for (const ref of [...source.sourceTruthRefs, source.activeBaselineRef]) if (snapshot) snapshot.read(ref); else readBoundFile(root, ref);
  ...
  for (const key of ['baselineDiscoveryEvidenceRef', 'approvedSourceRepairLedgerRef', 'approvedSourceExceptionLedgerRef']) {
    ...
    const evidence = load(root, applicability.status === 'REQUIRED' ? source[key] : applicability.evidenceRef, snapshot);
  ...
  const registry = normalizeSourceExamIdRegistry(load(root, authority?.sourceExamIdRegistryRef, snapshot));
  ...
  const migrations = authority.uidMigrationEvidenceRefs.map(ref => load(root, ref, snapshot));
...
function validateEvidenceLifecycle(root, run, evidenceRefs, snapshot = null) {
  const errors = [];
  ...
  for (const row of run.evidenceLifecycle || []) {
    try {
      ...
      const evidence = snapshot?.json(row.evidenceRef) || JSON.parse(readBoundFile(root, row.evidenceRef));
...
export function auditV2Run(root, run) {
  ...
  try {
    ...
    for (const ref of run.inputs) snapshot.read(ref);
    ...
    bindAuthority(root, run, snapshot);
    const actual = loadBoundQuestionBanks(root, run, { snapshot });
    ...
    const contexts = Object.fromEntries(actual.map(q => [q.questionUid, Object.fromEntries(required[q.questionUid].map(axis => [axis, projectionContext(root, run, q, axis, snapshot)]))]));
    ...
    if (run.revision > 1) {
      ...
      previous = load(root, p?.runRef, snapshot); predecessorClosure = load(root, p?.closureSetRef, snapshot);
      ...
      if (validateQuestionQualityClosureSet(predecessorClosure, { runId: previous.runId, revision: previous.revision, currentRunInputSha: previous.inputSha, questionUids: previous.questions.map(q => q.questionUid) }).status !== 'PASS') {
        ...
        const saved = load(root, p.partialAuditRef, snapshot), recomputed = auditV2Run(root, previous);
      ...
      before = loadBoundQuestionBanks(root, previous, { snapshot });
      previousAxisShas = Object.fromEntries(before.map(q => [q.questionUid, Object.fromEntries((required[q.questionUid] || []).map(axis => [axis, axisInputSha(q, axis, projectionContext(root, previous, q, axis, snapshot))]))]));
    ...
    for (const ref of run.evidence || []) { const e = load(root, ref, snapshot); errors.push(...validateSchema(e, contracts['evidence-v2'])); if (evidence.has(e.evidenceId)) throw new Error('DUPLICATE_EVIDENCE_ID'); evidence.set(e.evidenceId, e); evidenceRefs.set(e.evidenceId, ref); }
    errors.push(...validateEvidenceLifecycle(root, run, [...evidenceRefs.values()], snapshot));
    const receipts = (run.reuseReceipts || []).map(ref => load(root, ref, snapshot)), packets = (run.auditorPacketRefs || []).map(ref => load(root, ref, snapshot));
    for (const receipt of receipts) errors.push(...validateSchema(receipt, contracts['reuse-receipt-v1']));
    for (const ref of run.renderReviewReuseReceiptRefs || []) errors.push(...validateSchema(load(root, ref, snapshot), contracts['render-review-reuse-receipt-v1']));
    ...
    if (captureRows.length && previous) {
      const oldRows = (previous.evidence || []).map(ref => load(root, ref, snapshot)).filter(e => e.axis === 'RENDER_CAPTURE').flatMap(e => e.payload?.itemWitnesses || []);
    ...
    for (const ref of run.contextDependencyRefs || []) {
      const dependency = load(root, ref, snapshot);
    ...
    const candidateContext = packets.some(packet => packet.phase === 'U3') ? loadCandidateReviewContext(root, run, { snapshot, assetCache }) : null;
    ...
    for (const q of run.questions) {
      ...
      if (q.visual?.requirement === 'VISUAL_OPTIONAL' && q.visual.origin === 'LEGACY_VISUAL_RECOMMENDED') {
        const migration = load(root, q.visual.migrationEvidenceRef, snapshot);
      ...
      for (const axis of required[q.questionUid]) {
        ...
        const rootEvidence = receipt ? load(root, receipt.rootFreshEvidenceRef, snapshot) : null, eligibility = receipt ? load(root, receipt.eligibilityEvidenceRef, snapshot) : null;
        if (receipt) {
          const lifecycle = load(root, receipt.lifecycleSnapshotRef, snapshot);
          ...
          const rootRun = load(root, receipt.rootFreshRunRef, snapshot), rootPacket = load(root, receipt.rootFreshPacketRef, snapshot);
      ...
      if (u1 && u2) {
        const packetFor = e => packets.find(p => p.packetSha === e.reviewIsolationProvenanceSha) || (() => { const receipt = receipts.find(r => r.rootFreshEvidenceId === e.evidenceId); return receipt ? load(root, receipt.rootFreshPacketRef, snapshot) : null; })();
    ...
    const ledger = (run.buildWorkLedgerRefs || []).map(ref => load(root, ref, snapshot));
    ...
    const closure = load(root, run.questionQualityClosureSetRef, snapshot);
    ...
    { // Both REQUIRED and canonical NOT_APPLICABLE closures are mandatory.
      const release = load(root, run.examReleaseClosureRef, snapshot);
    ...
    if (run.editClosureRef && !same(load(root, run.editClosureRef, snapshot), editClosure)) errors.push('EDIT_CLOSURE_DECLARATION_MISMATCH');
    for (const ref of [...run.inputs, ...(run.evidence || [])]) snapshot.read(ref);
