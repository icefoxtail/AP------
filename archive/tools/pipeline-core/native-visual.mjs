...
export function visualAssetPayload(root, ref, assetCache = null) {
  const cacheKey = `${ref?.path || ''}|${ref?.bytes ?? ''}|${ref?.sha256 || ''}`;
  ...
  if (path.extname(ref.path).toLowerCase() === '.svg') {
    ...
    assetCache?.set(cacheKey, native);
    return { ...ref, ...native };
  }
  const payload = { mimeType: mimeFor(ref.path), dataUrl: `data:${mimeFor(ref.path)};base64,${bytes.toString('base64')}` };
