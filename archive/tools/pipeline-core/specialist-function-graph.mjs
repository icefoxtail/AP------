...
export function auditFunctionGraphSpecialist(root, manifestPath) {
  ...
  let candidateManifest, bindings, render, renderReview, renderRefresh;
  try {
    ...
    renderRefresh = load(root, required(run.renderRefreshRef, 'RENDER_REFRESH'));
  ...
  for (const row of candidateRows) {
    ...
    for (const [axis, key] of [['V1', 'v1Evidence'], ['V2', 'v2Evidence'], ['V3', 'v3Evidence']]) {
      try {
        ...
        if (axis === 'V1' && !['apmath-add-general-v1-source-only', 'apmath-add-legacy-v1-source-only', 'apmath-rebuild-general-v1-source-only', 'apmath-source-only-v1', 'apmath-visual-v1-source-only-v1'].includes(evidence.schemaVersion)) rowErrors.push('V1_SCHEMA_ROUTE_MISMATCH');
        if (axis === 'V2' && !['apmath-add-general-v2-artifact-only', 'apmath-add-legacy-v2-artifact-only', 'apmath-rebuild-general-v2-artifact-only', 'apmath-artifact-only-v2', 'apmath-visual-v2-artifact-only-v1'].includes(evidence.schemaVersion)) rowErrors.push('V2_SCHEMA_ROUTE_MISMATCH');
        if (axis === 'V3' && !['apmath-add-general-v3-parity', 'apmath-add-legacy-v3-parity', 'apmath-rebuild-general-v3-parity', 'apmath-parity-v3', 'apmath-visual-v3-parity-v1'].includes(evidence.schemaVersion)) rowErrors.push('V3_SCHEMA_ROUTE_MISMATCH');
  ...
  if (renderRefresh?.status !== 'PASS' || renderRefresh?.expectedCases !== 18 || renderRefresh?.observedCases !== 18 || renderRefresh?.passCases !== 18 || renderRefresh?.failCases !== 0 || !Array.isArray(renderRefresh?.rows) || renderRefresh.rows.length !== 18 || renderRefresh.rows.some((row) => row.status !== 'PASS' || row.imageDecodeErrors !== 0 || row.mathErrors !== 0 || row.overflow !== false)) errors.push('RENDER_REFRESH_NOT_CLOSED');
