...
def risk_flags(question, full_pages):
    flags = []
...
def main():
    ...
    for candidate in sorted(root.rglob("*.candidate.js")):
        ...
        for question in questions:
            ...
            flags = risk_flags(question, pages)
            auxiliary_warnings = []
            if not stats.get("exists"):
                auxiliary_warnings.append("optional_zoom_crop_missing")
            else:
                if stats.get("blankLike"):
                    auxiliary_warnings.append("optional_zoom_crop_blank_like")
                if stats.get("tiny"):
                    auxiliary_warnings.append("optional_zoom_crop_tiny")
            for flag in flags:
                flag_counts[flag] += 1
                by_term[term][flag] += 1
            row = {
                ...
                "primaryEvidenceImagePaths": pages,
                "sourceTruthImagePaths": pages,
                "auxiliaryQuestionCropPath": crop,
                "auxiliaryQuestionCropStats": stats,
                "auxiliaryEvidenceWarnings": auxiliary_warnings,
                "fullPageImagePaths": pages,
                "sourceFile": question.get("sourceFile") or "",
                "reviewFlags": flags,
                "sourceEvidencePolicy": "full_page_first_crop_zoom_only",
                "reviewInstruction": "Compare JS content/choices against fullPageImagePaths/sourceTruthImagePaths first. Use auxiliaryQuestionCropPath only as optional zoom evidence. Do not mark content/choices wrong merely because an auxiliary crop is missing, blank, or narrow. Do not inspect or change answer/solution in this round.",
