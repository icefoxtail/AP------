...
const required = ['examUid', 'examFile', 'grade', 'lane', 'stage', 'sourceBlobSha', 'inputCommit', 'totalQuestions', 'changedQuestions', 'changedSvgFiles', 'metaDispositionSummary', 'unresolvedItems', 'authorityRefs', 'nextState', 'updatedAt'];
const fullExamPath = value => value.startsWith('archive/exams/') ? value : `archive/exams/${value.replace(/^exams\//, '')}`;
export function inventory(repo, { fetch = true } = {}) {
