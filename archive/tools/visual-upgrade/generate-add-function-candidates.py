...
def write(name,title,desc,body,fact):
    ...
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 420" width="720" height="420" preserveAspectRatio="xMidYMid meet" data-graph-style-version="AP_GRAPH_PRINT_V1_1_DRAFT" data-graph-preset="SOLUTION_GRAPH" data-fact-hash="{h}" data-visual-provenance="deterministic-python-fact-model-candidate"><title>{esc(title)}</title><desc>{esc(desc)}</desc><style>.axis{{stroke:#111;stroke-width:1.4;fill:none}}.guide{{stroke:#777;stroke-width:.9;stroke-dasharray:5 4;fill:none}}.curve{{stroke:#111;stroke-width:2;fill:none;stroke-linecap:round}}.label{{font:13px {FONT};fill:#111}}</style><rect width="100%" height="100%" fill="white"/>{body}</svg>\n'''
...
for lo,hi in [(-math.pi,-math.pi/3),(-math.pi/3,math.pi/3),(math.pi/3,math.pi)]:
    pts=[]
    for i in range(301):
        x=lo+(hi-lo)*i/300; y=math.tan(1.5*x)
...
x0,x1=-1,5; pts=[]
for i in range(601):
    x=x0+(x1-x0)*i/600; y=2**abs(x-2)+3; pts.append(f"{sx(x,x0,x1):.2f},{sy(y,3,16):.2f}")
body=f'<line class="axis" x1="70" y1="{sy(3,3,16):.2f}" x2="690" y2="{sy(3,3,16):.2f}"/><line class="axis" x1="{sx(0,x0,x1):.2f}" y1="30" x2="{sx(0,x0,x1):.2f}" y2="374"/><line class="guide" x1="70" y1="{sy(4,3,16):.2f}" x2="690" y2="{sy(4,3,16):.2f}"/><polyline class="curve" points="{" ".join(pts)}"/><circle cx="{sx(2,x0,x1):.2f}" cy="{sy(4,3,16):.2f}" r="4" fill="#111"/><text class="label" x="80" y="24">y=2^abs(x−2)+3, minimum 4 at x=2</text>'
fact={"questionUid":"25_순천여고_1학기_중간_고2_대수::q14","expression":"y=2^{|x-2|}+3","minimum":4,"minimumAt":2}
