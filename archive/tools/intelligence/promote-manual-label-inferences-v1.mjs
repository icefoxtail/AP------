...
const adjudicationPath = path.join(phase1Dir, 'master-audit', 'manual-label-adjudication', 'manual-label-adjudication-v1.json');
...
function applySourceFile(relativeFile, rows, subunitLabels) {
    ...
    for (const row of rows) {
        ...
        const subUnit = subunitLabels.get(row.proposedSubUnitKey);
        if (!subUnit) throw new Error(`subUnit not found in master: ${row.proposedSubUnitKey}`);
        after = replaceQuestionBlock(after, question.id, {
            ...
            subUnitKey: row.proposedSubUnitKey,
            subUnit,
...
export function promoteManualInferences() {
    const adjudication = JSON.parse(fs.readFileSync(adjudicationPath, 'utf8'));
    if (adjudication.totals.holdQuestions !== 0 || adjudication.missingDecisions.length || adjudication.invalidTargets.length) {
        throw new Error('manual adjudication contains unresolved or invalid targets');
    }
    ...
    for (const row of rows) {
        ...
        record.classification.confidence = 'category_or_cue_inferred';
        record.classification.classificationDepth = 'complete_category';
        record.classification.uncertainty = false;
        record.classification.evidence = {
            ...(record.classification.evidence || {}),
            sourceDisposition: 'MANUAL_LABEL_CONTENT_INFERRED',
            observedLabel: row.evidence.observedLabel,
            rationale: row.evidence.rationale,
            currentStandardUnitKey: row.evidence.currentStandardUnitKey,
            currentSubUnitKey: row.evidence.currentSubUnitKey
        };
        record.classification.productionWriteAllowed = false;
    }
    ...
    const stable = {
        schemaVersion: 'phase1-manual-label-promotion-v1',
        ...
        aggregate: { sourceGroups: adjudication.totals.groups, sourceQuestions: adjudication.totals.questions, inferredQuestions: rows.length, unresolvedQuestions: adjudication.totals.holdQuestions },
...
function main() {
    const report = promoteManualInferences();
    fs.mkdirSync(outputDir, { recursive: true });
    fs.writeFileSync(path.join(outputDir, 'manual-label-promotion-v1.json'), JSON.stringify(report, null, 2) + '\n', 'utf8');
    console.log(JSON.stringify({ output: path.relative(archiveDir, path.join(outputDir, 'manual-label-promotion-v1.json')).replace(/\\/g, '/'), digest: report.digest, totals: report.totals, aggregate: report.aggregate, gates: report.gates }, null, 2));
}
