...
/** Materialize the next source-order batch for independent review. */
...
export function buildSequentialSubunitReviewBatchV1() {
    ...
    const stablePayload = {
        schemaVersion: 'archive-sequential-subunit-review-batch-v1',
        ...
        totals: {
            ...
            sourceJoinFailures,
            pendingIndependentReview: records.filter(record => record.reviewStatus === 'PENDING_INDEPENDENT_REVIEW').length,
            checkpointConfirmations: records.filter(record => record.reviewStatus === 'CHECKPOINT_CONFIRMATION').length,
...
function main() {
    ...
    fs.writeFileSync(path.join(outputDir, 'archive-sequential-subunit-review-batch-001-v1.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
    console.log(JSON.stringify({
        output: 'archive/_generated/intelligence/phase3/sequential-review/archive-sequential-subunit-review-batch-001-v1.json',
