...
export function buildSequentialSubunitReviewBatch004V1() {
  ...
  const records = queueRecords.map(record => {
    ...
    return { ...record, reviewStatus: 'PENDING_INDEPENDENT_REVIEW', source: sourcePayload(source) };
  });
  const stablePayload = { schemaVersion: 'archive-sequential-subunit-review-batch-004-v1', queueDigest: queue.digest, batchNumber, batchSize: queue.batchSize, productionWriteAllowed: false, totals: { records: records.length, sourceJoinFailures, pendingIndependentReview: records.length, checkpointConfirmations: 0, missingSourceContent: records.filter(record => !record.source?.content).length }, records };
  return { generatedAt: new Date().toISOString(), digest: sha256(JSON.stringify(stablePayload)), ...stablePayload };
}

function main() {
  const report = buildSequentialSubunitReviewBatch004V1();
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(path.join(outputDir, 'archive-sequential-subunit-review-batch-004-v1.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify({ output: 'archive/_generated/intelligence/phase3/sequential-review/archive-sequential-subunit-review-batch-004-v1.json', digest: report.digest, totals: report.totals }, null, 2));
}
