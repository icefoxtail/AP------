...
function assertDbSourceParity(examFiles, dbExams) {
    const sourceFiles = new Set(
        examFiles
            .map(file => normalizePath(path.relative(examsDir, file)))
            .filter(isProductionExamRelativePath)
    );
    ...
    const missingInDb = [...sourceFiles].filter(file => !dbFiles.has(file)).sort();
    const missingInSource = [...dbFiles].filter(file => !sourceFiles.has(file)).sort();
    if (missingInDb.length || missingInSource.length) {
        const details = [
            missingInDb.length ? `missing in db.js (${missingInDb.length}): ${missingInDb.slice(0, 20).join(', ')}` : '',
            missingInSource.length ? `missing in archive/exams (${missingInSource.length}): ${missingInSource.slice(0, 20).join(', ')}` : ''
        ].filter(Boolean).join(' | ');
        throw new Error(`DB_SOURCE_PARITY_FAIL: ${details}`);
    }
}
...
function getTrackedExamFiles() {
    ...
    return out.toString('utf8')
        ...
        .filter(file => isProductionExamRelativePath(path.relative(examsDir, file)))
...
function collectExamFiles() {
    ...
    try {
        ...
        if (tracked.length) {
            ...
            return { files: official.sort(), scope: 'git-tracked + db-listed production (parity-gated)' };
        }
    } catch (error) {
...
assertDbSourceParity(examFiles, db.exams);
...
for (const file of examFiles) {
    ...
    try {
        ...
        const expectedQuestionCount = Number(meta.qCount);
        if (Number.isInteger(expectedQuestionCount) && expectedQuestionCount !== questions.length) {
            report.failures.push({
                sourceFile,
                error: `qCount mismatch: db=${expectedQuestionCount}, js=${questions.length}`
            });
            continue;
        }
...
// Keep generated registration output reproducible so archive-registration-sync
// is a no-op when it runs after a reviewed apply commit.
...
// Fail closed before writing any generated index when source loading,
// metadata, qCount, or DB/source parity is invalid. A partial index is more
// dangerous than a missing index because downstream consumers can load it.
if (report.failures.length) {
    console.error(`question-index build failed: ${report.failures.length} file(s) require review`);
    for (const failure of report.failures.slice(0, 20)) {
        console.error(` - ${failure.sourceFile}: ${failure.error}`);
    }
    process.exitCode = 1;
    process.exit(1);
}
