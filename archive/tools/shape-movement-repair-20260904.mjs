...
function pointLabel(p) {
  const hasDecimalCoordinate = [p.x, p.y].some(value => Math.abs(value - Math.round(value)) >= 1e-9);
  return hasDecimalCoordinate ? p.label : p.label + ' (' + fmtLabel(p.x) + ',' + fmtLabel(p.y) + ')';
}
...
function render(spec) {
  ...
  for(const p of spec.points || []) body.push('<g data-point-label="'+esc(p.label)+'" data-point-x="'+fmt(p.x)+'" data-point-y="'+fmt(p.y)+'"><circle cx="'+sx(p.x)+'" cy="'+sy(p.y)+'" r="4.6" fill="'+p.color+'" stroke="#fff" stroke-width="1.5"/><text x="'+(sx(p.x)+(p.dx ?? 7))+'" y="'+(sy(p.y)+(p.dy ?? -7))+'" class="label" fill="'+p.color+'">'+esc(pointLabel(p))+'</text></g>');
