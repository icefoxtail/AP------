...
const V = (x,y,label,color) => ({x,y,label,color:color || '#dc2626'});
...
const S = (x1,y1,x2,y2,label,color,dash) => ({x1,y1,x2,y2,label,color:color || '#7c3aed',dash:dash || ''});
...
add('22_매산고_1학기_기말_고1_기출',2,[-6,5,-4,8],['원래 직선 x−y−3=0','이동벡터 (−2,3)','이동 후 직선 x−y+2=0'],{points:[V(0,3,'원래'),V(-2,6,'이동 후','#16a34a'),V(0,2,'교점')],lines:[L(1,-1,-3,'원래 l'),L(1,-1,2,'이동 l′','#16a34a')],arrows:[A(0,3,-2,6,'(−2,3)')]});
...
add('24_금당고_2학기_중간_고1_기출',9,[-3,5,-6,5],['원래 y=2x−1','이동벡터 (1,−2)','이동 후 y=2x−5, P=(3,1)'],{points:[V(0,-1,'원래'),V(1,-3,'이동 점','#16a34a'),V(3,1,'P')],lines:[L(2,-1,1,'원래'),L(2,-1,5,'이동 후','#16a34a')],arrows:[A(0,-1,1,-3,'(1,−2)')]});
...
add('23_복성고_1학기_기말_고1_기출',15,[-1,5,-2,4],['a=2, 직사각형 4×2','대각선 접는 선','겹친 삼각형 높이 2, 밑변 d=5/2','겹친 넓이 5/2'],{points:[V(0,0,'O'),V(4,0,'U'),V(4,2,'V'),V(1.5,0,'W','#7c3aed')],polygons:[G([[0,0],[4,0],[4,2]],'겹친 영역','#7c3aed','#ede9fe')],lines:[L(-1,2,0,'대각선 접는 선','#dc2626','7 5')],segments:[S(1.5,0,4,0,'d=5/2','#7c3aed'),S(4,0,4,2,'h=2','#16a34a')],arrows:[A(0,2,1.6,-1.2,'대칭 접기')]});
...
add('24_매산고_1학기_기말_고1_기출',10,[-2,5,-3,8],['원래 y=−x²+3x−4','x축 대칭 y=x²−3x+4','대칭축 y=x','a=3에서 T=(2,2) 접함'],{points:[V(2,2,'T')],lines:[L(1,-1,0,'y=x','#dc2626','7 5')],curves:[{formula:'parabola-24-original',start:-1,end:4,label:'원래',color:'#2563eb'},{formula:'parabola-24-reflected',start:-1,end:4,label:'대칭 후',color:'#16a34a'}]});
...
add('21_효천고_2학기_중간_고1_기출',19,[-3,5,-1,5],['a=0: l:y=2, l′:x=3, 교점 (3,2)','a≠0 형식 해 a=1','a=1에서 l=l′=y=x+2','고유 교점 없음'],{points:[V(3,2,'a=0 교점')],lines:[L(1,-1,-2,'a=1: l=l′'),L(0,1,-2,'a=0: l','#16a34a'),L(1,0,-3,'a=0: l′','#7c3aed')]});
...
add('22_제일고_2학기_중간_고1_기출',3,[-5,3,-6,3],['P=(−3,1), P′=(−3/5,−19/5)','대칭축 x−2y−1=0','PP′⊥대칭축'],{points:[V(-3,1,'P'),V(-3/5,-19/5,'P′','#16a34a'),V(-9/5,-9/5,'중점','#7c3aed')],lines:[L(1,-2,-1,'대칭축','#dc2626')],segments:[S(-3,1,-3/5,-19/5,'PP′')]});
...
add('22_제일고_2학기_중간_고1_기출',17,[-3,4,-3,4],['좌표변환 (u,v)=(y,x+1)','결과 중심 원점의 왼쪽 반원','수직 선분 x=0, −1≤y≤1'],{points:[V(0,1,'끝점'),V(0,-1,'끝점','#16a34a')],lines:[L(1,0,0,'x=0','#dc2626'),L(1,-1,0,'y=x','#64748b','7 5')],segments:[S(0,-1,0,1,'세로 선분','#16a34a')]});
...
add('22_효천고_2학기_중간_고1_기출',17,[-4,4,-4,4],['고정점 6개를 지나는 축','추가 교환축 y=x, y=−x','총 8개의 대칭축'],{points:[V(1,0,'A'),V(-1,0,'B'),V(0,1,'C'),V(0,-1,'D')],lines:[L(0,1,0,'x축'),L(1,0,0,'y축'),L(1,-1,0,'y=x','#dc2626','7 5'),L(1,1,0,'y=−x','#dc2626','7 5')]});
...
add('23_매산여고_2학기_중간_고1_기출',12,[-3,5,-2,7],['l₁·l₂는 서로 독립적인 평행이동','두 경계선 y=2x+3/4','포물선과 T=(1/2,7/4)에서 접'],{points:[V(.5,1.75,'T')],curves:[{formula:'parabola-maesan',start:-2,end:3,label:'포물선',color:'#2563eb'}],lines:[L(2,-1,-.75,'이동 직선','#16a34a')]});
...
add('23_매산여고_2학기_중간_고1_기출',21,[-2,6,-5,6],['원래 중심 (4,−3), r=2','평행이동 (−2,3) → (2,0)','y=x 대칭 → (0,2)','최종 원을 y=2x+2가 이등분'],{points:[V(4,-3,'O'),V(2,0,'O₁','#7c3aed'),V(0,2,'O₂','#16a34a')],circles:[C(4,-3,2,'원래'),C(2,0,2,'중간','#7c3aed','#ede9fe','6 4'),C(0,2,2,'최종','#16a34a','#dcfce7')],lines:[L(1,-1,0,'y=x','#dc2626','7 5'),L(2,-1,-2,'y=2x+2','#dc2626')],arrows:[A(4,-3,2,0,'(−2,3)'),A(2,0,0,2,'y=x 대칭','#16a34a')]});
add('23_팔마고_2학기_중간_고1_기출',7,[-3,5,-3,11],['원래 y=2x−1','이동벡터 (2,8)','이동 후 y=2x+3','포물선과 (0,3)에서 접'],{points:[V(0,-1,'원래'),V(2,7,'이동 점','#16a34a'),V(0,3,'접점')],lines:[L(2,-1,1,'원래'),L(2,-1,-3,'이동 후','#16a34a')],curves:[{formula:'parabola-palmar',start:-2,end:4,label:'포물선',color:'#7c3aed'}],arrows:[A(0,-1,2,7,'(2,8)')]});
add('23_팔마고_2학기_중간_고1_기출',13,[-5,4,-4,5],['A=(0,4), B=(2,0)','대칭축 y=−2x−1','A′=(−4,2)','축과 A′B의 교점 H=(−1,1)'],{points:[V(0,4,'A'),V(2,0,'B'),V(-4,2,'A′','#16a34a'),V(-1,1,'H')],lines:[L(2,1,1,'대칭축','#dc2626'),L(1,3,-6,'A′B')]});
...
add('25_금당고_2학기_중간_고1_기출',15,[-2,8,-5,8],['x축 대칭 후 원 중심 (3,2), r=2√2','점 (0,2)에서 그은 접선','최대 기울기 m=2√2'],{points:[V(3,2,'O'),V(0,2,'S')],circles:[C(3,2,2*Math.sqrt(2),'C')],lines:[L(0,1,-2,'y=2','#64748b','7 5')],segments:[S(0,2,3,2,'OS')]});
add('25_금당고_2학기_중간_고1_기출',20,[-5,5,-5,5],['C 중심 (−2,−1), C′ 중심 (2,1)','원점 대칭·반지름 1 보존','내공통접선 y=mx','최대 기울기 m=4/3'],{points:[V(-2,-1,'O'),V(2,1,'O′','#16a34a')],circles:[C(-2,-1,1,'C'),C(2,1,1,'C′','#16a34a','#dcfce7')],lines:[L(1,-4/3,0,'최대 내공통접선','#7c3aed'),L(1,0,0,'원점대칭','#dc2626','7 5')]});
add('25_금당고_2학기_중간_고1_기출',22,[-2,8,-2,9],['C:y=x²−2x','C′:y=−(x−4)²+6','A=(3,3)에서 중근','k=−1,3,7 교점 합집합'],{points:[V(3,3,'A'),V(1,-1,'C 꼭짓점'),V(4,6,'C′ 꼭짓점','#16a34a')],curves:[{formula:'parabola-25-c',start:-1,end:4,label:'C',color:'#2563eb'},{formula:'parabola-25-cprime',start:1,end:7,label:'C′',color:'#16a34a'}],lines:[L(0,1,-3,'y=3','#dc2626','6 4'),L(0,1,1,'y=−1','#7c3aed','6 4'),L(0,1,-7,'y=7','#7c3aed','6 4')]});
...
add('25_효천고_2학기_중간_고1_기출',13,[-7,7,-7,7],['(2,2)→(−5,0), (5,7)→(−2,−5)','전체 변환: x=u−7, y=2−v','x축 반사와 (−7,2) 이동'],{points:[V(2,2,'A'),V(5,7,'B'),V(-5,0,'A′','#16a34a'),V(-2,-5,'B′','#16a34a')],lines:[L(0,1,0,'x축','#dc2626','7 5')],arrows:[A(2,2,-5,0,'(−7,2)'),A(5,7,-2,-5,'대응','#16a34a')]});
...
add('25_효천고_2학기_중간_고1_기출',23,[-3,11,-6,8],['원 중심 O=(4,2), r=√2','그래프 y=|x−a|+b가 O를 지남','다른 branch는 원 밖 또는 접함','중심 branch의 현은 지름'],{points:[V(4,2,'O'),V(3,1,'V₁','#7c3aed'),V(2,0,'V₂','#7c3aed'),V(5,1,'V₃','#7c3aed'),V(6,0,'V₄','#7c3aed')],circles:[C(4,2,Math.sqrt(2),'원')],lines:[L(1,-1,-2,'중심 branch','#16a34a'),L(1,1,-6,'다른 branch','#7c3aed','6 4')],segments:[S(3,1,4,2,'지름 방향','#dc2626')]});
...
add('25_제일고_2학기_중간_고1_기출',18,[-2,18,-3,19],['정사각형 OABC 한 변 16','O→O′, F→F′는 접는 선의 수직이등분선','PQ:y=−x/2+10','P=(0,10), Q=(16,2)'],{points:[V(0,0,'O'),V(0,16,'A'),V(16,16,'B'),V(16,0,'C'),V(0,10,'P'),V(16,2,'Q'),V(0,5,'F','#7c3aed'),V(16,12,'F′','#16a34a'),V(16,16,'O′','#16a34a')],polygons:[G([[0,0],[0,16],[16,16],[16,0]],'OABC')],lines:[L(1,2,-20,'접는 선 PQ','#dc2626')],segments:[S(0,0,16,16,'O→O′','#16a34a','5 4'),S(0,5,16,12,'F→F′','#7c3aed','5 4')]});
...
function fmt(value) { if (Math.abs(value)<1e-9) value=0; return Math.abs(value-Math.round(value))<1e-9 ? String(Math.round(value)) : value.toFixed(4).replace(/0+$/,'').replace(/\.$/,'').replace('-','−'); }
...
function curvePoints(formula,start,end,fx,fy) {
  ...
  for(let i=0;i<=240;i++) {
    ...
    else if(formula==='parabola-25-cprime') y=-(x-4)*(x-4)+6;
...
function render(spec) {
  ...
  for(const cu of spec.curves || []) body.push('<polyline data-geometry="curve" data-formula="'+esc(cu.formula)+'" points="'+curvePoints(cu.formula,cu.start,cu.end,sx,sy)+'" fill="none" stroke="'+cu.color+'" stroke-width="2.35" stroke-linecap="round"/>');
  ...
  for(const l of spec.lines || []) { const ps=lineBox(l.a,l.b,l.c,b),p1=ps[0],p2=ps[1];body.push('<line data-geometry="line" data-equation="'+esc(fmt(l.a)+'x+'+fmt(l.b)+'y+'+fmt(l.c)+'=0')+'" x1="'+sx(p1[0]).toFixed(2)+'" y1="'+sy(p1[1]).toFixed(2)+'" x2="'+sx(p2[0]).toFixed(2)+'" y2="'+sy(p2[1]).toFixed(2)+'" stroke="'+l.color+'" stroke-width="2.25" '+(l.dash?'stroke-dasharray="'+l.dash+'"':'')+'/><text x="'+(Math.min(sx(p1[0]),sx(p2[0]))+8)+'" y="'+Math.max(72,Math.min(sy(p1[1]),sy(p2[1]))-7)+'" class="label" fill="'+l.color+'">'+esc(l.label)+'</text>'); }
  for(const s of spec.segments || []) body.push('<line data-geometry="segment" x1="'+sx(s.x1)+'" y1="'+sy(s.y1)+'" x2="'+sx(s.x2)+'" y2="'+sy(s.y2)+'" stroke="'+s.color+'" stroke-width="2.15" '+(s.dash?'stroke-dasharray="'+s.dash+'"':'')+'/><text x="'+((sx(s.x1)+sx(s.x2))/2+5)+'" y="'+((sy(s.y1)+sy(s.y2))/2-6)+'" class="label" fill="'+s.color+'">'+esc(s.label)+'</text>');
  ...
  for(const p of spec.points || []) body.push('<g data-point-label="'+esc(p.label)+'" data-point-x="'+fmt(p.x)+'" data-point-y="'+fmt(p.y)+'"><circle cx="'+sx(p.x)+'" cy="'+sy(p.y)+'" r="4.6" fill="'+p.color+'" stroke="#fff" stroke-width="1.5"/><text x="'+(sx(p.x)+7)+'" y="'+(sy(p.y)-7)+'" class="label" fill="'+p.color+'">'+esc(p.label+' ('+fmt(p.x)+','+fmt(p.y)+')')+'</text></g>');
...
console.log(JSON.stringify({status:'PASS',updatedJsFiles:fileTexts.size,visualRows:rows.length,missingSpecs:[...specs.keys()].filter(key=>{const s=specs.get(key);return !fs.existsSync(path.join(assetsRoot,s.folder,'q'+String(s.id).padStart(2,'0')+'-solution.svg'));}).length},null,2));
