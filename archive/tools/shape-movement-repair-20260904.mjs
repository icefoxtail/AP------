...
add('23_팔마고_2학기_중간_고1_기출',7,[-3,5,-6,11],['원래 y=2x−1','이동벡터 (2,8)','이동 후 y=2x+3','포물선과 (0,3)에서 접'],{points:[V(0,-1,'원래'),V(2,7,'이동 점','#16a34a'),V(0,3,'접점')],lines:[L(2,-1,1,'원래'),L(2,-1,-3,'이동 후','#16a34a')],curves:[{formula:'parabola-palmar',start:-2,end:4,label:'포물선',color:'#7c3aed'}],arrows:[A(0,-1,2,7,'(2,8)')]});
...
function render(spec) {
  ...
  for(const p of spec.points || []) body.push('<g data-point-label="'+esc(p.label)+'" data-point-x="'+fmt(p.x)+'" data-point-y="'+fmt(p.y)+'"><circle cx="'+sx(p.x)+'" cy="'+sy(p.y)+'" r="4.6" fill="'+p.color+'" stroke="#fff" stroke-width="1.5"/><text x="'+(sx(p.x)+7)+'" y="'+(sy(p.y)-7)+'" class="label" fill="'+p.color+'">'+esc(p.label+' ('+fmtLabel(p.x)+','+fmtLabel(p.y)+')')+'</text></g>');
