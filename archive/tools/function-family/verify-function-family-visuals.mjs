...
function checkSvg(row) {
  ...
  const actualCurveSampleCounts = [...raw.matchAll(/<polyline\b[^>]*\bpoints="([^"]+)"/gi)].map((match) => sampleCountFromPolyline(match[1]));
  const expectedCurveSampleCounts = Array.isArray(row.curveSampleCounts) ? row.curveSampleCounts : [];
  if (actualCurveSampleCounts.length !== expectedCurveSampleCounts.length) errors.push('curve-polyline-count-mismatch');
  if (actualCurveSampleCounts.some((count, index) => count !== expectedCurveSampleCounts[index])) errors.push('curve-polyline-sample-count-mismatch');
  ...
  return { caseId: row.caseId, assetRef: row.assetRef, bytes: Buffer.byteLength(raw), assetSha256: sha256(raw), curveSampleCounts: actualCurveSampleCounts, expectedCurveSampleCounts, minimumCurveSampleCount: minimum, densityFailures, status: errors.length ? 'FAIL' : 'PASS', errors };
}
