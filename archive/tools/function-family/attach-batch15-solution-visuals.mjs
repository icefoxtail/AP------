...
const PATCHES = [
  ['original/high/h1/2final/25_제일고_2학기_기말_고1_기출.js', 18, 'assets/images/25_제일고_2학기_기말_고1_기출/q18-solution.svg', '합성함수의 조각별 그래프와 좌표축 넓이를 나타낸 해설 그래프', '구간별 식 2−x, 2x−1, −4x+8과 넓이 11/4를 확인한다.'],
...
function main() {
  ...
  const report = { reportType: 'FUNCTION_FAMILY_BATCH15_SOLUTION_VISUAL_ATTACHMENT', generatedAt: new Date().toISOString(), status: 'PASS', changedQuestionCount: ledger.length, protectedPayloadParity: ledger.every((row) => row.beforeProtectedHash === row.afterProtectedHash), ledger };
  fs.writeFileSync(path.join(REPORT_DIR, 'function_family_batch15_attachment_ledger.json'), JSON.stringify(report, null, 2) + '\n', 'utf8');
