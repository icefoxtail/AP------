...
window.questionBank = [
  ...
  q(13, "집합", "객관식", "두 집합 $A=\\{(x,y)\\mid y=\\dfrac{|x|-1}{|x-1|},\\;$ 단 $x\\ne1\\}$, $B=\\{(x,y)\\mid y=mx\\}$에 대하여 $A\\cap B=\\varnothing$일 때, 실수 $m$의 값의 범위는? [4.4점]", ["$-1\\le m\\lt-3+2\\sqrt2$", "$-1\\lt m\\le-3+2\\sqrt2$", "$-2\\le m\\lt-3+2\\sqrt2$", "$-2\\lt m\\le-3+2\\sqrt2$", "$-2\\le m\\le-3+2\\sqrt2$"], {
    level: "상", category: "두 그래프의 교점", tags: ["집합", "절댓값함수", "기울기", "교점"], answer: "①",
    solution: "[키포인트] 원점을 지나는 직선 $y=mx$가 집합 $A$의 그래프와 만날 때 가능한 기울기의 전체 범위를 구한 뒤 그 여집합을 찾는다.\n조건 정리: $0\\lt x\\lt1$에서는 $y=-1$, $x\\gt1$에서는 $y=1$이다. $x\\lt0$에서는 $u=-x\\gt0$으로 놓으면 $m=\\dfrac{y}{x}=-\\dfrac{u-1}{u(u+1)}$이다.\n풀이 방향: 세 구간에서 교점이 생기는 $m$의 범위를 각각 구해 합친다.\n정석 풀이: $0\\lt x\\lt1$에서는 $m=-1/x$이므로 $m\\lt-1$이다. $x\\gt1$에서는 $m=1/x$이므로 $0\\lt m\\lt1$이다. $x\\lt0$에서 $0\\lt u\\lt1$이면 $m=\\dfrac{1-u}{u(u+1)}\\gt0$이고, $u$가 $0$에서 $1$까지 변할 때 이 값은 모든 양의 값을 갖는다. $u\\ge1$이면 $m\\le0$이고, $\\dfrac{u-1}{u(u+1)}\\le3-2\\sqrt2$이다. 실제로 $(3-2\\sqrt2)-\\dfrac{u-1}{u(u+1)}=\\dfrac{(3-2\\sqrt2)(u-1-\\sqrt2)^2}{u(u+1)}\\ge0$이므로 $-3+2\\sqrt2\\le m\\le0$이다. 따라서 교점이 생기는 기울기는 $m\\lt-1$ 또는 $m\\ge-3+2\\sqrt2$이고, 교점이 없는 범위는 $-1\\le m\\lt-3+2\\sqrt2$이다.\n따라서 정답은 ①이다."
  }),
  ...
  q(18, "집합", "단답형", "단답형 1. 두 집합 $A=\\{(x,y)\\mid y=\\dfrac{x^2+n}{6},\\;x\\ge0\\}$, $B=\\{(x,y)\\mid y=\\sqrt{6x-n}\\}$에 대하여 $n(A\\cap B)=1$이 되도록 하는 양의 정수 $n$의 값은? [4점]", [], {
    ...
    solution: "[키포인트] 두 그래프의 교점 조건을 서로 대칭인 두 식으로 바꾼다.\n조건 정리: 교점에서는 $x^2+n=6y$, $y^2+n=6x$이고 $x,y\\ge0$이다.\n풀이 방향: 두 식의 차를 이용해 $x=y$를 얻고 하나의 이차방정식의 근의 개수를 조사한다.\n정석 풀이: 두 식을 빼면 $(x-y)(x+y+6)=0$이다. $x,y\\ge0$이므로 $x=y$이고 $x^2-6x+n=0$이다. 양의 정수 $n$에 대하여 $1\\le n\\lt9$이면 서로 다른 두 양의 근을 가지고, $n\\gt9$이면 실근이 없다. $n=9$일 때만 $(x-3)^2=0$이 되어 교점이 하나이다.\n따라서 구하는 값은 $9$이다."
  }),
  ...
  q(21, "유리함수", "서술형", "서술형 2. 함수 $f(x)=\\dfrac{ax+1}{bx+c}$는 두 직선 $y=x+\\dfrac12$, $y=-x+\\dfrac32$에 대하여 대칭이고 점 $(1,3)$을 지난다. 함수 $f(x)$와 접점의 $x$좌표가 양수인 서로 다른 두 점에서 접하는 원이 있다. $2\\sqrt2$보다 큰 양수 $k$에 대하여 점 $(\\dfrac12,1)$과 중심 사이의 거리가 $k$인 두 원의 넓이 중 큰 것부터 $g(k)$, $h(k)$라 할 때, $g(3)-h(3)$의 값을 구하고 그 과정을 서술하시오. (단, $a,b,c$는 상수이다.) [10점]", [], {
    level: "상", category: "유리함수와 원의 접선", tags: ["유리함수", "대칭", "원", "접선"], answer: "$4\\pi$",
