...
window.questionBank = [
  ...
  },
  {
    ...
    "choices": [],
    "answer": "$72$",
    "solution": "[키포인트] 공통 영역, 대칭차집합, 바깥 영역을 네 칸으로 나누어 합 조건을 세운다.\n$A\\cap B=\\{c\\}$, $S=(A-B)\\cup(B-A)$라 하면 $|S|=2$ 또는 3이다. 전체 합이 55이고 바깥 영역 합이 $4\\sum S$이므로 $c+\\sum S+4\\sum S=55$, 즉 $c+5\\sum S=55$이다.\n따라서 c=5 또는 10이다. $|S|=2$인 조합은 8개, $|S|=3$인 조합은 5개이고, 각 S의 원소를 A만 또는 B만에 배치하는 방법은 각각 $2^2$, $2^3$이다.\n따라서 순서쌍은 $8\\cdot4+5\\cdot8=72$개이다.\n[시각자료 읽기] A만과 B만을 합친 것이 S이므로 각 대칭차 원소마다 두 배치 방향이 생긴다.",
