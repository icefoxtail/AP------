...
window.questionBank = [
  ...
  },
  {
    ...
    "answer": "③",
    "solution": "$(A+2B)-(B+C)=A+B-C=(x^{2}-xy+2y^{2})+(x^{2}+xy+y^{2})-(x^{2}-y^{2})=x^{2}+4y^{2}$이므로 $xy$의 계수는 $0$이다. 따라서 정답은 ③이다.",
  ...
  },
  {
    ...
    "wide": false,
    "content": "다음은 다항식 $3x^{2}-5x+6$을 $x-1$로 나누는 과정을 나타낸 것이다.<br>$$\\begin{array}{r|l} & ax-2\\\\ \\hline x-1 & 3x^{2}-5x+6\\\\ & \\underline{3x^{2}-3x}\\\\ & -2x+6\\\\ & \\underline{-2x+2}\\\\ & b\\end{array}$$<br>이때 상수 $a,b$에 대하여 $a+b$의 값은? [4.3점]",
    "choices": [
      "3",
      "4",
      "5",
  ...
  },
  {
    ...
    "wide": false,
    "content": "[상] 소수 $p$와 50이하 자연수 $x, y$에 대하여 행렬 상등 $x-1=p, y=11$ 일 때, $p$의 최댓값은? [4.8점]",
    "choices": [
      "41",
      "43",
      "47",
    ...
    "answer": "④",
    "solution": "문제에 제시된 행렬 상등의 실제 두 행렬이 누락되어 있어, $x-1=p$와 $y=11$이 어떤 성분 비교에서 나온 조건인지 확인할 수 없다. 누락된 행렬을 임의로 복원하지 않으면 현재 정보만으로는 $p$의 최댓값을 결정할 수 없다.",
    "subUnitKey": "H22-C-09-CORE",
    "subUnit": "경우의 수 핵심 개념",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    ...
    "wide": false,
    "content": "모든 실수 $x$에 대하여 최고차항의 계수가 1인 두 이차다항식 $P(x),Q(x)$가 다음 조건을 만족시킨다.<div class='box'>(가) $P(x)-Q(x)=2$<br>(나) $10-Q(x)$는 $x+1$로 나누어떨어진다.<br>(다) 다항식 $P(x)Q(x)=0$의 근은 연속하는 네 자연수이다.</div>다항식 $P(x)Q(x)+k$가 실수 $a,b$에 대하여 이차식 $x^{2}+ax+b$의 제곱으로 인수분해될 때, 상수 $k+a+b$의 값은? [4.9점]",
    "choices": [
      "1",
      "2",
      "3",
    ...
    "answer": "①",
    "solution": "$P(x)-Q(x)=2$이므로 두 monic quadratic의 $x$ 계수는 같다. $P(x)Q(x)=0$의 네 근이 연속하는 네 자연수 $n,n+1,n+2,n+3$이므로 같은 근의 합을 갖는 쌍은 끝의 두 수와 가운데 두 수이다. 또한 $P-Q=2$이므로 $P(x)=(x-n-1)(x-n-2)$, $Q(x)=(x-n)(x-n-3)$이다. $10-Q(x)$가 $x+1$로 나누어떨어지므로 $Q(-1)=10$, 즉 $(n+1)(n+4)=10$이다. $n$은 자연수이므로 $n=1$. 따라서 $P(x)=x^{2}-5x+6$, $Q(x)=x^{2}-5x+4$이다. $t=x^{2}-5x$라 하면 $P(x)Q(x)+k=(t+6)(t+4)+k=(t+5)^{2}+k-1$이다. 이 식이 $(x^{2}+ax+b)^{2}$가 되려면 $k=1$, $a=-5$, $b=5$이므로 $k+a+b=1$이다. 따라서 정답은 ①이다.",
  ...
  },
  {
    ...
    "wide": false,
    "content": "1. $x$에 대한 이차방정식<br>$x^2-2(k-1)x+k^2-ak+b-3=0$<br>이 실수 $k$의 값에 관계없이 항상 중근을 가질 때, 상수 $a,b$에 대하여 다항식 $x^3-4x^2+ax+b$를 복소수 범위에서 인수분해하고 그 풀이 과정을 서술하시오. [6.0]",
    "choices": [],
    "answer": "$(x-2)(x-1-\\sqrt{3})(x-1+\\sqrt{3})$",
    "solution": "[키포인트] 모든 실수 $k$에 대하여 중근을 가지므로 판별식이 $k$에 대한 항등식으로 $0$이 되어야 한다.\n\n조건 정리:\n주어진 이차방정식은\n$x^2-2(k-1)x+k^2-ak+b-3=0$이다.\n\n정석 풀이:\n중근을 가지려면 판별식 $D=0$이어야 한다.\n짝수 판별식을 사용하면\n$\\dfrac{D}{4}=(k-1)^2-(k^2-ak+b-3)$\n$=k^2-2k+1-k^2+ak-b+3$\n$=(a-2)k+4-b$이다.\n\n이 식이 모든 실수 $k$에 대하여 항상 $0$이므로\n$a-2=0$, $4-b=0$이다.\n따라서\n$a=2$, $b=4$이다.\n\n이 값을 다항식에 대입하면\n$x^3-4x^2+ax+b$\n$=x^3-4x^2+2x+4$이다.\n\n$x=2$를 대입하면\n$2^3-4\\cdot2^2+2\\cdot2+4$\n$=8-16+4+4=0$이므로\n$x-2$가 인수이다.\n\n따라서\n$x^3-4x^2+2x+4$\n$=(x-2)(x^2-2x-2)$이다.\n\n이제\n$x^2-2x-2=0$의 두 근은\n$x=\\dfrac{2\\pm\\sqrt{4+8}}{2}$\n$=1\\pm\\sqrt{3}$이다.\n\n그러므로 복소수 범위에서\n$x^3-4x^2+ax+b$\n$=(x-2)(x-1-\\sqrt{3})(x-1+\\sqrt{3})$이다.\n\n결론:\n따라서 구하는 인수분해는\n$(x-2)(x-1-\\sqrt{3})(x-1+\\sqrt{3})$이다.",
    "subUnitKey": "H22-C-05-QUADRATIC_EQUATION",
    "subUnit": "이차방정식",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    ...
    "wide": false,
    "content": "[서술형 2] 다음 그림과 같이 반지름의 길이가 $10$인 원 $O$의 지름 $AB$와 현 $CD$가 만나는 점 $P$에 대하여 $CP=9$, $DP=6$이다. 선분 $AP$와 선분 $BP$의 길이를 두 근으로 하고, $x^2$의 계수가 $1$인 이차방정식을 구하고, 그 풀이 과정을 서술하시오. [6.0점]",
