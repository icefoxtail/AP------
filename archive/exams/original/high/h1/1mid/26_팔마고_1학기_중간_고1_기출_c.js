window.examTitle = "26_팔마고_1학기_중간_고1_기출";

window.questionBank = [
  {
    "id": 1,
    "level": "하",
    "category": "다항식의 연산",
    "originalCategory": "다항식의 연산",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-01",
    "standardUnit": "다항식의 연산",
    "standardUnitOrder": 1,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "다항식",
      "조건해석",
      "계산",
      "하난도"
    ],
    "wide": false,
    "content": "$(2x^2+3x+2)-(x^2+4x-3)=ax^2+bx+c$를 만족할 때, $a+b+c$의 값은? (단, $a, b, c$는 실수)",
    "choices": [
      "1",
      "2",
      "3",
      "4",
      "5"
    ],
    "answer": "④",
    "solution": "[키포인트] 뺄셈을 전개한 뒤 같은 차수의 계수를 비교한다.\n\n$(2x^2+3x+2)-(x^2+4x-3)=x^2-x+5$이다. 따라서 $a=1$, $b=-1$, $c=5$이므로\n$a+b+c=1-1+5=5$이다.\n\n따라서 정답은 ④이다.",
    "subUnitKey": "H22-C-01-POLYNOMIAL_BASIC",
    "subUnit": "다항식의 연산",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 2,
    "level": "중",
    "category": "복소수와 이차방정식",
    "originalCategory": "복소수와 이차방정식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-04",
    "standardUnit": "복소수와 이차방정식",
    "standardUnitOrder": 4,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "제곱근",
      "근호",
      "복소수",
      "이차방정식",
      "계산"
    ],
    "wide": false,
    "content": "$(\\sqrt{-7} \\times \\sqrt{7}) - \\left(\\dfrac{\\sqrt{128}}{\\sqrt{-8}}\\right) + (\\sqrt{-3} \\times \\sqrt{-3}) \\times i$의 값은?\n(단, $i=\\sqrt{-1}$)",
    "choices": [
      "$-12i$",
      "$-6i$",
      "$0$",
      "$8i$",
      "$20i$"
    ],
    "answer": "④",
    "solution": "$\\sqrt{-7}\\sqrt{7}=7i$, $\\sqrt{128}=8\\sqrt{2}$, $\\sqrt{-8}=2\\sqrt{2}i$이므로 $\\dfrac{\\sqrt{128}}{\\sqrt{-8}}=\\dfrac{4}{i}=-4i$이다. 또 $\\sqrt{-3}\\sqrt{-3}=(\\sqrt{3}i)^2=-3$이다. 따라서 주어진 식의 값은 $7i-(-4i)-3i=8i$이므로 정답은 ④이다.",
    "subUnitKey": "H22-C-04-COMPLEX_ROOT",
    "subUnit": "복소수와 이차방정식",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 3,
    "level": "하",
    "category": "복소수와 이차방정식",
    "originalCategory": "복소수와 이차방정식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-04",
    "standardUnit": "복소수와 이차방정식",
    "standardUnitOrder": 4,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "복소수",
      "이차방정식",
      "계산",
      "조건해석"
    ],
    "wide": false,
    "content": "이차방정식 $x^2-3x-6=0$의 두 근을 $\\alpha, \\beta$라 할 때, $(\\alpha-\\beta)^2$의 값은?",
    "choices": [
      "30",
      "31",
      "32",
      "33",
      "34"
    ],
    "answer": "④",
    "solution": "주어진 식을 정리하고 필요한 값을 대입한 뒤 등식 또는 부등식의 기본 성질에 따라 계산한다. 주어진 정답과 일치하는 결과는 ④이다.",
    "subUnitKey": "H22-C-04-COMPLEX_ROOT",
    "subUnit": "복소수와 이차방정식",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 4,
    "level": "중",
    "category": "이차방정식과 이차함수",
    "originalCategory": "이차방정식과 이차함수",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-05",
    "standardUnit": "이차방정식과 이차함수",
    "standardUnitOrder": 5,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "이차방정식",
      "이차함수",
      "최대최소",
      "함수",
      "그래프",
      "정수조건",
      "계산"
    ],
    "wide": false,
    "content": "$x$에 대한 이차함수 $y=x^2-2x+k-6$의 그래프와 $x$축이 만나지 않도록 하는 정수 $k$의 최솟값은?",
    "choices": [
      "6",
      "7",
      "8",
      "9",
      "10"
    ],
    "answer": "③",
    "solution": "[키포인트]\n이차함수가 $x$축과 만나지 않으려면 꼭짓점의 $y$좌표가 양수여야 한다.\n\n풀이 과정\n$y=x^2-2x+k-6=(x-1)^2+k-7$이다. 꼭짓점의 $y$좌표는 $k-7$이다. 그래프가 $x$축과 만나지 않으려면 위로 열린 포물선이 $x$축 위에 있어야 하므로 $k-7>0$, 즉 $k>7$이다. $k$가 정수이므로 최솟값은 $8$이고, 정답은 ③이다.\n\n결론\n따라서 정답은 ③이다.",
    "subUnitKey": "H22-C-05-QUADRATIC_FUNCTION_GRAPH",
    "subUnit": "이차함수의 그래프",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 5,
    "level": "중",
    "category": "이차방정식과 이차함수",
    "originalCategory": "이차방정식과 이차함수",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-05",
    "standardUnit": "이차방정식과 이차함수",
    "standardUnitOrder": 5,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "절댓값",
      "이차방정식",
      "이차함수",
      "함수",
      "그래프",
      "계산"
    ],
    "wide": false,
    "content": "이차함수 $y=2x^2+3x-1$의 그래프와 직선 $y=-2x+k$가 한 점에서 만나도록 하는 $k$에 대해 $\\dfrac{8}{11}|k|$의 값은?",
    "choices": [
      "$-3$",
      "$-2$",
      "$-1$",
      "2",
      "3"
    ],
    "answer": "⑤",
    "solution": "[키포인트]\n두 그래프가 한 점에서 만나는 조건은 교점 방정식의 판별식이 $0$이라는 뜻이다.\n\n풀이 과정\n두 식을 같게 놓으면 $2x^2+3x-1=-2x+k$이므로 $2x^2+5x-(k+1)=0$이다. 판별식은 $D=5^2-4\\cdot2\\cdot(-(k+1))=33+8k$이다. 한 점에서 만나려면 $D=0$이므로 $k=-\\dfrac{33}{8}$이다. 따라서 $\\dfrac{8}{11}|k|=\\dfrac{8}{11}\\cdot\\dfrac{33}{8}=3$이고, 정답은 ⑤이다.\n\n결론\n따라서 정답은 ⑤이다.",
    "subUnitKey": "H22-C-05-FUNCTION_BASIC",
    "subUnit": "함수의 뜻과 그래프",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 6,
    "level": "중",
    "category": "인수분해",
    "originalCategory": "인수분해",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-03",
    "standardUnit": "인수분해",
    "standardUnitOrder": 3,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "다항식",
      "인수분해",
      "계산",
      "조건해석"
    ],
    "wide": false,
    "content": "다항식 $3x^4+5x^3-x^2-5x-2$를 인수분해 하면\n$3 \\times (x+a)(x+b)(x+c)(x+d)$이다.\n이때, $a+b+c+d$의 값은? (단, $a, b, c, d$는 실수이다.)",
    "choices": [
      "$-\\dfrac{5}{3}$",
      "$-\\dfrac{4}{3}$",
      "$\\dfrac{1}{3}$",
      "$\\dfrac{4}{3}$",
      "$\\dfrac{5}{3}$"
    ],
    "answer": "⑤",
    "solution": "[키포인트] 네 인수의 합은 전개했을 때 $x^3$의 계수와 연결되므로 계수비교로 구한다.\n\n$3(x+a)(x+b)(x+c)(x+d)$에서 $x^4$의 계수는 $3$이고 $x^3$의 계수는 $3(a+b+c+d)$이다. 주어진 다항식의 $x^3$의 계수는 $5$이므로\n$3(a+b+c+d)=5$이다.\n따라서 $a+b+c+d=\\dfrac53$이고, 정답은 ⑤이다.",
    "subUnitKey": "H22-C-03-POLYNOMIAL_BASIC",
    "subUnit": "다항식의 연산",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 7,
    "level": "중",
    "category": "복소수와 이차방정식",
    "originalCategory": "복소수와 이차방정식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-04",
    "standardUnit": "복소수와 이차방정식",
    "standardUnitOrder": 4,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "계수비교",
      "복소수",
      "이차방정식",
      "계산"
    ],
    "wide": false,
    "content": "이차방정식 $x^2+5x+1=0$의 두 근을 $\\alpha, \\beta$라 할 때,\n$x^2$의 계수가 1이고 두 수 $\\dfrac{\\beta+1}{\\alpha}, \\dfrac{\\alpha+1}{\\beta}$를 두 근으로 하는 이차방정식을 $x^2+px+q=0$이라 할 때, $p+q$의 값은?\n(단, $p, q$는 실수이다.)",
    "choices": [
      "$-25$",
      "$-24$",
      "$-23$",
      "$-21$",
      "$-20$"
    ],
    "answer": "④",
    "solution": "주어진 식을 정리하고 필요한 값을 대입한 뒤 등식 또는 부등식의 기본 성질에 따라 계산한다. 주어진 정답과 일치하는 결과는 ④이다.",
    "subUnitKey": "H22-C-04-COMPLEX_ROOT",
    "subUnit": "복소수와 이차방정식",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 8,
    "level": "상",
    "category": "항등식과 나머지 정리",
    "originalCategory": "항등식과 나머지 정리",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-02",
    "standardUnit": "항등식과 나머지 정리",
    "standardUnitOrder": 2,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "다항식",
      "항등식",
      "나머지정리",
      "계수비교",
      "조건해석",
      "계산"
    ],
    "wide": false,
    "content": "최고차항의 계수가 양수인 다항식 $P(x)$가 모든 실수 $x$에 대하여\n$9\\{P(x)\\}^2=12P(x)+4x^4-12x^3+9x^2-4$를 만족할 때, $P(1)$의 값은?",
    "choices": [
      "$\\dfrac{1}{2}$",
      "$\\dfrac{1}{3}$",
      "$\\dfrac{1}{4}$",
      "$\\dfrac{1}{5}$",
      "$\\dfrac{1}{6}$"
    ],
    "answer": "②",
    "solution": "[키포인트] 양변을 완전제곱식으로 바꾸고 최고차항의 계수가 양수라는 조건으로 부호를 고른다.\n\n주어진 식을 이항하면\n$9P(x)^2-12P(x)+4=4x^4-12x^3+9x^2=x^2(2x-3)^2$이다. 따라서\n$\\{3P(x)-2\\}^2=\\{x(2x-3)\\}^2$\n이므로 다항식의 곱을 이용해\n$3P(x)-2=x(2x-3)$ 또는 $3P(x)-2=-x(2x-3)$이다.\n두 번째 경우에는 $P(x)$의 최고차항 계수가 $-\\dfrac23$이 되어 조건에 맞지 않는다. 따라서\n$P(x)=\\dfrac{2x^2-3x+2}{3}$이고\n$P(1)=\\dfrac{2-3+2}{3}=\\dfrac13$이다.\n\n따라서 정답은 ②이다.",
    "subUnitKey": "H22-C-02-CORE",
    "subUnit": "항등식과 나머지 정리 핵심 개념",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    "id": 9,
    "level": "상",
    "category": "이차방정식과 이차함수",
    "originalCategory": "이차방정식과 이차함수",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-05",
    "standardUnit": "이차방정식과 이차함수",
    "standardUnitOrder": 5,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "그래프",
      "도형"
    ],
    "wide": false,
    "content": "다음 그림과 같이 직사각형 $ABCD$의 두 꼭짓점 $A, B$는 이차함수 $y=-\\dfrac{1}{3}x^2+3$의 그래프 위에 있고, 두 꼭짓점 $C, D$는 이차함수 $y=x^2-9$의 그래프 위에 있다. 이때 직사각형 $ABCD$의 둘레의 길이에 대한 최댓값은?\n(단, 점 $A$는 제1사분면, 점 $D$는 제4사분면 위의 점이다.)",
    "image": "assets/images/26_팔마고_1학기_중간_고1_기출/q9.png",
    "imageSize": "tall",
    "choices": [
      "$\\dfrac{51}{2}$",
      "$\\dfrac{53}{2}$",
      "$\\dfrac{55}{2}$",
      "$\\dfrac{57}{2}$",
      "$\\dfrac{59}{2}$"
    ],
    "answer": "①",
    "solution": "그림의 좌우 대칭을 이용해 직사각형의 오른쪽 위 x좌표를 $t$라 하자. 위쪽 포물선은 $y=-x^2/3+3$, 아래쪽 포물선은 $y=x^2-9$이므로 직사각형의 너비는 $2t$, 높이는 $(-t^2/3+3)-(t^2-9)=12-4t^2/3$이다. $0<t\\le3$에서 둘레는 $P(t)=2(2t+12-4t^2/3)=-8t^2/3+4t+24$이다. 완전제곱식으로 $P(t)=-8/3(t-3/4)^2+51/2$이므로 최댓값은 $51/2$이다. 선택지 ①이 정답이다.",
    "subUnitKey": "H22-C-05-QUADRATIC_FUNCTION_GRAPH",
    "subUnit": "이차함수의 그래프",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 10,
    "level": "중",
    "category": "여러 가지 방정식과 부등식",
    "originalCategory": "여러 가지 방정식과 부등식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-06",
    "standardUnit": "여러 가지 방정식과 부등식",
    "standardUnitOrder": 6,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "판별식",
      "부등식",
      "계산",
      "조건해석"
    ],
    "wide": false,
    "content": "사차방정식 $(x^2-4x)(x^2-4x+7)+10=0$의 서로 다른 두 실근을 $\\alpha, \\beta$라 할 때, $\\alpha\\beta$의 값은?",
    "choices": [
      "1",
      "2",
      "3",
      "4",
      "5"
    ],
    "answer": "②",
    "solution": "$A=x^2-4x$라 두면 주어진 방정식은 $A(A+7)+10=(A+2)(A+5)=0$이다. 따라서 $x^2-4x+2=0$ 또는 $x^2-4x+5=0$이다.\n후자의 판별식은 $(-4)^2-4\\cdot1\\cdot5=-4\\lt0$이므로 실근이 없다. 전자의 판별식은 $16-8=8\\gt0$이므로 서로 다른 두 실근 $\\alpha,\\beta$는 모두 $x^2-4x+2=0$의 근이다. 근과 계수의 관계에 따라 $\\alpha\\beta=2$이므로 정답은 ②이다.",
    "subUnitKey": "H22-C-06-EQUATION_BASIC",
    "subUnit": "방정식의 풀이",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 11,
    "level": "상",
    "category": "항등식과 나머지 정리",
    "originalCategory": "항등식과 나머지 정리",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-02",
    "standardUnit": "항등식과 나머지 정리",
    "standardUnitOrder": 2,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "다항식",
      "항등식",
      "나머지정리",
      "계산"
    ],
    "wide": false,
    "content": "다항식 $f(x)$를 $x^4+x^3+4x+2$로 나누었을 때, 나머지는 $x^3+2x^2-5x+4$이고, $x^2+2x+1$로 나누었을 때, 몫은 $x^3+3x^2-2x+7$이다. 이때, $f(1)$의 값은?",
    "choices": [
      "41",
      "42",
      "43",
      "44",
      "45"
    ],
    "answer": "②",
    "solution": "[키포인트] 두 나눗셈의 관계식을 세우고, 다항식 나눗셈으로 나머지를 결정한 뒤 $x=1$을 대입한다.\n\n첫 번째 나눗셈의 나눗셈식에서\n$D(x)=x^4+x^3+4x+2,\\quad R(x)=x^3+2x^2-5x+4$\n라 두면\n$f(x)=D(x)L(x)+R(x)$\n이다. 두 번째 나눗셈의 몫을\n$Q(x)=x^3+3x^2-2x+7$\n이라 하고, 나머지를 $ux+v$라 두면\n$f(x)=(x^2+2x+1)Q(x)+ux+v$\n이다.\n\n먼저 곱을 계산하면\n$(x^2+2x+1)Q(x)\n=x^5+5x^4+5x^3+6x^2+12x+7$\n이다. 따라서 이 식에서 첫 번째 나머지를 빼면\n$(x^2+2x+1)Q(x)-R(x)\n=x^5+5x^4+4x^3+4x^2+17x+3$\n이다.\n\n한편\n$D(x)(x+4)\n=(x^4+x^3+4x+2)(x+4)\n=x^5+5x^4+4x^3+4x^2+18x+8$\n이므로\n$(x^2+2x+1)Q(x)-R(x)\n=D(x)(x+4)-x-5$\n이다. 두 표현이 같은 $f(x)$를 나타내려면\n$D(x)L(x)=D(x)(x+4)+(u-1)x+(v-5)$\n가 되어야 한다. 왼쪽은 $D(x)$의 배수이므로 오른쪽의 나머지인 $(u-1)x+(v-5)$가 0이어야 한다. 따라서\n$u=1,\\quad v=5$\n이다.\n\n이제 두 번째 나눗셈식에 $x=1$을 대입하면\n$f(1)=(1+2+1)(1+3-2+7)+1+5\n=4\\cdot9+6=42$\n이다. 따라서 정답은 ②이다.",
    "subUnitKey": "H22-C-02-CORE",
    "subUnit": "항등식과 나머지 정리 핵심 개념",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    "id": 12,
    "level": "상",
    "category": "항등식과 나머지 정리",
    "originalCategory": "항등식과 나머지 정리",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-02",
    "standardUnit": "항등식과 나머지 정리",
    "standardUnitOrder": 2,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "표"
    ],
    "wide": false,
    "content": "두 다항식 $f(x), g(x)$가 다음 조건을 만족시킨다.\n<div class=\"question-table-wrap\">\n<table class=\"question-table\">\n<tr><td>(가) 모든 실수 $x$에 대하여 $f(x)=xg(x)$이다.</td></tr>\n<tr><td>(나) 다항식 $f(x)g(x)$를 $3x+2$로 나눈 나머지는 $-\\dfrac{2}{3}$이다.</td></tr>\n</table>\n</div>\n다항식 $(9x^2+\\dfrac{3}{4}) \\times f(2x-3)$을 $6x-7$로 나눈 나머지의 최솟값은?",
    "choices": [
      "$-\\dfrac{26}{3}$",
      "$-\\dfrac{13}{3}$",
      "$-\\dfrac{7}{3}$",
      "$\\dfrac{13}{3}$",
      "$\\dfrac{26}{3}$"
    ],
    "answer": "①",
    "solution": "[키포인트] $f=xg$ 관계와 나머지정리를 이용해 $g$의 가능한 값을 찾은 뒤, 주어진 일차식의 나머지를 비교한다.\n\n$3x+2=0$에서 $x=-\\dfrac23$이다. $f(x)=xg(x)$이므로 나머지 조건은\n$f(-\\dfrac23)g(-\\dfrac23)=-\\dfrac23 g(-\\dfrac23)^2=-\\dfrac23$\n이고, 따라서 $g(-\\dfrac23)=\\pm1$이다.\n\n$6x-7=0$에서 $x=\\dfrac76$이고 $2x-3=-\\dfrac23$이다. 또\n$9x^2+\\dfrac34=9\\cdot\\dfrac{49}{36}+\\dfrac34=13$\n이므로 구하는 식의 값은\n$13f(-\\dfrac23)=13\\cdot(-\\dfrac23)g(-\\dfrac23)=\\mp\\dfrac{26}{3}$이다.\n가능한 두 값 중 최솟값은 $-\\dfrac{26}{3}$이므로 정답은 ①이다.",
    "subUnitKey": "H22-C-02-CORE",
    "subUnit": "항등식과 나머지 정리 핵심 개념",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    "id": 13,
    "level": "중",
    "category": "여러 가지 방정식과 부등식",
    "originalCategory": "여러 가지 방정식과 부등식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-06",
    "standardUnit": "여러 가지 방정식과 부등식",
    "standardUnitOrder": 6,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "부등식",
      "도형",
      "사각형",
      "넓이",
      "조건해석",
      "계산"
    ],
    "wide": false,
    "content": "길이가 $80\\text{cm}$인 철사를 잘라서 한 변의 길이가 각각 $a\\text{cm}, b\\text{cm}$ ($a>b$)인 두 개의 정사각형을 만들었다. 이 두 정사각형의 넓이의 합이 $218\\text{cm}^2$일 때, $a$의 값은?\n(단, 철사는 모두 사용하고 굵기는 무시한다.)",
    "choices": [
      "11",
      "12",
      "13",
      "14",
      "15"
    ],
    "answer": "③",
    "solution": "두 정사각형에 사용한 철사의 길이를 더하면 $4a+4b=80$이므로 $a+b=20$이다. 넓이의 합이 $a^2+b^2=218$이므로 $(a+b)^2-2ab=218$, 즉 $400-2ab=218$에서 $ab=91$이다.\n따라서 $a,b$는 $t^2-20t+91=(t-13)(t-7)=0$의 두 근이다. $a>b$이므로 $a=13$, $b=7$이고 정답은 ③이다.",
    "subUnitKey": "H22-C-06-EQUATION_BASIC",
    "subUnit": "방정식의 풀이",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 14,
    "level": "중",
    "category": "여러 가지 방정식과 부등식",
    "originalCategory": "여러 가지 방정식과 부등식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-06",
    "standardUnit": "여러 가지 방정식과 부등식",
    "standardUnitOrder": 6,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "판별식",
      "부등식",
      "조건해석",
      "계산"
    ],
    "wide": false,
    "content": "삼차방정식 $x^3=-1$의 한 허근을 $\\omega$라 할 때,\n$\\omega+2\\omega^2+3\\omega^3+4\\omega^4+5\\omega^5+6\\omega^6+7\\omega^7+8\\omega^8+9\\omega^9=a+b\\omega$\n를 만족시키는 두 실수 $a, b$에 대하여 $b-a$의 값은?",
    "choices": [
      "0",
      "5",
      "10",
      "15",
      "20"
    ],
    "answer": "⑤",
    "solution": "$\\omega^3=-1$이고 $\\omega$는 허근이므로 $\\omega^3+1=(\\omega+1)(\\omega^2-\\omega+1)=0$에서 $\\omega^2=\\omega-1$이다. 또한 $\\omega^6=1$이다.\n주어진 아홉 항에서 $\\omega^3=-1$, $\\omega^4=-\\omega$, $\\omega^5=-\\omega^2$, $\\omega^6=1$, $\\omega^7=\\omega$, $\\omega^8=\\omega^2$, $\\omega^9=-1$을 대입하면 합은 $4\\omega+5\\omega^2-6$이다. 이를 정리하면 $4\\omega+5(\\omega-1)-6=9\\omega-11$이므로 $a=-11$, $b=9$이다. 따라서 $b-a=20$이고 정답은 ⑤이다.",
    "subUnitKey": "H22-C-06-EQUATION",
    "subUnit": "여러 가지 방정식",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 15,
    "level": "상",
    "category": "이차방정식과 이차함수",
    "originalCategory": "이차방정식과 이차함수",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-05",
    "standardUnit": "이차방정식과 이차함수",
    "standardUnitOrder": 5,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "표"
    ],
    "wide": false,
    "content": "최고차항의 계수가 양수인 이차함수 $f(x)$가 다음 조건을 만족시킬 때, $f(3)$의 값은?\n<div class=\"question-table-wrap\">\n<table class=\"question-table\">\n<tr><td>(가) 함수 $y=f(x)$의 그래프와 직선 $y=7$이 만나는 두 점의 $x$좌표는 각각 $-3, 1$이다.</td></tr>\n<tr><td>(나) $-2 \\le x \\le 2$에서 이차함수 $f(x)$의 최댓값은 22이다.</td></tr>\n</table>\n</div>",
    "choices": [
      "41",
      "42",
      "43",
      "44",
      "45"
    ],
    "answer": "③",
    "solution": "두 교점의 x좌표가 $-3,1$이므로 최고차항의 계수가 양수인 이차함수는 $f(x)=a(x+3)(x-1)+7$로 쓸 수 있다. 구간 $[-2,2]$에서 위로 열린 포물선의 최댓값은 오른쪽 끝점에서 생기므로 $f(2)=5a+7=22$에서 $a=3$이다. 따라서 $f(3)=3(6)(2)+7=43$이고, 선택지 ③이 정답이다.",
    "subUnitKey": "H22-C-05-FUNCTION_BASIC",
    "subUnit": "함수의 뜻과 그래프",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 16,
    "level": "상",
    "category": "복소수와 이차방정식",
    "originalCategory": "복소수와 이차방정식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-04",
    "standardUnit": "복소수와 이차방정식",
    "standardUnitOrder": 4,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "제곱근",
      "근호",
      "복소수",
      "이차방정식",
      "최대최소",
      "자연수조건",
      "범위"
    ],
    "wide": false,
    "content": "$\\left\\{(\\dfrac{1-i}{\\sqrt{2}})^m + (-i)^n\\right\\}^2 = -2i$를 만족하는 100이하의 자연수 $m, n$에 대하여 $m+n$의 최댓값은? (단, $i=\\sqrt{-1}$)",
    "choices": [
      "196",
      "197",
      "198",
      "199",
      "200"
    ],
    "answer": "④",
    "solution": "$z=\\dfrac{1-i}{\\sqrt{2}}$라 두면 $z^2=-i$, $z^8=1$이다. $(z^m+(-i)^n)^2=-2i$이므로 $z^m+(-i)^n=\\pm(1-i)$이다. 두 항의 절댓값이 각각 $1$이고 $(-i)^n$은 $1,-i,-1,i$ 중 하나이므로 두 항은 순서와 무관하게 $\\{1,-i\\}$ 또는 $\\{-1,i\\}$이다. 따라서 $m$은 짝수이고 $z^m=(-i)^{m/2}$이며, $\\dfrac m2+n\\equiv1\\pmod4$이다. $m+n\\le200$이고, $m=n=100$이면 $\\dfrac m2+n=150$이므로 조건을 만족하지 않는다. 반면 $m=100, n=99$이면 $\\dfrac m2+n=149\\equiv1\\pmod4$이고 실제로 $z^{100}+(-i)^{99}=-1+i$이므로 제곱은 $-2i$이다. 따라서 최댓값은 $100+99=199$, 정답은 ④이다.",
    "subUnitKey": "H22-C-04-COMPLEX_ROOT",
    "subUnit": "복소수와 이차방정식",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 17,
    "level": "상",
    "category": "항등식과 나머지 정리",
    "originalCategory": "항등식과 나머지 정리",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-02",
    "standardUnit": "항등식과 나머지 정리",
    "standardUnitOrder": 2,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "표"
    ],
    "wide": false,
    "content": "계수가 각각 실수인 삼차다항식 $P(x)$, 일차다항식 $Q(x)$가 아래의 조건을 만족할 때, $\\dfrac{P(0) \\times Q(1)}{a}$의 값은?\n\n(가) $P(x) \\times Q(x)$는 $(x^2-6x+10)(x-2)$로 나누어 떨어진다.\n\n(나) 모든 $x$에 대하여 $x^3-4x^2+ax-a-1=P(x)+\\{Q(x)\\}^2$\n\n(다) $Q(0)>0,\\ Q(1)>0,\\ P(0)<0,\\ P(2)=0$",
    "choices": [
      "$-2$",
      "$-1$",
      "1",
      "2",
      "3"
    ],
    "answer": "①",
    "solution": "[키포인트] 나머지정리와 계수비교로 $Q(x)$의 계수를 정한 뒤, 구하는 값을 계산한다.\n\n$F(x)=x^3-4x^2+ax-a-1$, $Q(x)=mx+n$이라 하자. 조건 (나)에서\n$P(x)=F(x)-Q(x)^2$\n이다.\n\n$x^2-6x+10$은 실수 범위에서 인수분해되지 않는 이차식이고 $Q(x)$는 일차식이므로, 조건 (가)에 의해 이 이차식은 $P(x)$의 인수이다. $x^2\\equiv6x-10$, $x^3\\equiv26x-60$을 이용해 $P(x)$를 이 이차식으로 나눈 나머지를 계산하면\n$a+2=6m^2+2mn,\\qquad -a-21=n^2-10m^2$\n이다. 따라서\n$4m^2-2mn-n^2=19$\n을 얻는다.\n\n또 조건 (다)의 $P(2)=0$을 사용한다. $F(2)=a-9$이므로\n$(2m+n)^2=a-9$\n이다. 이 식과 $4m^2-2mn-n^2=19$를 함께 정리하면\n$2m^2+11=19$,\n즉 $m^2=4$\n이다.\n\n$m=2$이면 $n^2+4n+3=0$에서 $n=-1,-3$인데 $n>0$을 만족하지 않는다. $m=-2$이면\n$n^2-4n+3=0$에서 $n=1,3$이고, $m+n>0$을 만족하는 것은 $n=3$이다. 따라서\n$m=-2,\\quad n=3$\n이다.\n\n$a+2=6m^2+2mn$에 대입하면 $a=10$이다. 이제\n$P(0)=F(0)-Q(0)^2=-11-9=-20$,\n$Q(1)=m+n=1$\n이므로\n$\\dfrac{P(0)Q(1)}a=\\dfrac{-20\\cdot1}{10}=-2$\n이다. 따라서 정답은 ①이다.",
    "subUnitKey": "H22-C-02-CORE",
    "subUnit": "항등식과 나머지 정리 핵심 개념",
    "subUnitConfidence": "rule_inferred",
    "subUnitClassificationDepth": "complete_rule"
  },
  {
    "id": 18,
    "level": "상",
    "category": "여러 가지 방정식과 부등식",
    "originalCategory": "여러 가지 방정식과 부등식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-06",
    "standardUnit": "여러 가지 방정식과 부등식",
    "standardUnitOrder": 6,
    "questionType": "객관식",
    "layoutTag": "grid",
    "tags": [
      "객관식",
      "판별식",
      "부등식",
      "계산",
      "조건해석"
    ],
    "wide": false,
    "content": "$x$에 대한 사차방정식 $x^4-(2m+3)x^2+m^2+2=0$이 서로 다른 네 실근 $a, b, c, d$를 갖고, $ad+bc=-11$의 관계가 성립할 때, 양수 $m$의 값은? (단, $a<b<c<d$이다.)",
    "choices": [
      "2",
      "4",
      "6",
      "8",
      "10"
    ],
    "answer": "②",
    "solution": "$t=x^2$로 두면 $t^2-(2m+3)t+m^2+2=0$은 서로 다른 두 양의 근 $p<q$를 갖는다. 원래 사차방정식의 네 실근을 작은 순서대로 쓰면 $a=-\\sqrt q$, $b=-\\sqrt p$, $c=\\sqrt p$, $d=\\sqrt q$이다.\n따라서 $ad+bc=-q-p=-(p+q)=-(2m+3)=-11$이므로 $m=4$이다. 실제로 $m=4$이면 $t^2-11t+18=(t-2)(t-9)=0$이어서 서로 다른 네 실근이 존재한다. 정답은 ②이다.",
    "subUnitKey": "H22-C-06-EQUATION_BASIC",
    "subUnit": "방정식의 풀이",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 19,
    "level": "중",
    "category": "인수분해",
    "originalCategory": "인수분해",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-03",
    "standardUnit": "인수분해",
    "standardUnitOrder": 3,
    "questionType": "서술형",
    "layoutTag": "grid",
    "tags": [
      "서술형"
    ],
    "wide": false,
    "content": "[서답형(서술형) 1]\n실수 $a, b, c$에 대해서 삼각형 $ABC$가 다음을 만족할 때,\n$a^3+a^2b-ab^2-b^3-ac^2-bc^2=0$ 삼각형 $ABC$가 무슨 삼각형인지 정확하게 풀이과정과 함께 쓰시오.",
    "choices": [],
    "answer": "직각삼각형($a$가 빗변)",
    "solution": "[키포인트] 식을 인수분해하여 피타고라스 정리의 형태를 만든다.\n\n$a^3+a^2b-ab^2-b^3$\n$=a^2(a+b)-b^2(a+b)$\n$=(a+b)(a^2-b^2)$\n이다. 마지막 두 항까지 묶으면 전체 식은\n$(a+b)(a^2-b^2)-c^2(a+b)\n=(a+b)(a^2-b^2-c^2)$\n이다.\n\n삼각형의 변의 길이인 $a,b,c$는 양수이므로 $a+b>0$이다. 따라서 주어진 식이 0이 되려면\n$a^2-b^2-c^2=0$,\n즉\n$a^2=b^2+c^2$\n이어야 한다.\n\n이는 변 $a$가 빗변인 직각삼각형의 피타고라스 정리이다. 따라서 삼각형 $ABC$는 $a$를 빗변으로 하는 직각삼각형이다.",
    "subUnitKey": "H22-C-03-FACTORIZATION",
    "subUnit": "인수분해",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 20,
    "level": "하",
    "category": "다항식의 연산",
    "originalCategory": "다항식의 연산",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-01",
    "standardUnit": "다항식의 연산",
    "standardUnitOrder": 1,
    "questionType": "서술형",
    "layoutTag": "grid",
    "tags": [
      "서술형"
    ],
    "wide": false,
    "content": "[서답형(서술형) 2]\n실수 $x, y$에 대하여 $x>y$이고 다음을 만족한다.\n$x+y=-3$이고 $xy=1$이다.\n(1) $x-y$의 값을 구하시오.\n(2) $x^3-y^3$의 값을 구하시오.\n(3) $x^6-y^6$의 값을 구하시오.",
    "choices": [],
    "answer": "(1) $\\sqrt{5}$ (2) $8\\sqrt{5}$ (3) $-144\\sqrt{5}$",
    "solution": "[키포인트] 합과 곱으로 $x-y$, 세제곱의 차, 세제곱의 합을 차례로 계산한다.\n\n$(x-y)^2=(x+y)^2-4xy=(-3)^2-4=5$\n이다. $x>y$이므로\n$x-y=\\sqrt5$\n이다.\n\n또\n$x^2+y^2=(x+y)^2-2xy=9-2=7$\n이므로\n$x^2+xy+y^2=7+1=8$\n이다. 따라서\n$x^3-y^3=(x-y)(x^2+xy+y^2)=8\\sqrt5$\n이다.\n\n한편\n$x^3+y^3=(x+y)^3-3xy(x+y)\n=(-3)^3-3\\cdot1\\cdot(-3)=-18$\n이다. 그러므로\n$x^6-y^6=(x^3-y^3)(x^3+y^3)\n=8\\sqrt5\\cdot(-18)=-144\\sqrt5$\n이다.\n\n따라서 (1)은 $\\sqrt5$, (2)는 $8\\sqrt5$, (3)은 $-144\\sqrt5$이다.",
    "subUnitKey": "H22-C-01-POLYNOMIAL_BASIC",
    "subUnit": "다항식의 연산",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 21,
    "level": "중",
    "category": "여러 가지 방정식과 부등식",
    "originalCategory": "여러 가지 방정식과 부등식",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-06",
    "standardUnit": "여러 가지 방정식과 부등식",
    "standardUnitOrder": 6,
    "questionType": "서술형",
    "layoutTag": "grid",
    "tags": [
      "서술형"
    ],
    "wide": false,
    "content": "[서답형(서술형) 3]\n연립방정식 $\\begin{cases} x-y=3 \\\\ x^2+y^2=17 \\end{cases}$의 해를 $x=\\alpha, y=\\beta$라 할 때, 다음의 물음에 답하시오.\n(1) $\\alpha, \\beta$의 값을 모두 구하시오. (단, 한 쌍씩 정확하게 쓰시오.)\n(2) $\\alpha+\\beta$의 최댓값을 구하시오.",
    "choices": [],
    "answer": "(1) ($4, 1$), ($-1, -4$) (2) 5",
    "solution": "$x-y=3$에서 $x=y+3$이다. 이를 $x^2+y^2=17$에 대입하면 $(y+3)^2+y^2=17$, 즉 $y^2+3y-4=(y+4)(y-1)=0$이다.\n따라서 $y=1$이면 $x=4$이고, $y=-4$이면 $x=-1$이다. (1) 가능한 $(\\alpha,\\beta)$는 $(4,1)$과 $(-1,-4)$이다. (2) 두 경우의 $\\alpha+\\beta$는 각각 $5$, $-5$이므로 최댓값은 $5$이다.",
    "subUnitKey": "H22-C-06-EQUATION_BASIC",
    "subUnit": "방정식의 풀이",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  },
  {
    "id": 22,
    "level": "상",
    "category": "이차방정식과 이차함수",
    "originalCategory": "이차방정식과 이차함수",
    "standardCourse": "공통수학1",
    "standardUnitKey": "H22-C-05",
    "standardUnit": "이차방정식과 이차함수",
    "standardUnitOrder": 5,
    "questionType": "서술형",
    "layoutTag": "grid",
    "tags": [
      "서술형",
      "그래프"
    ],
    "wide": false,
    "content": "[서답형(서술형) 4]\n두 이차함수 $f(x)=x^2+4x+4, g(x)=-(x+1)^2+5$\n$h(x) = \\begin{cases} f(x) (x \\le -3 \\text{ 또는 } x \\ge 0) \\\\ g(x) (-3 < x < 0) \\end{cases}$\n이라 하자. 직선 $y=mx+7$과 $y=h(x)$의 그래프가 서로 다른 세 점에서 만나도록 하는 실수 $m$의 값의 합을 $S$라 할 때, $10S^2$의 값을 구하시오.",
    "choices": [],
    "answer": "120",
    "solution": "직선 $y=mx+7$과 $f(x)=(x+2)^2$의 교점은 $x^2+(4-m)x-3=0$의 근이다. 두 근의 곱이 $-3$이므로 한 근은 항상 $x>0$이고, 다른 음의 근은 $m\\le2$일 때에만 $x\\le-3$으로서 $h$의 정의역에 포함된다. 따라서 $f$ 구간의 교점은 $m\\le2$에서 2개, $m>2$에서 1개이다. 한편 $g(x)=-(x+1)^2+5$ 구간의 교점은 $x^2+(m+2)x+3=0$의 근 중 $-3<x<0$인 것이다. 이 구간에서 접할 때 판별식 $(m+2)^2-12=0$이고 접점은 $x=-\\sqrt3$이므로 $m=-2+2\\sqrt3$이다. 이때 $g$ 구간에서 1개, $-2+2\\sqrt3<m<2$에서 2개, $m\\ge2$에서 1개($m=2$일 때 $x=-3$은 제외)이다. 따라서 전체 교점이 정확히 3개인 경우는 $m=-2+2\\sqrt3$ 또는 $m=2$이다. $S=(-2+2\\sqrt3)+2=2\\sqrt3$이므로 $10S^2=10(2\\sqrt3)^2=120$이다.",
    "subUnitKey": "H22-C-05-FUNCTION_BASIC",
    "subUnit": "함수의 뜻과 그래프",
    "subUnitConfidence": "category_or_cue_inferred",
    "subUnitClassificationDepth": "complete_category"
  }
];
