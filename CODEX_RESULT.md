...
## 11. 장기 계획 문구 시뮬레이션 재검증

- 중1 원내평가, 중2 기출, 중3 기출, 고1~고3 내신 케이스 12개를 상/중/하/만점 밴드로 시뮬레이션해 문장 연결을 확인했다.
- 상세 보호자 메시지가 한 문단으로 뭉개지지 않도록 각 문단을 개별 정화한 뒤 문단 구분을 유지하도록 수정했다.
- `이번 학교 기출시험 결과` 표현은 시험 제목에 `기출`이 있을 때만 사용하고, 일반 중간/기말 제목에서는 `이번 시험 결과`로 낮춰 어색함을 줄였다.
- 만점 케이스에서 `다음 수업에서는... 다음 수업에서는...`처럼 반복되던 난도 확장 문구를 정리했다.
- 첫 문단의 `이번 시험 위치는` 표현을 `동일 평가 기준으로 보면`으로 바꿔 학부모 안내 문장에 가깝게 다듬었다.

## 12. 기출 문항별 문구 시뮬레이션

- 기출 문항별 코멘트를 정답률 높음/중간/낮음, 계산/부호/조건 해석/도형/개념 적용 조합으로 시뮬레이션했다.
- `다항식의 계산의 기본 풀이`처럼 조사 `의`가 겹치는 표현을 `다항식의 계산 기본 풀이`로 정리했다.
- 코드성 trap 텍스트는 문항 코멘트에 노출되지 않는 것을 확인했다.
- 문항별 코멘트 회귀 테스트에 조사 중복 방지 케이스를 추가했다.
