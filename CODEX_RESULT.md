...
- `apmath/worker-backup/worker/index.js`
  - `wrong-clinics/packet` 및 `wrong-clinics/set` 공개 접근은 `GET`에만 허용하도록 수정했다.
  - 기존 라우터가 메서드와 무관하게 `packet/set` 경로 인증을 생략해, 선생님 `DELETE wrong-clinics/packet/...` 요청도 teacher가 `null`로 들어가 `Forbidden`이 발생하던 문제를 해결했다.

- `tests/student-portal-wrong-clinic-hotfix.test.js`
  - 공개 packet/set 조회는 유지하면서 packet 삭제에는 인증을 요구하는 라우터 guard를 추가했다.
