...
- 예외: 수업자료/교재 오답(`material-omr`, `student_material_submissions`, `student_material_wrong_answers`)은 일반 시험 OMR이 아니므로 학생이 제출 후에도 다시 수정할 수 있다.
- 수업자료/교재 오답 수정 가능 정책은 일반 시험 OMR(`exam_sessions`, `wrong_answers`, `check-omr`, `exams`) 제출 완료 수정 금지 원칙을 바꾸지 않는다.
...
| `apmath/js/management.js` | 반/학생 관리, 일부 foundation UI | 반명/학생 상태 흐름 주의 |
...
| `apmath/js/timetable.js` | 시간표/시간표 버전 관리 UI | 운영 시간표와 draft 화면 분리 주의 |
| `apmath/js/study-material-wrong.js` | 수업자료 오답 강사용 UI | 학생 포털/강사용 역할 분리 주의 |
| `apmath/js/textbook.js` | 교재/진도 관리 | 기존 수업일지 흐름 보존 |
| `apmath/js/wangji-foundation.js` | 왕지교육 foundation 공통 계산/보조 UI | branch/time overlap helper 의미 보존 |
...
| `routes/timetable-versions.js` | 시간표 버전 관리 | `/api/timetable-versions/*` | `timetable_versions`, `timetable_version_slots`, `class_time_slots`, `classes`, `student_enrollments` | draft/scheduled/active/cancelled 흐름 보존, 기존 시간표 화면 갈아엎기 금지 |
| `routes/study-material-wrongs.js` | 수업자료 오답 | `/api/study-materials`, `/api/material-*`, `/api/class-material-assignments` | `study_materials`, `material_unit_ranges`, `material_question_tags`, `class_material_assignments`, `student_material_submissions`, `student_material_wrong_answers` | 일반 시험 OMR 수정 금지 정책과 분리해서 관리 |
...
| `/api/study-materials`, `/api/material-*`, `/api/class-material-assignments` | `routes/study-material-wrongs.js` | 분리 완료 | materials, assignments, wrongs |
...
| `apmath/js/management.js` | 반 관리, 수납·출납 foundation 설정/요약 | `/api/classes`, `/api/class-students`, `/api/billing-accounting-foundation/*` | `routes/classes.js`, `routes/billing-accounting-foundation.js` | 반명/구조 변경 주의, 운영 진입점 숨김 유지 |
...
| 시간표 foundation | `timetable.js` | `/api/class-time-slots`, `/api/timetable-conflicts*` | `routes/class-time-slots.js`, `routes/timetable-conflicts.js` | `class_time_slots`, `timetable_conflict_logs`, `timetable_conflict_overrides` | 중등 90분, 고등 120분 |
...
| `timetable_version_slots` | 시간표 버전별 수업 슬롯 | timetable-versions | 시간표 버전 UI | active `class_time_slots`와 직접 혼용 금지 |
| `study_materials` | 수업자료/교재 메타 | study-material-wrongs | 수업자료 오답 UI | 학생 포털 원문 열기 금지 |
| `material_unit_ranges` | 수업자료 단원 범위 | study-material-wrongs | 수업자료 오답 UI | 범위 중복 주의 |
| `material_question_tags` | 수업자료 문항 태그 | study-material-wrongs | 수업자료 오답 UI | 문항 번호/태그 정합성 주의 |
| `class_material_assignments` | 반별 수업자료 배정 | study-material-wrongs | 수업자료 오답 UI, 학생 포털 | 반/학생 배정 범위 주의 |
| `student_material_submissions` | 학생 수업자료 제출 상태 | study-material-wrongs | 수업자료 오답 UI, 학생 포털 | 제출 후 재수정 허용 정책 유지 |
| `student_material_wrong_answers` | 학생 수업자료 오답 번호 | study-material-wrongs | 수업자료 오답 UI, 학생 포털 | 일반 시험 오답과 분리 유지 |
...
# 11. Codex / AI 작업 지시서 표준

## 11.1 목적

큰 프로젝트를 Codex, ChatGPT, Gemini, Claude, 사람 작업자가 수행할 때는 작업을 검수 가능한 단일 미션 단위로 쪼갠다.

이 표준의 목적은 작업자가 임의로 범위를 넓히거나, 기존 기능을 훼손하거나, 완료 보고와 실제 코드가 불일치하는 사고를 줄이는 것이다.

모든 큰 작업은 하나의 명확한 목표, 명확한 제약, 검증 가능한 완료 조건, 중단 규칙을 가진다.

## 11.2 필수 지시서 구조

모든 `CODEX_TASK.md`는 아래 구조를 기본으로 한다.
...
작업 규모가 작더라도 이 구조를 최대한 유지한다.

## 11.3 GOAL

하나의 명확하고 측정 가능한 목표를 적는다.

원칙:

- 미션은 반드시 하나만 둔다.
- 한 지시서에 여러 기능을 섞지 않는다.
- “가능하면 같이 정리” 같은 문구를 쓰지 않는다.
- 목표 달성 여부를 검수자가 확인할 수 있어야 한다.

예시:

```text
좋음: homework-photo 배정 삭제 후 목록 재진입 오류를 수정한다.
나쁨: homework-photo도 고치고 화면도 예쁘게 정리하고 관련 기능도 개선한다.
```
...
- 어떤 파일을 다루는지
- 어떤 구조인지
- 어느 route/API/DB와 연결되는지
- 이전에 내린 결정은 무엇인지
- 반드시 보존해야 할 운영 흐름은 무엇인지

CONTEXT는 작업자가 수정 전에 전체 흐름을 오해하지 않도록 하기 위한 구간이다.
...
CONSTRAINTS는 작업자의 창의적 개선을 막기 위한 것이 아니라, 운영 중인 프로젝트를 보호하기 위한 장치다.

## 11.6 PRIORITY

여러 기준이 충돌할 때 무엇을 먼저 지킬지 순서를 정한다.
...
우선순위가 없으면 작업자가 편한 방향으로 판단할 수 있으므로, 큰 작업에서는 반드시 명시한다.

## 11.7 PLAN

작업자가 바로 수정하지 않고 먼저 이해하도록 지시한다.
...
- 수정 범위를 최소화한다.
- 큰 변경이 필요하면 먼저 멈춘다.
- 이해한 내용과 실제 수정이 일치하도록 한다.

큰 리팩터링 작업에서는 PLAN 단계에서 분석 문서를 먼저 만들고, 구현은 다음 미션으로 분리한다.

## 11.8 DONE WHEN

완료 상태를 검증 가능한 문장으로 적는다.

예시:

```text
- 삭제된 숙제 사진 배정은 다시 목록에 나타나지 않는다.
- 삭제 후 교재관리 모달을 다시 열어도 화면이 깨지지 않는다.
- node --check가 통과한다.
- CODEX_RESULT.md가 작성되어 있다.
```

DONE WHEN은 “잘 동작함”처럼 추상적으로 쓰지 않는다.
...
검증할 수 없는 항목은 PASS라고 쓰지 않고, 검증하지 못한 이유를 적는다.

예시:

```text
운영 API smoke test는 사용자 명시 지시가 없어 실행하지 않았다.
```
...
모든 작업자는 루트의 `CODEX_RESULT.md`에 완료 보고를 저장한다.
...
- 요청 범위를 넘는 수정이 필요하면 멈춘다.
- 기존 UI 문구 변경이 필요하면 멈춘다.
- DB migration이 필요한데 지시서에 없으면 멈춘다.
- 운영 데이터 호출이 필요하면 멈춘다.
- 관리자 화면 변경이 필요하면 멈춘다.
- 인증/권한 정책 변경이 필요하면 멈춘다.
- 목표를 달성했으면 추가 개선하지 않는다.
- 확신이 없으면 추측으로 구현하지 않는다.

STOP RULES는 큰 프로젝트에서 범위 폭주를 막는 핵심 장치다.
...
1. 현재 `initial-data` 구성표 작성
2. 프론트 파일별 사용 데이터 지도 작성
3. 첫 화면 필수 데이터와 lazy load 가능 데이터 분류
4. 분리 가능한 데이터와 보류 데이터 구분
5. 1차 분리 대상 선정
6. replacement API 준비
7. 프론트 lazy loader 연결
8. 기존 `/api/initial-data` 응답 key 제거 검토
9. 회귀 검수

직접 구현은 분석과 설계가 끝난 뒤 별도 미션으로 진행한다.

## 11.13 우리 프로젝트에 특히 해당되는 작업 유형

이 표준은 아래 작업에서 반드시 적용한다.
...
## 11.15 완료 보고와 실제 코드 일치 원칙

완료 보고는 실제 코드와 반드시 일치해야 한다.

금지:

- 구현하지 않은 내용을 완료했다고 쓰기
- 검증하지 않은 항목을 PASS라고 쓰기
- 수정하지 않은 파일을 수정했다고 쓰기
- 실제 코드와 다른 결과 요약 작성
- 실패한 테스트를 생략하기
- 위험했던 점을 숨기기

완료 보고와 실제 코드가 불일치하면 작업 실패로 본다.
...
- 이번 작업의 GOAL이 하나로 명확한가?
- CONTEXT/CONSTRAINTS/PRIORITY/PLAN/DONE WHEN/VERIFY/OUTPUT/STOP RULES가 지시서에 포함되어 있는가?
- 목표 달성 후 범위를 확장하려고 하지 않는가?
- 이번 작업이 UI 작업인가?
- 기존 문구·버튼명·화면명을 바꾸는가?
...
- DONE WHEN에 적힌 완료 조건을 실제로 만족하는가?
- VERIFY에 적힌 검증을 실제로 수행했는가?
- 검증하지 못한 항목을 PASS라고 쓰지 않았는가?
- STOP RULES를 어기고 범위를 확장하지 않았는가?
- CODEX_RESULT.md 내용이 실제 코드 변경과 일치하는가?
...
- 학생은 제출 후에도 수업자료/교재 오답을 다시 수정할 수 있다.
- 학생 포털의 수업자료/교재 오답 카드는 제출 전 `오답 입력`, 제출 후 `오답 수정` 진입을 유지한다.
- 수업자료/교재 오답은 기존 `student_material_submissions` row를 갱신하고, 현재 남은 오답 번호 기준으로 `student_material_wrong_answers`를 다시 저장한다.
- 이 수정 가능 정책은 일반 시험 OMR 제출 완료 수정 금지와 별개이며, `student-portal/omr-submit`, `exam_sessions`, `wrong_answers`, `check-omr`, `exams` 흐름은 계속 수정 금지로 유지한다.
...
# 2026-05-19 APMS 운영 이력/시간표 안정화 통합 1차

## 적용 원칙

학생 상세 lazy loader와 시간표 충돌 상세 UI는 기존 AP Math OS 화면을 갈아엎지 않고, 이미 존재하는 화면 안에서 확인 가능한 형태로만 연결한다.

## 학생 상세 이력 UI

- 학생 상세의 `상담기록` 탭 안에 `학생 이력` 확인 영역을 둔다.
- `상태 변경 이력`과 `반 이동 이력`은 버튼을 눌렀을 때 모달로만 확인한다.
- 이력 UI는 read-only이며, 이 작업에서는 상태 변경/반 이동 수정 기능을 만들지 않는다.
- `student_status_history`, `class_transfer_history`는 `/api/students/:id/detail-data`와 `state.ui.studentDetailLazyData`를 통해 지연 로딩한다.
- `message_logs`는 학생 상세 detail-data 응답에 포함하지 않는다.
- 연락 이력은 기존 연락 이력 보기 흐름 또는 별도 2차 lazy loader 후보로 둔다.

## 시간표 충돌 상세 UI

- 시간표 충돌 상세는 시간표 draft/preview 흐름 안에서만 확인한다.
- 학생 충돌은 위험 항목으로 보고, 선생님 충돌은 합반/클리닉/격주 등 운영 예외 가능 항목으로 분리한다.
- 교실 충돌은 교실명이 입력된 경우의 확인 항목으로 본다.
- 원장 대시보드나 메인 운영 화면에 새 카드로 노출하지 않는다.

## 금지

- 원장/관리자 대시보드 새 카드 추가 금지
- 숨겨진 foundation 기능 기본 노출 금지
...
- APMS/왕지교육/Academy OS 작업에서 `취소`는 단순 닫기가 아니라 바로 전 단계로 돌아가는 흐름을 우선한다.
- 모달 안에서 하위 모달이나 상세 화면으로 들어간 경우 `취소`는 직전 모달/직전 단계로 복귀해야 한다.
- 상위 큰 화면, 대시보드, 반 화면, 학생 목록 등으로 갑자기 튀는 동작은 UX 회귀로 본다.
- 전체 닫기, 저장 완료 후 닫기, 강제 종료가 필요한 경우에는 명시적으로 suppress/close 성격을 구분한다.
- 모달 내부 클릭은 overlay 닫기/큰 화면 복귀로 전파되지 않도록 방어한다.

---

## 2026-05-19 추가 원칙: 학생 등록 중복 방지와 숨김 학생 정리

- 신입생 등록은 저장 중 중복 클릭으로 같은 학생이 두 번 생성되지 않도록 프론트 버튼 잠금과 백엔드 최근 동일 요청 방어를 함께 둔다.
- 신입생 등록 시 PIN은 자동 생성 기준으로 본다. 운영 대시보드의 `PIN 미발급` 확인 카드와 기본 노출형 PIN 생성 진입점은 운영 체크 항목으로 두지 않는다.
- `숨김` 상태 학생은 재원/퇴원/반 정리 필요 집계에서 제외한다. 중복 생성 정리용으로만 별도 숨김 학생 목록에서 관리한다.
- 숨김 학생의 완전 삭제는 중복 생성된 학생 정리용으로 제한한다. 출결·숙제·시험·상담·수납 등 운영 기록이 있는 학생은 완전 삭제하지 않고 숨김 상태로 보관한다.
- 학생 관련 금전·상담·시험 기록이 남아 있는 경우에는 데이터 보존을 우선한다.



## APMS 시간표 원장/admin 보기 기준

- 원장/admin 계정에는 `내 반 보기`가 필요 없다.
- 원장/admin 시간표는 항상 전체 선생님/전체 반 기준으로 보여야 한다.
- 원장/admin에서 이전 `내 반 보기` 상태가 남아 있어도 시간표 렌더링에는 적용하지 않는다.
- 선생님 계정의 기존 `전체 보기` / `내 반 보기` 동작은 별도 지시 없이는 변경하지 않는다.
- 시간표 개편안 진입 버튼은 상단 오른쪽 끝에 작고 조용한 보조 버튼으로 둔다.

## APMS 새학기 시간표 개편안 기준

- 새학기 개편안은 운영 시간표를 즉시 변경하지 않는 draft/staging 작업 공간이다.
- 1월이 되어도 자동 적용하지 않는다.
- 사용자가 적용일을 직접 저장하고, 운영 시간표 적용을 직접 실행해야 실제 운영 데이터에 반영한다.
- 적용 전 새 반/학생 배치는 운영 `class_students`, `student_enrollments`, `class_time_slots`에 섞이지 않는다.
- 적용 후 새 반은 실제 `classes`, `class_students`, `class_time_slots`, `teacher_classes`에 반영되어 클래스룸에서 바로 열려야 한다.
- 새학기 개편안에서는 새 반 추가, 신규 학생 추가, 기존 학생 드래그 배치가 가능해야 한다.
- 학생 드래그와 반 드래그는 반드시 payload를 분리하고 이벤트 전파를 차단한다.
