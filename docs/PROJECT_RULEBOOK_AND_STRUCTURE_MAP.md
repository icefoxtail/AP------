...
| `apmath/js/management.js` | 수납·출납 foundation 설정/요약 | `/api/billing-accounting-foundation/*` | `routes/billing-accounting-foundation.js` | 실제 수납/환불/이월 처리 금지 |
...
| 수납·출납 foundation | `dashboard.js` 관리 진입점, `management.js` 설정/요약 모달 | `/api/billing-accounting-foundation/*` | `routes/billing-accounting-foundation.js` | `payment_methods`, `payment_transactions`, `cashbook_entries`, `refund_records`, `carryover_records`, `billing_policy_rules`, `accounting_daily_summaries`, `accounting_monthly_summaries` | 실제 수납 처리 금지 |
...
- `cashbook_entries`처럼 status/is_active가 없는 테이블은 schema 보강 전까지 취소/비활성화 UI를 억지로 만들지 않는다.
...
- 수납·출납 foundation 1단계-D: 실제 운영 입력 흐름 검수 및 월별 수납 운영 화면 고도화
