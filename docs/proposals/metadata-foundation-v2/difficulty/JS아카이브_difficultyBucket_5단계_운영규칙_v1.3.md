...
작성일: 2026-09-16  
대상: 코드검사실 / JS아카이브  
상태: **CANONICAL_CANDIDATE — v1.2 독립재검 DF-005 핀포인트 수정 후 재검 대기**  
...
v1.2 독립재검에서 확인된 DF-005 report export mapping 결함만 핀포인트 수정했다.

- `final_bucket ← difficultyBucket` mapping을 추가했다.
- builder / generated metadata / runtime sidecar의 canonical field명을 camelCase 4개로 명시했다.
- `blind_bucket`은 blind first-pass evidence 전용 report field이며 canonical `difficultyBucket` export가 아님을 명시했다.
- canonical `difficultyBucket`의 report export는 `final_bucket`으로만 고정했다.

- **기존 v1.2에서 유지:** `BORDERLINE_ACCEPTABLE`의 저장 위치와 enum을 고정했다.
  - `legacyLevelCompatibility` 허용값에 `BORDERLINE_ACCEPTABLE`을 추가한다.
  - `BORDERLINE_REVIEW`는 인접 legacy-level mismatch의 재검 대기 상태다.
  - 재검 후 기존 `LEVEL BORDERLINE STABILITY LOCK`에 따라 해당 mismatch를 허용하기로 확정하면 `BORDERLINE_ACCEPTABLE`로 전환한다.
  - 순수한 difficulty 경계(`B12/B23/B34/B45`) 자체는 `difficultyBoundaryFlag`에만 기록하며, 그것만으로 `BORDERLINE_ACCEPTABLE`을 쓰지 않는다.
- **기존 v1.2에서 유지:** `difficultyConfidence`의 persistence 위치를 고정했다.
  - `difficultyBucket`, `difficultyConfidence`, `difficultyBoundaryFlag`, `legacyLevelCompatibility` 4개를 canonical metadata layer에 함께 저장한다.
  - generated metadata/runtime sidecar가 존재하는 경우 이 4개 필드를 1:1 보존한다.
  - `difficultyReason`, `structureEvidence`, `reviewer`는 audit sidecar의 선택 필드다.
- **기존 v1.2에서 유지:** §9·§25의 이중 backtick 표기를 정상 inline code로 정리했다.

v1.0~v1.2에서 확정한 5단계 의미, 4개 경계, blind-first-pass, solution evidence, 기존 `level` 보존 원칙은 변경하지 않았다.
---
...
1. `difficultyBucket`의 **확정 난이도 값**은 반드시 `1|2|3|4|5` 중 하나다.
   - 아직 판정되지 않은 문항만 storage sentinel `UNKNOWN`을 허용한다.
   - `UNKNOWN`은 난이도 단계가 아니며 자동출제·추천에 사용할 수 없다.
   - `HOLD`는 `difficultyBucket` 값으로 사용하지 않고 `reviewStatus`에 기록한다.
...
13. 경계 문항은 강제로 한쪽 논리를 만들어내지 말고 `difficultyBoundaryFlag`와 difficultyConfidence로 표시한다.
...
# 2.1 Storage / Status 모델

canonical 저장 규칙은 다음으로 고정한다.
...
difficultyBoundaryFlag:
  NONE | B12 | B23 | B34 | B45 | UNKNOWN

legacyLevelCompatibility:
  NORMAL | BORDERLINE_REVIEW | BORDERLINE_ACCEPTABLE | STRONG_CONFLICT | UNKNOWN

reviewStatus:
  기존 프로젝트의 review status 체계를 사용
  HOLD/manual_review/reviewed_pass/reviewed_fail 등은 여기서 관리
```
...
은 **아직 난이도 값이 확정되지 않음**을 뜻한다.

```text
reviewStatus = HOLD
```

는 **판정을 중단하고 추가 근거·재검을 기다리는 상태**를 뜻한다.

따라서 다음은 금지한다.

```text
difficultyBucket = HOLD
difficultyBucket = manual_review
difficultyBucket = reviewed_pass
```

validator는 `HOLD`를 유효한 bucket 값으로 인정하면 안 된다.

`legacyLevelCompatibility`의 의미는 다음으로 고정한다.

```text
NORMAL
  신규 difficultyBucket과 기존 level의 기본 mapping이 일치

BORDERLINE_REVIEW
  인접 legacy-level mismatch가 있어 재검이 필요한 상태

BORDERLINE_ACCEPTABLE
  인접 legacy-level mismatch를 재검했고,
  기존 LEVEL BORDERLINE STABILITY LOCK에 따라 허용하기로 확정한 상태

STRONG_CONFLICT
  강한 mismatch로 independent recheck/adjudication이 필요한 상태
  adjudication이 끝난 뒤에도 historical relation 표시는 유지할 수 있음

UNKNOWN
  아직 legacy compare를 수행하지 않았거나 비교 불가
```

중요:

- `BORDERLINE_ACCEPTABLE`은 `reviewStatus` 값이 아니다.
- `BORDERLINE_ACCEPTABLE`은 `difficultyBoundaryFlag` 값도 아니다.
- 단순히 `difficultyBoundaryFlag = B12|B23|B34|B45`라는 이유만으로 `BORDERLINE_ACCEPTABLE`을 기록하지 않는다.
- `BORDERLINE_ACCEPTABLE`은 **기존 level과 신규 bucket 사이의 인접 mismatch를 재검 후 허용한 경우에만** 사용한다.
...
→ bucket 판정
→ difficultyConfidence / difficultyBoundaryFlag 판정
...
- `low`는 자동 확정 금지
- `difficultyConfidence = medium` + `difficultyBoundaryFlag != NONE`는 recheck 대상
...
- boundary는 실패가 아니다.
- 경계 문항을 감추기 위해 difficultyConfidence를 억지로 high로 올리지 않는다.
- 재검 후에도 인접 두 bucket이 모두 합리적이면 최종 adjudication값을 유지하고 `difficultyBoundaryFlag`를 그대로 보존할 수 있다.
- 이 difficulty 경계 자체는 `BORDERLINE_ACCEPTABLE`의 근거가 아니다.
- 기존 `level`과의 인접 mismatch까지 함께 존재하고 §11 절차에서 허용 판정이 난 경우에만 `legacyLevelCompatibility = BORDERLINE_ACCEPTABLE`을 사용한다.
...
- first compare에서는 `legacyLevelCompatibility = BORDERLINE_REVIEW`
- 근거 기록 필요
- independent recheck 후 기존 `LEVEL BORDERLINE STABILITY LOCK`에 따라 인접 mismatch를 허용하면
  `legacyLevelCompatibility = BORDERLINE_ACCEPTABLE`로 전환
- 재검 결과 bucket 또는 legacy level을 수정해 기본 mapping으로 돌아오면 `NORMAL`
...
- adjudication 전에는 `reviewStatus = reviewed_pass`로 승격하지 않는다.
- adjudication 후 신규 bucket을 유지하기로 확정한 경우 `legacyLevelCompatibility = STRONG_CONFLICT` 표시는 historical relation으로 남길 수 있으며,
  이때 `reviewStatus = reviewed_pass`가 가능하다.
...
1. `difficultyConfidence = low`
2. `difficultyBoundaryFlag != NONE`
...
승인 후 **canonical metadata layer**에는 다음 difficulty canonical field를 함께 저장한다.
...
기존 metadata workflow field도 함께 유지한다.
...
- 위 4개 difficulty canonical field는 **source exam JS 본문을 수정해서 넣는 필드가 아니라 canonical metadata layer의 field**다.
- generated metadata/runtime sidecar가 존재하는 경우 위 4개 필드를 1:1 보존해야 한다.
- builder → generated metadata → runtime sidecar 사이에서 이름·값이 바뀌거나 누락되면 parity FAIL이다.
- UI가 모든 field를 노출할 필요는 없지만 저장·검증 계층에서는 보존해야 한다.

필요한 audit sidecar에는 다음을 추가로 보존할 수 있다.
...
즉 `difficultyConfidence`는 report-only 값이 아니며 canonical metadata에 지속 저장한다.
## 19.2 금지
...
`reviewStatus = reviewed_pass`로 승격하려면 difficulty canonical field가 다음 조건을 만족해야 한다.

```text
difficultyBucket ∈ {1,2,3,4,5}
difficultyConfidence ∈ {high,medium,low}
difficultyBoundaryFlag ∈ {NONE,B12,B23,B34,B45}
legacyLevelCompatibility ∈ {NORMAL,BORDERLINE_ACCEPTABLE,STRONG_CONFLICT}
```

추가 규칙:

- `UNKNOWN`이 하나라도 남아 있으면 `reviewed_pass` 금지.
- `legacyLevelCompatibility = BORDERLINE_REVIEW` 상태에서는 `reviewed_pass` 금지.
- `legacyLevelCompatibility = STRONG_CONFLICT`는 independent recheck/adjudication이 완료된 경우에만 `reviewed_pass` 허용.
- `difficultyConfidence = low`는 자동 실패가 아니지만 반드시 independent recheck 또는 명시적 adjudication evidence가 있어야 한다.
...
처럼 저장한다. `HOLD`를 `difficultyBucket` 값으로 저장하지 않는다.
...
difficulty_boundary_flag,
legacy_level_compatibility,
recheck_bucket,
final_bucket,
tag_status,
review_status,
...
Report export mapping은 다음 4개 canonical difficulty field와 1:1로 대응한다.
...
builder / generated metadata / runtime sidecar의 canonical field명은 camelCase 4개를 사용하고,
CSV/보고서에서만 위 snake_case export명을 사용한다.

`blind_bucket`은 blind first-pass에서 생성되는 evidence용 report field이며,
canonical `difficultyBucket`의 export field가 아니다.
canonical `difficultyBucket`의 report export는 `final_bucket`으로만 고정한다.
...
legacy normal:
legacy borderline review:
legacy borderline acceptable:
...
difficultyBoundaryFlag ∈ {NONE,B12,B23,B34,B45,UNKNOWN}
legacyLevelCompatibility ∈ {NORMAL,BORDERLINE_REVIEW,BORDERLINE_ACCEPTABLE,STRONG_CONFLICT,UNKNOWN}
...
difficultyBoundaryFlag != NONE → 경계 재검 여부 확인
legacyLevelCompatibility = BORDERLINE_REVIEW → reviewed_pass 금지
legacyLevelCompatibility = BORDERLINE_ACCEPTABLE → prior recheck/adjudication evidence 필수
legacyLevelCompatibility = STRONG_CONFLICT → independent recheck/adjudication 완료 전 reviewed_pass 금지

reviewStatus = reviewed_pass인 문항:
  difficultyBucket != UNKNOWN
...
canonical metadata layer ↔ generated metadata/runtime sidecar
difficultyBucket / difficultyConfidence / difficultyBoundaryFlag / legacyLevelCompatibility 1:1 parity PASS
...
8. `difficultyBucket`에서 `HOLD` 상태값이 제거되고 `UNKNOWN`과 `reviewStatus`가 분리되어 있음
9. `difficultyConfidence` / `difficultyBoundaryFlag` / `legacyLevelCompatibility` canonical field명이 문서 전체에서 일관됨

10. `BORDERLINE_ACCEPTABLE`이 `legacyLevelCompatibility` enum과 전이 규칙에 명시되어 있음
11. `difficultyConfidence`가 canonical metadata layer에 지속 저장되고 generated metadata/runtime sidecar parity 검증 대상임
...
difficultyBoundaryFlag
legacyLevelCompatibility
= canonical metadata layer에 함께 저장
= generated metadata/runtime sidecar 1:1 parity

state separation
difficultyBucket = UNKNOWN   # 미분류
...
STATE:
CANONICAL_CANDIDATE — v1.3 재검 대기
