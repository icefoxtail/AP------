# JS아카이브 `difficultyBucket` 5단계 운영규칙 v1.1

작성일: 2026-09-16  
대상: 코드검사실 / JS아카이브  
상태: **CANONICAL_CANDIDATE — v1.0 독립검수 P1 2건 핀포인트 수정 후 재검 대기**  
...
v1.0 독립 전수검수에서 확인된 P1 2건만 핀포인트 수정했다.

- **DF-001 해결:** `difficultyBucket`의 실제 난이도 값과 미분류/보류 상태를 분리했다.
  - 확정 난이도 값: `1|2|3|4|5`
  - 미분류 sentinel: `UNKNOWN`
  - `HOLD`는 `difficultyBucket` 값이 아니라 `reviewStatus` 상태다.
- **DF-002 해결:** canonical field명을 Metadata Contract 후보와 동일하게 통일했다.
  - `difficultyConfidence`
  - `difficultyBoundaryFlag`
  - `legacyLevelCompatibility`

v1.0의 5단계 의미, 4개 경계, blind-first-pass, solution evidence, 기존 `level` 보존 원칙은 변경하지 않았다.
...
legacyLevelCompatibility:
  NORMAL | BORDERLINE_REVIEW | STRONG_CONFLICT | UNKNOWN
...
UID cardinality 유지
difficultyBucket ∈ {1,2,3,4,5,UNKNOWN}; `HOLD`는 reviewStatus에서만 허용
불법 문자열 bucket = 0
blind first-pass ledger 존재
compare가 first-pass freeze 이후 수행됨
``difficultyConfidence = low`` 재검 또는 HOLD
...
```text
DOCUMENT:
JS아카이브_difficultyBucket_5단계_운영규칙_v1.1.md
