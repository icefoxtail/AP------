# JS아카이브 `difficultyBucket` 5단계 운영규칙 v1.2

작성일: 2026-09-16  
대상: 코드검사실 / JS아카이브  
상태: **CANONICAL_CANDIDATE — v1.1 독립재검 P1 2건 핀포인트 수정 후 재검 대기**  
...
v1.1 독립재검에서 확인된 P1 2건과 비차단 표기 오류만 핀포인트 수정했다.

- **DF-003 해결:** `BORDERLINE_ACCEPTABLE`의 저장 위치와 enum을 고정했다.
...
- **DF-004 해결:** `difficultyConfidence`의 persistence 위치를 고정했다.
...
- **문서 품질 정리:** §9·§25의 이중 backtick 표기를 정상 inline code로 정리했다.

v1.0~v1.1에서 확정한 5단계 의미, 4개 경계, blind-first-pass, solution evidence, 기존 `level` 보존 원칙은 변경하지 않았다.
---
...
report export mapping은 다음으로 고정한다.
...
builder/runtime/sidecar의 canonical field명은 camelCase 3개를 사용하고,
...
```text
DOCUMENT:
JS아카이브_difficultyBucket_5단계_운영규칙_v1.2.md

STATE:
CANONICAL_CANDIDATE — v1.2 재검 대기
