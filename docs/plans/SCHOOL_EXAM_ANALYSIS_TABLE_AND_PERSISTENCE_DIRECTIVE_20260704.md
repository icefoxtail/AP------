...
## 부록 B-0 — 검수 인수 기준 (STEP 2 표 — 이 기준 미달이면 반려)

사장님 최우선 지적 = 기존 "문항 분석 상태" 벽글. 검수 시 아래를 하니스 육안으로 **문항 하나하나** 확인한다:

1. **한 줄 = 한 문항.** 어떤 문항도 접힌 상태에서 2줄 이상 차지하지 않는다. 벽글 잔재(카드 나열) 0.
2. **기본 정보 완비.** 전 행에 `번호 · 유형(객/주관식) · 난도칩 · 단원 · 개념 한 줄 · 배점 · 전체 응시 정답률 · 오답 수 · 태그`가 채워져 있거나, 없는 값은 명시적 placeholder(`-`/`분석 대기`)로 표기 — 빈칸·`0/-`·`undefined` 노출 시 반려.
3. **난도·유형 칩이 색으로 구분**되고 다크모드에서도 깨지지 않는다(토큰 확인).
4. **행 펼침**이 클릭 한 번으로 열리고, 묻는것/함정/풀이/지도 + 오답 학생 명단이 정돈된 라벨 구조로 나온다(라벨 볼드+본문, 줄바꿈 뭉개짐 없음).
5. **수식**이 표 안에서도 MathJax로 조판된다(`x^2` 날것 노출 0).
6. **인쇄 문서**: A4에서 표 헤더가 페이지마다 반복되고, 행+펼침 블록이 페이지 경계에서 잘리지 않는다.
7. 23문항 시험 기준 표 전체가 **스크롤 2~3화면 내**에 들어온다(접힘 기본).
