...
#### 평면좌표·내분·직선 위 점의 위치관계 SVG 규칙

내분비, 직선 위 점의 위치, 선분 안·밖의 경우 구분이 풀이의 핵심 논리인 문항은
해설 텍스트만으로 끝내지 않는다. 학생의 위치 판단에 도움이 되면 `solutionImage` SVG에
수직선·한 직선 위 배치 또는 좌표평면을 직접 렌더한다. SVG에는 풀이에 필요한 사실을
실제로 표시해야 한다.

- 핵심 점 A, B, C 등과 점의 실제 순서
- `A-C-B`, `C-A-B`와 같은 위치관계
- `AP:PB=2:1`, `BD:DC=2:3`과 같은 내분비
- 좌표, 필요한 거리 관계, x좌표·y좌표 변화량, 동일 직선 위 관계

점 하나만 좌표평면에 찍는 것처럼 핵심 위치관계·내분비를 확인할 수 없는 그림은
시각해설 PASS로 인정하지 않으며, 필요한 위치관계 시각자료의 누락은
`SOLUTION_VISUAL_MISSING`으로 판정한다. SVG 자체를 보았을 때 해설에서 사용한
내분비와 점의 순서를 학생이 확인할 수 있어야 한다.

점의 위치에 따라 두 개 이상의 해가 발생하는 문항은 각 경우를 시각적으로 분리한다.
예를 들어 다음처럼 `CASE 1`, `CASE 2`를 구분하여 점의 순서와 거리비를 각각 표시하고,
필요하면 이어서 실제 좌표평면과 좌표 변화를 별도 패널에 둔다.
...
권장 구조는 `Panel A`에 점의 순서·거리비를, `Panel B`에 실제 좌표평면·좌표 변화를
배치하는 것이다. 한 SVG에 모든 내용을 억지로 겹쳐 가독성을 떨어뜨리지 않는다.

해설과 SVG의 평면좌표 사실은 다음과 같이 parity를 확인한다. 해설이
`BD:DC=2:3`을 핵심 근거로 사용하면 SVG에서도 B-D-C의 순서와 2:3 관계가 실제로
확인 가능해야 하며, 해설이 `C-A-B`를 사용하면 SVG가 반대 순서인 A-C-B를 그려서는
안 된다. solution의 점 순서·내분비·좌표·거리 관계·경우 수와 SVG의 대응 사실이
하나라도 다르면 다음 불일치로 기록하여 기존 semantic/parity FAIL 체계에 연결한다.

- solution 점 순서 ≠ SVG 점 순서
- solution 내분비 ≠ SVG 내분비
- solution 좌표 ≠ SVG 좌표
- solution 거리 관계 ≠ SVG 거리 관계
- solution 경우 수 ≠ SVG에서 표현하는 경우 수

이 불일치를 위한 새 상태명은 만들지 않는다.

#### 2-2-1-A. 2022 개정 공통수학2 도형의 방정식 단원별 SVG Hard Rule

다음 규칙은 `standardCourse = 공통수학2`이고 `standardUnitKey`가
`H22-C2-01`~`H22-C2-04`인 도형의 방정식 계열 문항에 적용한다.
단순히 단원명만 보고 그림을 추가하지 않으며, 실제 풀이에서 핵심이 된 수학 사실과
학생의 위치·관계 판단에 필요한 시각 정보를 기준으로 적용한다. 해당 사실이 해설의
핵심인데 SVG에 없으면 `SOLUTION_VISUAL_MISSING`이다.

##### H22-C2-02 직선의 방정식

직선의 식·기울기·절편·교점·평행·수직·거리·각이 풀이의 핵심이면 SVG에 해설에서
사용한 사실을 실제로 표시한다.

- 직선의 방정식과 문제에서 사용하는 점·절편·교점을 표시한다.
- 점과 직선 사이 거리이면 수선의 발 `H`, 수선분 `PH`, 필요한 경우 직각 표시와 거리값을 표시한다.
- 평행한 두 직선 사이 거리이면 두 직선과 공통 수선, 두 직선 사이의 거리 관계를 표시한다.
- 수직 관계이면 두 직선의 실제 교차 또는 수선 관계를 표시하고, 해설에서 선택한 기울기 관계와 일치시킨다.
- 두 직선이 이루는 각이면 예각·둔각 중 문제에서 요구한 각과 실제 선택 근거를 표시한다.
- 매개변수·절댓값으로 두 개 이상의 직선·교점·거리 조건이 생기면 `CASE 1`, `CASE 2`로 분리한다.

해설의 직선식 ≠ SVG 직선, 해설의 교점 ≠ SVG 교점, 해설의 수직·평행·거리 관계 ≠
SVG 관계이면 기존 semantic/parity FAIL 체계에 연결한다. 단순히 좌표평면에 선 하나만
그린 것은 거리·수직·평행·각을 사용한 풀이의 시각해설로 인정하지 않는다.

##### H22-C2-03 원의 방정식

원의 중심·반지름·표준형·원과 직선 또는 원과 원의 위치관계·접선이 풀이의 핵심이면
SVG에 중심, 반지름, 관련 직선·원, 교점·접점을 실제로 표시한다.

- 중심과 반지름을 좌표·길이·반지름 선분으로 확인할 수 있게 한다.
- 원과 직선은 중심에서 직선까지의 거리 `d`와 반지름 `r`의 관계에 따라 `d<r`, `d=r`, `d>r`을 각각 두 교점·접점 한 개·교점 없음으로 표시한다.
- 원과 원은 중심 사이 거리 `d`와 `r_1+r_2`, `|r_1-r_2|`의 관계에 따라 교점 0개·1개·2개를 구분한다.
- 접선은 중심–접점 반지름과 접선의 수직 관계를 표시한다. 두 접선이면 두 접선과 두 접점을 모두 표시한다.
- 매개변수에 따라 원의 위치·반지름·교점 수가 달라지면 경우별 원·직선·교점을 별도 패널 또는 `CASE`로 분리한다.
- `r^2=0`은 한 점, `r^2<0`은 실수 범위의 원 없음으로 표시하며, 성립하지 않는 원을 그리지 않는다.

해설의 중심·반지름·교점 수·접점·접선식 ≠ SVG의 대응 geometry이면 기존
semantic/parity FAIL 체계에 연결한다. 원만 그려 놓고 접선 조건이나 교점 개수를
확인할 수 없는 그림은 시각해설로 인정하지 않는다.

##### H22-C2-04 도형의 이동

평행이동·대칭이동·합성 이동·변환된 도형의 방정식·이동 후 교점 또는 최단거리가
풀이의 핵심이면 원래 도형과 이동 후 도형의 대응을 SVG에 실제로 표시한다.

- 평행이동은 원래 도형과 이동 후 도형, 대응점, 이동 방향과 이동량을 화살표 또는 동등하게 명확한 표시로 나타낸다.
- 대칭이동은 대칭축, 원래 점·도형과 대응점·도형, 축까지의 수직 또는 좌우·상하 대응을 표시한다.
- 합성 이동은 `STEP 1`, `STEP 2` 또는 별도 패널로 변환 순서를 분리한다. 모든 변환을 한 그림에 겹쳐 순서를 잃게 하지 않는다.
- 방정식 변환은 원래 식과 변환 후 식, 좌표 대응 또는 치환 방향을 함께 표시한다. 식의 치환 방향과 실제 도형 이동 방향이 반대가 되지 않도록 한다.
- 대칭이동을 이용한 최단거리이면 반사점, 대칭축, 직선으로 펴진 경로와 최단거리 선분을 표시한다.
- 이동 후에도 보존되는 평행·수직·거리·반지름·도형의 모양이 풀이 근거이면 그 보존 관계를 표시한다.

해설의 원래 도형·이동량·대칭축·변환 순서·이동 후 식·교점 ≠ SVG의 대응 사실이면
기존 semantic/parity FAIL 체계에 연결한다. “좌표평면에 표시한 그림”이라는 일반
캡션만 있고 원래 도형과 이동 후 도형의 대응을 확인할 수 없으면 PASS로 인정하지 않는다.

이 단원별 규칙은 `H22-C2-01`~`H22-C2-04` 범위에만 우선 적용한다.
`H22-GE-08` 도형의 방정식, `H15` 문항 및 선택과목의 도형 문항에 자동 소급하지 않으며,
그 범위는 해당 교육과정 프로필과 별도 규정으로 판정한다.
...
#### 3-7-1. 2022 개정 공통수학2 · 평면좌표: 내분·점 위치관계 Hard Rule

이하 규칙은 우선적으로 `standardCourse = 공통수학2`,
`standardUnit = 평면좌표`, `standardUnitKey = H22-C2-01`인 문항의 학생용 해설에
적용한다. 다만 단순 metadata 문자열만으로 범위를 결정하지 않고, 실제 문항이 내분점·중점·
무게중심·각의 이등분선에 의한 선분비·직선 위 점의 거리비·선분 안·밖의 점·좌표 변화량으로
위치를 결정하는지를 의미 기반으로 확인한다. 이 규칙을 구교육과정 문항까지 무조건
소급 적용한다고 해석하지 않는다.

2022 개정 교육과정의 공통수학2 평면좌표 학생용 해설에서는 선분의 외분, 외분점,
외분점 공식에 의존한 풀이를 사용하지 않는다. 수학적으로 같은 결과를 얻더라도 다음을
학생용 핵심 풀이로 사용하면 `CURRICULUM_FAIL`이다.

- 외분, 외분점, 외분점 공식
- 외분비를 공식으로 직접 계산하는 풀이
- `C=A+t(B-A)`와 같은 위치벡터·매개변수 형태
- `D=(3B+2C)/5`, `G=(A+2M)/3`, `A=3G-2M`
- 점 이름 자체를 수처럼 더하고 빼거나 상수배하여 좌표를 구하는 기타 표현

단순히 금지어 문자열의 존재 여부만 검사하지 않는다. 실제 풀이가 외분·외분점이라는
상위 개념이나 벡터적 점 연산에 의존하는지를 의미적으로 판정한다.

내분점을 구할 때는 점 이름을 직접 계산하지 않고 x좌표와 y좌표를 각각 내분점 공식으로
계산한다. 예를 들어 `BD:DC=2:3`이면 다음과 같이 표현한다.
...
따라서 `D=(3B+2C)/5`처럼 점을 직접 산술 연산하는 형태는 사용하지 않는다.

문제의 해가 선분 밖에 존재하더라도 외분점 공식으로 처리하지 않는다. 먼저 한 직선 또는
수직선 위에서 점의 순서와 거리 관계를 명시한다.
...
그 뒤 실제 점의 순서에 따라 `BC = AC + AB`와 같은 거리 관계를 세운다. 필요한 거리를
구한 후에는 좌표별 내분점 공식, 두 점 사이의 거리, x좌표·y좌표 변화량, 해당 단원에서
이미 학습한 좌표 관계 등 공통수학2 범위의 방법으로 좌표를 결정한다. 학생이 따라갈 수
있는 처리 순서는 다음과 같다.
...
핵심은 외분 공식으로 선분 밖의 점을 계산하지 않고, 점의 순서와 거리 관계를 먼저
드러내는 것이다.

#### 3-7-2. 2022 개정 공통수학2 · 직선의 방정식 Hard Rule

`standardCourse = 공통수학2`, `standardUnit = 직선의 방정식`,
`standardUnitKey = H22-C2-02`인 학생용 해설은 좌표·기울기·두 점을 지나는 직선·
절편·평행·수직·점과 직선 사이의 거리·해당 단원에서 배운 각의 관계로 구성한다.
벡터·법선벡터·내적·행렬·미적분·복소수 등 다른 과목의 개념을 핵심 풀이로 끌어오지
않으며, 점을 벡터처럼 더하고 빼서 직선이나 수선의 발을 구하지 않는다.

점과 직선 사이 거리, 수직 조건, 평행 조건, 두 직선의 교점 또는 각의 조건을 실제로
사용하면 그 조건을 식만으로 끝내지 않고 §2-2-1-A의 해당 SVG 사실까지 확인한다.
풀이가 교육과정 밖 방법에 의존하거나 학생이 좌표·기울기·거리만으로 재현할 수 없으면
`CURRICULUM_FAIL`이다.

#### 3-7-3. 2022 개정 공통수학2 · 원의 방정식 Hard Rule

`standardCourse = 공통수학2`, `standardUnit = 원의 방정식`,
`standardUnitKey = H22-C2-03`인 학생용 해설은 완전제곱식, 중심·반지름, 두 점 사이의
거리, 점과 직선 사이의 거리, 원과 직선·원 사이의 위치관계, 접선의 수직 관계 등 해당
단원에서 배운 방법으로 구성한다. 벡터·내적·미적분·행렬·복소수·설명 없는 고급 접선
정리를 핵심 도구로 사용하지 않는다. 공식을 쓰는 경우에도 학생이 배운 관계에서 왜
성립하는지 또는 이 문항에서 어떤 조건을 뜻하는지 연결한다.

실제 풀이가 내분·외분·무게중심·점의 산술 연산·벡터식에 의존하면 단순한
`standardUnitKey`만으로 PASS하지 않는다. 그 방법이 공통수학2 학생용 핵심 풀이에
필요하지 않다면 `CURRICULUM_FAIL`로 되돌리고, 좌표별 계산·거리·도형의 위치관계로
재구성할 수 있는지 확인한다.

#### 3-7-4. 2022 개정 공통수학2 · 도형의 이동 Hard Rule

`standardCourse = 공통수학2`, `standardUnit = 도형의 이동`,
`standardUnitKey = H22-C2-04`인 학생용 해설은 점의 좌표 변화, 평행이동·대칭이동의
좌표 대응, 원래 식에서 변환 후 식을 얻는 치환, 합성 변환의 순서와 해당 도형의
좌표·방정식 관계로 구성한다. 행렬·복소수·일반 affine 변환·벡터공간의 추상 연산을
학생용 핵심 풀이로 사용하지 않는다.

평행이동은 이동량과 대응점, 대칭이동은 대칭축과 대응점, 합성 이동은 각 단계의 결과를
명시한다. 실제 도형의 대응을 생략한 채 식만 변형하거나, 변환 순서를 바꾸어도 같은
것처럼 쓰면 `CURRICULUM_FAIL` 또는 기존 solution logic/parity FAIL로 판정한다.

위 세부 Hard Rule은 단원 metadata의 금지어 검색으로 판정하지 않는다. 실제 풀이의
핵심 도구가 무엇인지, 학생이 해당 단원에서 배운 좌표·거리·기울기·방정식·변환 관계로
결론을 재현할 수 있는지를 의미적으로 확인한다. 단원 밖의 교육과정 문항에 무조건
소급하지 않는다.
...
독립검수 input의 `anchors.origin`은 실제 SVG x-axis `<line>`과 y-axis `<line>` 두 개의
`INTERSECTION`만 허용하며, helper point나 임의 element로 대체하지 않는다. 선언한
`(originX, originY)`와 이 실제 축 교점이 `(0,0)`으로 역산되어야 한다. `anchors.xAxis`,
`anchors.yAxis`의 scale anchor 구조는 유지한다. 실제 축 확인 불가·교점 불일치·`NOT_TESTED`이면
`COORDINATE_MODEL_PARITY == PASS`가 아니므로 `SVG_MATH_PASS`를 선언하지 않는다.
...
않으며 그런 input은 `NOT_TESTED`로 V2 PASS를 금지한다. `OPEN_CLOSED_POINT`의 대상 element는
반드시 `<circle>`이며 circle 자체에 직접 `fill` attribute가 있어야 한다. 부모 `<g>`·CSS 상속은
이번 구현에서 해석하지 않으므로 circle이 아니거나 explicit fill이 없거나 직접 판정할 수 없는 fill
값이면 `NOT_TESTED`다.
...
- 2022 개정 공통수학2 도형의 방정식 계열을 `H22-C2-01`~`H22-C2-04`로 확장하여, 직선·원·도형의 이동의 교육과정 Hard Rule과 단원별 SVG 필수 사실·solution↔SVG parity를 구체화했다. 내분·외분·무게중심·점 산술·벡터식이 다른 도형 단원의 핵심 풀이로 사용된 경우에도 의미 기반으로 `CURRICULUM_FAIL`을 판정하되, `H22-GE-08`·`H15`·선택과목 문항에는 자동 소급하지 않는다.
