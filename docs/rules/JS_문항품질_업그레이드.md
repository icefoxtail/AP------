...
- 직선은 직선·핵심 점·절편·교점·접점을, 이차함수는 포물선·축·꼭짓점·절편·교점을, 이차방정식은 함수와 x축의 교점을, 이차부등식은 포물선·x축·부호 영역·해집합을 표시한다.
- `solutionImage` 또는 solution 내부 inline SVG를 사용할 수 있으며, 문제용 `image`를 해설용으로 재사용하지 않는다.
- SVG는 독립 풀이와 solution에서 확정된 좌표·식·관계로만 제작하고, 실제 학생용 해설지(`sol`) 화면에서 가독성·정합성·교육성을 확인한다.
- 의미 있는 실수 그래프를 만들 수 있는데 그림이 빠졌으면 `SOLUTION_VISUAL_MISSING`으로 재작업한다. 단원명이 같다는 이유로 계산과 무관한 그림을 추가하는 것은 금지한다.
