...
객관식 보기 번호(①~⑤)는 content와 choices 어디에도 넣지 않는다.

번호 표시는 엔진이 choices 배열 순서에 따라 렌더한다.
...
형식:choices: ["보기 내용 1","보기 내용 2","보기 내용 3","보기 내용 4","보기 내용 5"]

[CHOICE LABEL OWNERSHIP LOCK]

- 신규 choices 값에는 ①~⑤, 1., (1) 같은 보기 번호를 직접 넣지 않는다.
- 원문의 보기 번호는 배열 순서를 대조하기 위한 근거일 뿐 데이터 문자열의 일부가 아니다.
- 보기 번호의 화면 표시는 엔진이 담당한다.
- 기존 레거시 데이터에 번호가 있더라도 신규 추출 결과에는 이를 복사하지 않는다.
...
choices 배열이 원문 ①~⑤의 내용 순서와 일치하는지 확인

choices 값에 ①~⑤ 등 보기 번호가 직접 들어가지 않았는지 확인
