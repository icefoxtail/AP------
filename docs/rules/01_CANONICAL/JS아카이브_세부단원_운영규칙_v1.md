...
### 3-1. 2022 개정 기하 공식 내용 요소 매핑

경기도교육청의 `2022 개정 교육과정 고등학교 과목 선택 안내자료`에 제시된 기하의 공식 내용 요소는 다음과 같다.
...
현재 Archive의 승인 세부단원은 공식 내용 요소에서 직접 파생되는 내부 표시 라벨을 사용한다.

| standardUnitKey | 공식 내용 요소 | subUnitKey | subUnit | 비고 |
|---|---|---|---|---|
| `H22-GE-01` | 이차곡선 | `H22-GE-01-CORE` | 이차곡선 핵심 개념 | 공식 요소 직접 대응 |
| `H22-GE-02` | 이차곡선 | `H22-GE-02-CORE` | 이차곡선의 접선 핵심 개념 | 기존 Archive 내부 세분 |
| `H22-GE-03` | 공간도형 | `H22-GE-03-CORE` | 공간도형 핵심 개념 | 공식 요소 직접 대응 |
| `H22-GE-04` | 공간좌표 | `H22-GE-04-CORE` | 공간좌표 핵심 개념 | 공식 요소 직접 대응 |
| `H22-GE-05` | 벡터의 연산 | 기존 벡터 세부키 | 기존 벡터 라벨 유지 | 공식 요소 직접 대응 |
| `H22-GE-06` | 벡터의 성분과 내적 | 기존 벡터 세부키 | 벡터의 성분 | 공식 요소 내부 분할·호환 |
| `H22-GE-07` | 벡터의 성분과 내적 | 기존 벡터 세부키 | 벡터의 내적 | 공식 요소 내부 분할·호환 |
| `H22-GE-08` | 도형의 방정식 | `H22-GE-08-CORE` | 도형의 방정식 핵심 개념 | 공식 요소 직접 대응 |

`H22-GE-03-CORE`, `H22-GE-04-CORE`, `H22-GE-08-CORE`의 `핵심 개념`은 Archive 내부 운영 라벨이며, 공식 교육과정 원문에 없는 별도 내용 요소를 뜻하지 않는다. 또한 공식 내용 요소 `벡터의 성분과 내적`은 `H22-GE-06`·`H22-GE-07` 두 개의 공식 요소로 세지 않는다. 기존 벡터 키를 삭제·재명명하면 기존 JS와 필터가 깨질 수 있으므로, 호환 마이그레이션 전까지 현재 키를 보존한다.
