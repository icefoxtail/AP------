# PROJECT_RULEBOOK_AND_MAPS

이 문서는 AP Math OS / 왕지교육 프로젝트의 모든 작업자 공통 룰북이다.
...
| `migrations/` | DB migration | 적용 전 검수 필수 |
| `schema.sql` | DB 기준 스키마 | migration 누락 금지 |
...
| Route file | 담당 기능 | 주요 API/resource | 주의 |
|---|---|---|---|
| `routes/student-portal.js` | 학생 포털 | `/api/student-portal/*` | 시험지 직접 열기 금지, OMR 재수정 금지 |
| `routes/planner.js` | 플래너 | `/api/planner`, `/api/planner-auth`, `/api/planner-auth-by-name` | 학생포털 SSO 흐름 보존 |
| `routes/homework-photo.js` | 숙제 사진 확인/배정 | `/api/homework-photo/*` | “숙제” 용어 유지, 제출 완료 흐름 임의 변경 금지 |
| `routes/check-omr.js` | OMR 체크/QR | `/api/check-pin`, `/api/check-init`, `/api/qr-classes` | `teacher is not defined` 재발 금지 |
| `routes/reports-ai.js` | 리포트 AI | `/api/ai/*`, report-ai 계열 | 학부모 문구 품질/금지 표현 유지 |
| `routes/class-daily.js` | 수업일지/교재 진도 | class-daily 계열 | 기존 수업일지 UI 흐름 보존 |
| `routes/exams.js` | 시험/OMR/오답 | exam 계열 | MIXED archive 처리 보존 |
| `routes/operations.js` | 상담/운영 메모/일정 | operation 계열 | 메모 공유 정책 주의 |
| `routes/attendance-homework.js` | 출결/숙제 체크 | attendance/homework 계열 | 상태명/버튼명 변경 금지 |
| `routes/students.js` | 학생 관리 | `/api/students/*` | PIN/개인정보 보안 주의 |
| `routes/classes.js` | 반 관리 | `/api/classes/*`, class-students 계열 | 반 구조 변경 주의 |
| `routes/teachers.js` | 선생님/담당반 | teachers 계열 | 권한 범위 주의 |
| foundation 계열 route | foundation/time/billing | foundation-sync, timetable, billing foundation | 실제 운영 기능과 foundation 구분 |
...
|---|---|---|---|---|---|
| 운영센터 초기 로딩 | `apmath/js/*`, 운영센터 HTML | `GET /api/initial-data` | `index.js` | 대부분의 운영 테이블 | 응답 구조 변경 금지 |
| 학생 관리 | 운영센터 학생 관리 JS | `/api/students/*` | `routes/students.js` | `students`, `class_students` | PIN/개인정보 주의 |
| 반 관리 | 운영센터 반 관리 JS | `/api/classes/*` | `routes/classes.js` | `classes`, `class_students` | 기존 반명/화면명 보존 |
| 출결/숙제 체크 | 운영센터 출결/숙제 JS | attendance/homework 계열 | `routes/attendance-homework.js` | `attendance`, `homework` | “숙제” 용어 유지 |
| 시험/OMR/오답 | 운영센터 시험/OMR JS | exam 계열 | `routes/exams.js`, `routes/check-omr.js` | `exam_sessions`, `wrong_answers` | 제출 완료 재수정 금지 |
| 숙제 사진 확인 | 숙제관리 UI | `/api/homework-photo/*` | `routes/homework-photo.js` | `homework_photo_*` | “숙제”를 “과제”로 바꾸지 말 것 |
| 학생 포털 | `apmath/student/index.html` | `/api/student-portal/*` | `routes/student-portal.js` | `students`, `homework_photo_*` | 시험지 직접 열기 금지 |
| 플래너 | `apmath/planner/index.html` | `/api/planner*` | `routes/planner.js` | `student_plans`, `planner_feedback` | SSO 흐름 보존 |
| 리포트/AI | 리포트 관련 JS | `/api/ai/*` | `routes/reports-ai.js` | 시험/오답/학생 데이터 | 학부모 문구 품질 유지 |
| 수업일지 | class-daily UI | class-daily 계열 | `routes/class-daily.js` | `class_daily_*`, `daily_journals` | 기존 일지 문구 보존 |
| 시간표 foundation | 시간표 UI | timetable/foundation 계열 | foundation route | `class_time_slots`, conflict logs | 중등 90분, 고등 120분 |
...
| `homework_photo_assignments` | 숙제 사진 배정 | homework-photo | 숙제관리 UI | soft delete 권장, “숙제” 용어 유지 |
...
| payment/accounting 계열 | 수납·출납 foundation | billing foundation | 수납/출납 준비 UI | 실제 운영 기능은 별도 승인 |
...
| 변경 유형 | 업데이트 문서 |
|---|---|
| 새 route/API | `WANGJI_WORKER_ROUTE_MAP.md`, `AP_MATH_FRONTEND_API_MAP.md` |
| 새 프론트 버튼/화면 | `AP_MATH_FRONTEND_API_MAP.md`, `PROJECT_OVERVIEW.md` |
| UI 문구 변경 요청 | `PROJECT_RULEBOOK.md`, `AP_MATH_FRONTEND_API_MAP.md` |
| 새 DB 테이블/migration | `DB_TABLE_MAP.md` |
| 배포/검증 변경 | `DEPLOY_AND_SMOKE.md` |
| 큰 기능 완료 | `NEXT_WORK_QUEUE.md`, `PROJECT_OVERVIEW.md` |
| 새 폴더/큰 분류 | `PROJECT_OVERVIEW.md` |
| 금지 원칙 추가 | `PROJECT_RULEBOOK.md`, `PROJECT_UPDATE_RULES.md` |

## 7.3 완료 기준

큰 기능, 새 route, 새 폴더, 새 DB 테이블, 새 프론트 진입점이 생긴 작업은 기능 구현이 끝난 뒤 반드시 기준 문서를 업데이트한다.
...
git add PROJECT_RULEBOOK.md PROJECT_OVERVIEW.md AP_MATH_FRONTEND_API_MAP.md WANGJI_WORKER_ROUTE_MAP.md DB_TABLE_MAP.md DEPLOY_AND_SMOKE.md PROJECT_UPDATE_RULES.md NEXT_WORK_QUEUE.md CODEX_RESULT.md
git commit -m "Document AP Math OS project rulebook and maps"
git push
```
...
- node --check 또는 해당 검증을 실행했는가?
- 배포가 필요한 작업이면 smoke를 실행했는가?
- 큰 작업이면 기준 문서를 업데이트했는가?
- git add . 를 하지 않았는가?
...
- `routes/study-material-wrongs.js`: 수업자료 오답 전용 route
- `/api/study-materials`: 수업자료 마스터 조회/등록/수정/비활성화
- `/api/material-unit-ranges`: 단원별 번호 범위 조회/import/수정
- `/api/material-question-tags`: 문항별 유형/태그 조회/import
- `/api/class-material-assignments`: 반별 수업자료 배정 생성/조회/비활성화
- `/api/material-omr`: 학생 포털 배정 조회, OMR 조회/제출, 교사용 제출 현황
- `/api/material-wrongs`: 학생별/반별/학년별 오답 조회
- `/api/material-review`: 학생별/반별 복습 지시표 데이터
...
- `apmath/student/index.html`: 학생 포털 `수업자료 오답` 섹션과 번호 선택 제출 화면
- `apmath/js/study-material-wrong.js`: 강사용 수업자료 오답 관리 모달
- `apmath/js/ui.js`: 강사용 drawer 진입점만 추가

## Guardrails

- 수업자료 원문/PDF/archive 직접 열기 기능 없음
- 학생 제출 후 수정 기능 없음
- 원장/관리자 대시보드 카드 및 운영 요약 카드 변경 없음
- 배포, wrangler, 원격 smoke test, git add/commit/push 금지
