...
(async()=>{
 ...
 for(const width of [1440,390]){
  ...
  const defaultErrors=[];defaultPage.on('pageerror',e=>defaultErrors.push(String(e)));
  const defaultUrl='http://127.0.0.1:8766/archive/engine.html?data=exams/test-fixtures/render-authority-golden.js&mode=exam&submitQr=1&class=prewarm-test&date=2026-09-10&printDryRun=1';
  await defaultPage.goto(defaultUrl);
  await defaultPage.waitForFunction(()=>archiveScreenRuntime?.activeSnapshot,{},{timeout:60000});
  await defaultPage.evaluate(()=>archiveScreenRuntime.whenIdle());
  const defaultPrewarm=await defaultPage.evaluate(()=>({backgroundAttempts:archiveScreenRuntime.inspect().attempts.filter(a=>a.metrics?.foreground===false).length,sol:archiveScreenRuntime.currentSession.modeSnapshots.sol,ans:archiveScreenRuntime.currentSession.modeSnapshots.ans}));
  assert.equal(defaultPrewarm.backgroundAttempts,0);assert.equal(defaultPrewarm.sol,null);assert.equal(defaultPrewarm.ans,null);assert.deepEqual(defaultErrors,[]);
  await defaultPage.close();
  const p=await context.newPage();const errors=[];p.on('pageerror',e=>errors.push(String(e)));
  const url=defaultUrl+'&prewarm=1';
  await p.goto(url);
