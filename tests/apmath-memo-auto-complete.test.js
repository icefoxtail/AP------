...
test('자동 완료는 매일 자정 직후와 데이터 조회 시 모두 실행된다', () => {
  ...
  assert.doesNotMatch(wrangler, /"0 18 \* \* \*"/, 'backup cron must be deferred on the Free plan');
  ...
  assert.doesNotMatch(workerIndex, /D1_BACKUP_WORKFLOW\.create\(/, 'backup Workflow must not be scheduled on the Free plan');
