 ...
 * Hotfix: openExamGradeView 최신 세션 조회 로직 보강
 */
...
async function toggleAtt(sid, date) {
    ...
    const cur = list.find(a => String(a.student_id) === String(sid) && a.date === today); const next = cur?.status === '등원' ? '결석' : '등원';
...
async function toggleHw(sid, date) {
    ...
    const cur = list.find(h => String(h.student_id) === String(sid) && h.date === today); const next = cur?.status === '완료' ? '미완료' : '완료';
...
async function openExamGradeView(classId) {
    ...
    const examList = Object.values(examMap).sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
...
async function openExamDetail(classId, examTitle, examDate) {
    ...
    const sessions = sessionSource.filter(es =>
        es.exam_title === examTitle && es.exam_date === examDate && ids.includes(String(es.student_id))
    );
    ...
    const weakUnitHtml = typeof renderWeakUnitSummary === 'function' ? renderWeakUnitSummary(classWeakUnits, '이 시험의 누적 오답 단원 데이터 없음', { clickable: true, mode: 'class', titlePrefix: '반 취약 단원', context: { targetType: 'class', targetId: classId, targetLabel: (state.db.classes || []).find(c => String(c.id) === String(classId))?.name || '반' } }) : '';
