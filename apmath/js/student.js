 ...
 * [IRONCLAD 5G]: 로직 무결성 사수 버전
 * [Final Polish]: 미니멀리즘 폴리시(1px 테두리, Soft Dark) 적용본
 */

// [Polish] 공통 미니멀리즘 스타일 주입
function injectStudentStyles() {
    ...
    style.textContent = `
        ...
        .std-badge { display: inline-flex; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 800; }
        .std-input-base { width: 100%; background: var(--surface-2); border: 1px solid var(--border); border-radius: 12px; color: var(--text); padding: 12px 14px; font-family: inherit; outline: none; }
    `;
...
function renderStudentDetailTab(sid, tab) {
    ...
    // 1. 프로필 헤더 (Graphic Polish: 하드코딩 색상 제거 및 테두리 중심)
    const headerHtml = `
        <div style="margin-bottom:24px; padding: 4px;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    <h1 style="margin:0; font-size:26px; font-weight:950; color:var(--text); letter-spacing:-0.5px;">${s.name}</h1>
                    <div style="display:flex; gap:6px; margin-top:10px; flex-wrap:wrap;">
                        <span class="std-badge" style="background:rgba(26,92,255,0.08); color:var(--primary);">${s.school_name} ${s.grade}</span>
                        <span class="std-badge" style="background:transparent; border:1px solid var(--border); color:var(--secondary);">${cls?.name || '반 미배정'}</span>
                        ${s.student_pin ? `<span class="std-badge" style="background:var(--surface); border:1.5px solid var(--border); color:var(--text); letter-spacing:1px;">PIN ${s.student_pin}</span>` : ''}
                    </div>
                </div>
                <button class="btn" style="padding:10px 14px; font-size:12px; background:var(--surface-2); border:1px solid var(--border); font-weight:700;" onclick="openEditStudent('${sid}')">정보 수정</button>
            </div>
            
            <div style="margin-top:20px; display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <div style="background:transparent; border:1px solid var(--border); padding:12px; border-radius:16px;">
    ...
    // 2. 탭 바 (스타일 사수)
    const tabBarHtml = `
        <div class="tab-bar" style="background:var(--bg); padding:4px; border-radius:14px; margin-bottom:20px;">
    ...
    // 4. 하단 액션바 (1px 테두리와 투명도 조절)
    const footerHtml = `
        <div style="margin-top:24px; padding-top:20px; border-top:1px solid var(--border); display:flex; gap:10px;">
            <button class="btn btn-primary" style="flex:1.5; font-size:13px; font-weight:800; box-shadow:none;" onclick="openReportPreview('${sid}')">알림톡 문구 생성</button>
            <button class="btn" style="flex:1; font-size:13px; font-weight:800; color:var(--primary); border:1px solid var(--primary); background:transparent;" onclick="openClinicBasketForStudent('${sid}')">클리닉 바구니</button>
        </div>
    `;
...
/**
 * [Tab 1] 성적분석 렌더러 (로직 사수)
 */
function renderGradeTab(sid) {
    const exs = (state.db.exam_sessions || []).filter(e => e.student_id === sid).sort((a,b)=>b.exam_date.localeCompare(a.exam_date));
    
    const chartArea = exs.length > 0 
        ? `<div style="margin-bottom:24px; padding:12px; background:var(--surface); border:1px solid var(--border); border-radius:18px;">
             <canvas id="studentGradeChart" style="width:100%; height:180px;"></canvas>
           </div>`
        : `<div style="padding:40px 20px; text-align:center; color:var(--secondary); background:var(--surface-2); border:1px solid var(--border); border-radius:18px; margin-bottom:20px; font-size:13px; font-weight:700;">누적된 성적 기록이 없습니다.</div>`;
    ...
    const historyRows = exs.map(e => {
        const wrs = state.db.wrong_answers
            ...
            .map(w => typeof buildWrongUnitChip === 'function' ? buildWrongUnitChip(e, w.question_id) : `<span style="font-size:10px; padding:2px 6px; background:var(--surface-2); border:1px solid var(--border); border-radius:4px; margin:1px; color:var(--text-soft);">Q${w.question_id}</span>`)
            .join('');
            
        return `
            <div style="padding:16px 4px; border-bottom:1px solid var(--border);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;">
                    <div>
                        <div style="font-size:14.5px; font-weight:800; color:var(--text);">${e.exam_title}</div>
                        <div style="font-size:12px; color:var(--secondary); font-weight:600; margin-top:2px;">${e.exam_date}</div>
                    </div>
                    <div style="font-size:19px; font-weight:900; color:var(--primary);">${e.score}점</div>
                </div>
                ...
                <div style="display:flex; gap:6px; justify-content:flex-end;">
                    <button class="btn" style="padding:6px 12px; font-size:11px; color:var(--warning); border:1px solid rgba(245,159,0,0.2); background:rgba(245,159,0,0.05);" onclick="handleResetSessionWrongs('${e.id}','${sid}')">오답 초기화</button>
                    <button class="btn" style="padding:6px 12px; font-size:11px; color:var(--error); border:1px solid rgba(232,65,79,0.2); background:rgba(232,65,79,0.05);" onclick="handleDeleteSession('${e.id}','${sid}')">기록 삭제</button>
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div>
            <h4 style="margin:0 0 12px 4px; font-size:15px; font-weight:900; color:var(--text);">최근 성적 추이</h4>
            ${chartArea}
            <h4 style="margin:24px 0 12px 4px; font-size:15px; font-weight:900; color:var(--text);">전체 시험 이력</h4>
...
function renderWeakTab(sid) {
    ...
    return `
        <div style="padding: 0 4px;">
            <h4 style="margin:0 0 4px 0; font-size:15px; font-weight:900; color:var(--text);">취약 단원 분석</h4>
            ...
            ${typeof renderWeakUnitSummary === 'function' 
                ...
                : '<div style="padding:20px; text-align:center; color:var(--secondary);">데이터를 불러올 수 없습니다.</div>'}
        </div>
    `;
}

/**
 * [Tab 3] 상담기록 렌더러 (로직 사수)
 */
function renderCnsTab(sid) {
        ...
        <div class="card" style="padding:16px; margin-bottom:12px; border:1px solid var(--border); box-shadow:none; background:var(--surface);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <div style="display:flex; align-items:center; gap:8px;">
                    ...
                    <span style="background:rgba(26,92,255,0.08); color:var(--primary); padding:2px 8px; border-radius:6px; font-size:11px; font-weight:800;">${c.type}</span>
                </div>
            ...
            <div style="font-size:14.5px; font-weight:600; line-height:1.6; color:var(--text); white-space:pre-wrap;">${apEscapeHtml(c.content)}</div>
            ${c.next_action ? `
                <div style="margin-top:12px; padding:10px; background:rgba(245,159,0,0.06); border:1px solid rgba(245,159,0,0.1); border-radius:10px; font-size:12px; color:var(--warning); font-weight:700;">
    ...
    return `
        <div style="padding: 0 4px;">
            <button class="btn btn-primary" style="width:100%; margin-bottom:20px; padding:14px; font-size:14px; font-weight:800; border-radius:16px; box-shadow:none;" onclick="openAddConsultationModal('${sid}')">+ 새 상담 기록하기</button>
            <div style="max-height:450px; overflow-y:auto; padding-right:4px;">
                ${cnsCards || '<div style="text-align:center; padding:40px; color:var(--secondary); font-weight:700;">상담 기록이 없습니다.</div>'}
            </div>
        </div>
    `;
}
...
/**
 * 알림톡 문구 미리보기 및 복사 시스템 (Master Style)
 */
function openReportPreview(sid) {
    ...
    showModal('알림톡 문구 미리보기', `
        <div style="background:var(--surface-2); border:1px solid var(--border); padding:16px; border-radius:18px; margin-bottom:16px;">
            <p style="margin:0 0 10px 4px; font-size:12px; color:var(--secondary); font-weight:800;">내용을 확인하고 필요하면 수정하세요.</p>
            <textarea id="report-preview-text" class="std-input-base" style="height:280px; text-align:left; background:var(--surface); border:1px solid var(--border); line-height:1.7; resize:none;">${template}</textarea>
        </div>
    `, '최종 복사하기', () => {
    ...
    showModal('학생 정보 수정', `
        <div style="display:flex; flex-direction:column; gap:12px;">
            ...
            <div style="margin-top:10px;">
                <button class="btn" style="width:100%; min-height:44px; color:var(--error); border:1px solid rgba(232,65,79,0.2); background:rgba(232,65,79,0.05); font-weight:800;" onclick="handleDelete('${sid}')">퇴원(제적) 처리</button>
            </div>
        </div>
...
function openDischargedStudents() {
    ...
    const rows = discharged.map(s => `
        <div style="padding:16px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; background:var(--surface);">
            <div><b style="font-size:15px; color:var(--text);">${s.name}</b> <span style="font-size:12px; color:var(--secondary);">${s.school_name}</span></div>
            <button class="btn btn-primary" style="padding:8px 14px; font-size:12px; font-weight:800; box-shadow:none;" onclick="handleRestore('${s.id}')">재원 복구</button>
        </div>
    `).join('');
    showModal('퇴원생 관리', `<div style="max-height:60vh; overflow-y:auto; margin:-20px 0;">${rows || '<div style="padding:40px; text-align:center; color:var(--secondary); font-weight:800;">퇴원생이 없습니다.</div>'}</div>`);
}