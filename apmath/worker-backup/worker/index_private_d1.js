...
// JS아카이브 Private 기출 DB 헬퍼
function parseArchiveExamTitle(raw = '') {
...
function extractImagePathFromContent(content = '') {
  const m = String(content || '').match(/<img[^>]+src=["']([^"']+)["']/i);
  return m ? m[1] : '';
}

function buildPrivateQuestionId(examId, questionNo) {
  return `${examId}__q${questionNo}`;
}

function normalizePrivateQuestion(q, examId) {
  const tags = Array.isArray(q.tags) ? q.tags : [];
  const content = String(q.content || '');
  const imagePath = q.image || extractImagePathFromContent(content) || '';
  const hasTable = tags.includes('표') || /<table\b/i.test(content) || content.includes('question-table');
  const hasGraph = tags.includes('그래프');
  const hasGeometry = tags.includes('도형');
  const hasImage = !!imagePath || /<img\b/i.test(content);
  const questionNo = Number(q.id || q.question_no || 0);

  return {
    id: buildPrivateQuestionId(examId, questionNo),
    exam_id: examId,
    question_no: questionNo,
    level: q.level || '',
    category: q.category || '',
    original_category: q.originalCategory || q.original_category || '',
    standard_course: q.standardCourse || q.standard_course || '',
    standard_unit_key: q.standardUnitKey || q.standard_unit_key || '',
    standard_unit: q.standardUnit || q.standard_unit || '',
    standard_unit_order: Number(q.standardUnitOrder || q.standard_unit_order || 0),
    question_type: q.questionType || q.question_type || '',
    layout_tag: q.layoutTag || q.layout_tag || 'grid',
    tags_json: safeJsonStringify(tags, []),
    wide: asIntFlag(q.wide),
    content,
    choices_json: safeJsonStringify(Array.isArray(q.choices) ? q.choices : [], []),
    answer: q.answer || '',
    solution: q.solution || '',
    has_table: asIntFlag(hasTable),
    has_graph: asIntFlag(hasGraph),
    has_geometry: asIntFlag(hasGeometry),
    has_image: asIntFlag(hasImage),
  ...
  async fetch(request, env) {
    ...
    try {
      if (path[0] === 'api') {
        ...
        // --- 0. JS아카이브 Private 기출 D1 입고/검색 ---
        if (resource === 'private-exam-import' && method === 'POST') {
          ...
          if (teacher.role !== 'admin') return new Response(JSON.stringify({ error: 'Admin only' }), { status: 403, headers });

          const body = await request.json();
          const examTitle = String(body.examTitle || body.title || '').trim();
          const questions = Array.isArray(body.questions) ? body.questions : (Array.isArray(body.questionBank) ? body.questionBank : []);

          if (!examTitle) return new Response(JSON.stringify({ success: false, error: 'examTitle required' }), { status: 400, headers });
          if (!questions.length) return new Response(JSON.stringify({ success: false, error: 'questions array required' }), { status: 400, headers });

          const meta = parseArchiveExamTitle(examTitle);
          const examId = examTitle;
          const course = body.course || questions.find(q => q.standardCourse)?.standardCourse || questions.find(q => q.standard_course)?.standard_course || '';

          const ids = questions.map(q => Number(q.id || q.question_no || 0));
          const invalidIds = ids.filter(v => !Number.isInteger(v) || v <= 0);
          if (invalidIds.length) {
            return new Response(JSON.stringify({ success: false, error: 'invalid question id', invalidIds }), { status: 400, headers });
          }

          const normalized = questions.map(q => normalizePrivateQuestion(q, examId));
          const objectiveCount = normalized.filter(q => q.question_type === '객관식' || JSON.parse(q.choices_json).length > 0).length;
          const subjectiveCount = normalized.length - objectiveCount;
          const tableCount = normalized.filter(q => q.has_table).length;
          const graphCount = normalized.filter(q => q.has_graph).length;
          const geometryCount = normalized.filter(q => q.has_geometry).length;
          const imageCount = normalized.filter(q => q.has_image).length;

          const stmts = [];
          stmts.push(env.DB.prepare(`
            INSERT INTO private_exams (
              id, title, school, year, semester, exam_type, grade, course,
              source_type, visibility, public_allowed, generation_allowed,
              question_count, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'original_exam', 'private', 0, 1, ?, DATETIME('now'), DATETIME('now'))
            ON CONFLICT(id) DO UPDATE SET
              ...
              source_type='original_exam',
              visibility='private',
              public_allowed=0,
              generation_allowed=1,
              question_count=excluded.question_count,
              updated_at=DATETIME('now')
          `).bind(examId, examTitle, meta.school, meta.year, meta.semester, meta.exam_type, meta.grade, course, normalized.length));

          stmts.push(env.DB.prepare('DELETE FROM private_questions WHERE exam_id = ?').bind(examId));

          for (const q of normalized) {
            stmts.push(env.DB.prepare(`
              INSERT INTO private_questions (
                id, exam_id, question_no,
                level, category, original_category,
                standard_course, standard_unit_key, standard_unit, standard_unit_order,
                question_type, layout_tag, tags_json, wide,
                content, choices_json, answer, solution,
                has_table, has_graph, has_geometry, has_image, image_path,
                source_visibility, output_allowed, generation_allowed,
                created_at, updated_at
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'private', 0, 1, DATETIME('now'), DATETIME('now'))
            `).bind(
              q.id, q.exam_id, q.question_no,
              q.level, q.category, q.original_category,
              q.standard_course, q.standard_unit_key, q.standard_unit, q.standard_unit_order,
              q.question_type, q.layout_tag, q.tags_json, q.wide,
              q.content, q.choices_json, q.answer, q.solution,
              q.has_table, q.has_graph, q.has_geometry, q.has_image, q.image_path
            ));
          }
        ...
        if (resource === 'private-exams' && method === 'GET') {
          const teacher = await verifyAuth(request, env);
          if (!teacher) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
          ...
          if (school) { where.push('school = ?'); params.push(school); }
          if (grade) { where.push('grade = ?'); params.push(grade); }
          if (year) { where.push('year = ?'); params.push(year); }

          const sql = `
            SELECT id, title, school, year, semester, exam_type, grade, course,
                   source_type, visibility, public_allowed, generation_allowed,
                   question_count, created_at, updated_at
            FROM private_exams
            ${where.length ? 'WHERE ' + where.join(' AND ') : ''}
            ORDER BY year DESC, school ASC, semester ASC, exam_type ASC, grade ASC, title ASC
          `;
          const res = await env.DB.prepare(sql).bind(...params).all();
          return new Response(JSON.stringify({ success: true, exams: res.results }), { headers });
        }

        if (resource === 'private-questions' && method === 'GET') {
          const teacher = await verifyAuth(request, env);
          if (!teacher) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
          ...
          const school = url.searchParams.get('school') || '';
          const grade = url.searchParams.get('grade') || '';
          const unitKey = url.searchParams.get('unit_key') || '';
          const level = url.searchParams.get('level') || '';
          const hasGraph = url.searchParams.get('has_graph');
          const hasGeometry = url.searchParams.get('has_geometry');
          const hasTable = url.searchParams.get('has_table');
          const limitRaw = Number(url.searchParams.get('limit') || 200);
          const limit = Math.min(Math.max(Number.isFinite(limitRaw) ? limitRaw : 200, 1), 500);

          if (examId) { filters.push('q.exam_id = ?'); params.push(examId); }
          if (school) { filters.push('e.school = ?'); params.push(school); }
          if (grade) { filters.push('e.grade = ?'); params.push(grade); }
          if (unitKey) { filters.push('q.standard_unit_key = ?'); params.push(unitKey); }
          if (level) { filters.push('q.level = ?'); params.push(level); }
          if (hasGraph === '1' || hasGraph === '0') { filters.push('q.has_graph = ?'); params.push(Number(hasGraph)); }
          if (hasGeometry === '1' || hasGeometry === '0') { filters.push('q.has_geometry = ?'); params.push(Number(hasGeometry)); }
          if (hasTable === '1' || hasTable === '0') { filters.push('q.has_table = ?'); params.push(Number(hasTable)); }

          const sql = `
            SELECT
              q.*,
              e.title AS exam_title,
              e.school, e.year, e.semester, e.exam_type, e.grade, e.course
            FROM private_questions q
            JOIN private_exams e ON e.id = q.exam_id
            ${filters.length ? 'WHERE ' + filters.join(' AND ') : ''}
            ORDER BY e.year DESC, e.school ASC, e.title ASC, q.question_no ASC
            LIMIT ?
          `;
          params.push(limit);
          const res = await env.DB.prepare(sql).bind(...params).all();
          ...
          const questions = (res.results || []).map(q => ({
            ...
            choices: (() => { try { return JSON.parse(q.choices_json || '[]'); } catch { return []; } })(),
            source_visibility: 'private',
            output_allowed: 0,
            generation_allowed: 1
          }));

          return new Response(JSON.stringify({ success: true, count: questions.length, questions }), { headers });
        }
