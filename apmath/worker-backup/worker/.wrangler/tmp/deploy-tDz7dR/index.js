    ...
    try {
      if (path[0] === "api") {
        ...
        if (resource === "students" && method === "POST") {
          ...
          if (data.class_id) {
            stmts.push(
              env.DB.prepare("INSERT INTO class_students (class_id, student_id) VALUES (?, ?)").bind(data.class_id, sid)
            );
          }
        ...
        if (method === "PATCH") {
          ...
          if (resource === "exam-sessions") {
            ...
            for (const qId of data.wrong_ids || []) {
              stmts.push(
                env.DB.prepare("INSERT INTO wrong_answers (session_id, question_id, student_id) VALUES (?, ?, ?)").bind(sid, qId, data.student_id)
              );
            }
            await env.DB.batch(stmts);
            return new Response(JSON.stringify({ success: true, id: sid }), { headers });
