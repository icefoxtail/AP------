...
nextPlan:
- 4~6문장, 240자 이상.
- 이 리포트는 오답 정리 수업이 끝난 뒤 발송된다. 오답 조치는 "이미 수업에서 다시 풀어 정리했다"는 완료 보고로 쓰고, 이후 계획은 새 단원 진도·다음 커리큘럼 중심으로 쓴다.
- nextActions를 그대로 이어붙인 문장이 아니라 자연스러운 문단이어야 한다.
- 풀이 과정 확인, 조건 표시, 식 세우기, 검산, 유사 문제, 클리닉 중 필요한 조치를 포함한다.

parentMessage:
- 6~9문장, 420자 이상.
- 순서: 결과 확인 → 평가 해석 → 오답 정리 완료 보고(수업에서 이미 다시 풀어 정리) → 다음 커리큘럼 안내 → 마무리.
- 가정 지도/가정 확인 제안은 절대 쓰지 않는다. 관리 주체는 항상 학원이다.
- 기관 보고서체가 아니라 담당 선생님이 학부모에게 직접 보내는 안내 문체로 쓴다. 학생 이름을 주어로 쓰고, '안정적인 성취'·'바탕으로'·'유지하면서' 같은 평가어 대신 실제 결과와 실제 조치를 서술한다.
- 같은 문장을 diagnosis나 nextPlan에서 복사하지 않는다.

kakaoSummary:
- 반드시 "안녕하세요, AP수학입니다."로 시작한다.
- 6~10줄 내외.
- 점수, 결과 요약, 오답 정리 완료 보고, 다음 커리큘럼 안내, PDF 확인 안내를 포함한다.
...
function buildReportWritingSeeds(payload = {}) {
  ...
  const parentGuideMap = {
    careless: ['풀이 후 확인하는 순서는 수업에서 이미 같이 점검했음을 안내', '오답 정리는 수업에서 완료했음을 안내'],
    conceptGap: ['오답 문항은 수업에서 개념부터 다시 풀어 정리했음을 안내', '이후 관리는 학원에서 이어간다는 안내'],
    interpretation: ['조건을 식으로 옮기는 과정은 수업에서 다시 확인했음을 안내', '오답 정리는 수업에서 완료했음을 안내'],
    strategy: ['오답 문항은 우선순위대로 수업에서 다시 풀어 정리했음을 안내', '이후 관리는 학원에서 이어간다는 안내'],
    mixed: ['오답 정리는 수업에서 완료했음을 안내', '이후 관리는 학원에서 이어간다는 안내']
  };
  const parentGuide = pick(parentGuideMap[errorType] || parentGuideMap.mixed, variabilitySeed + 29, '오답 정리는 수업에서 완료했음을 안내');
...
function buildLearningLifeSeeds(payload = {}) {
  ...
  const parentReassurances = lowerScoreSupportMode ? [
    '오답 문항은 수업에서 중요한 문항부터 이미 다시 풀어 정리했습니다.',
    '학원에서 방향을 잡아가고 있으니 편안한 마음으로 지켜봐 주시면 감사하겠습니다.',
    '한꺼번에 많은 양을 풀리기보다 순서대로, 수업에서 직접 챙기면서 관리하겠습니다.',
    '학원에서 체계적으로 순서를 잡아 관리하겠습니다.',
    '아이의 성장 속도를 믿고, 학원 관리 계획을 함께 응원해 주시면 큰 힘이 됩니다.',
    '지금은 과정에 집중하는 시기입니다. 학원에서 세심하게 챙겨드리겠습니다.',
    '빠른 결과보다는 꾸준한 습관 형성에 초점을 맞춰 함께 나아가겠습니다.',
    '수업에서 정리한 내용이 이어지도록 다음 진도에서도 같은 방식으로 확인하겠습니다.',
    '다음 진도에서 연결되는 개념은 수업에서 먼저 짚고 넘어가겠습니다.',
    ...
    '학원에서 방향을 잡아 수업 안에서 직접 관리하겠습니다.',
    '아이의 컨디션과 학습 상태를 함께 살피며 균형 있게 지도하겠습니다.',
    '조급하게 몰아가기보다 안정적인 학습 루틴을 만드는 데 집중하겠습니다.',
    '다시 정리가 필요한 부분은 수업 안에서 차분히 다시 잡겠습니다.',
    '학원에서 우선순위를 잡아 수업 안에서 관리하겠습니다.'
  ] : [
    '오답 문항은 수업에서 이미 다시 풀어 정리했습니다.',
  ...
  const parentMessageExampleSeeds = {
    lower: [
      `어머님, 안녕하세요.
이번 평가는 점수 자체보다 앞으로 어떤 부분을 먼저 정리하면 좋을지 확인하는 자료로 보시면 좋겠습니다. 수업 안에서 다시 잡아갈 지점이 분명하게 보였고, 조건 확인과 풀이 과정 정리를 통해 충분히 흐름을 회복할 수 있습니다. 학원에서는 확인할 문항을 차근차근 다시 살펴보며, 작은 성공 경험을 쌓을 수 있도록 난도와 순서를 조절하겠습니다. 학원에서 방향을 잡아가고 있으니 편안한 마음으로 지켜봐 주시면 감사하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가를 통해 아이가 어떤 부분에서 먼저 도움을 받으면 좋을지 구체적으로 확인했습니다. 지금은 결과를 크게 부담으로 느끼기보다, 풀이 습관과 조건 표시 과정을 다시 정리하는 것이 더 중요합니다. 학원에서는 맞출 수 있었던 문항부터 자신감을 회복하고, 이후 유사 문제를 통해 풀이 흐름을 안정화하겠습니다. 아이가 수학에 대한 부담을 줄이고 다시 차분히 따라올 수 있도록 세심하게 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 리포트는 아이의 부족한 점을 지적하기 위한 자료가 아니라, 앞으로의 학습 순서를 정리하기 위한 자료로 보시면 좋겠습니다. 확인할 문항이 여러 개 있었지만, 한꺼번에 몰아가기보다 기본 풀이 흐름부터 다시 세우는 것이 우선입니다. 학원에서는 조건 표시, 식 정리, 계산 검산 순서로 수업 안에서 반복 확인하겠습니다. 안정적인 흐름을 만들 수 있도록 학원에서 책임 있게 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서는 아이가 앞으로 어떤 부분을 먼저 정리해야 할지가 비교적 분명하게 보였습니다. 점수만으로 판단하기보다, 수업 안에서 다시 잡아갈 수 있는 풀이 습관과 확인 순서를 보는 것이 중요합니다. 학원에서는 확인할 문항을 단계별로 다시 풀어보며, 작은 성공 경험을 쌓을 수 있도록 문제 난도를 세심하게 조절하겠습니다. 아이가 자신감을 잃지 않고 다시 흐름을 잡을 수 있도록 차분히 이끌겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이가 어떤 유형에서 조금 더 도움이 필요한지 확인할 수 있었던 중요한 자료입니다. 지금은 빠른 결과보다 풀이 과정을 안정적으로 정리하고, 수업 안에서 다시 연결할 수 있는 지점을 하나씩 잡아가는 것이 필요합니다. 학원에서는 확인할 문항을 풀이 흔적 중심으로 다시 보고, 유사 문제를 통해 같은 유형에서 조금씩 자신감을 쌓도록 지도하겠습니다. 학원에서 우선순위를 잡아 안정적으로 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가 결과는 앞으로의 학습 방향을 정리하는 데 의미가 있습니다. 아이에게 필요한 것은 많은 문제를 한꺼번에 다시 푸는 것보다, 문제를 읽고 조건을 표시한 뒤 차분히 풀이를 정리하는 습관입니다. 학원에서는 수업 안에서 풀이 과정을 함께 확인하고, 막히는 지점을 바로 피드백하며 회복 흐름을 만들겠습니다. 아이가 다시 자신감을 가질 수 있도록 학원에서 세심하게 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가를 통해 아이가 어떤 순서로 다시 정리하면 좋을지 확인했습니다. 확인할 문항이 있었지만, 이는 앞으로의 학습 루틴을 다시 잡는 데 도움이 되는 자료입니다. 학원에서는 맞출 수 있었던 문항의 풀이 흐름부터 먼저 회복하고, 이후 난도 있는 문항으로 자연스럽게 이어가겠습니다. 학원에서 단계적으로 관리하고 있으니 편안하게 지켜봐 주셔도 됩니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이의 현재 위치를 차분히 확인하고, 앞으로 어떤 부분을 먼저 보완하면 좋을지 정리하는 기회가 되었습니다. 수업 안에서 다시 잡아갈 수 있는 부분이 분명히 보였기 때문에, 지금은 결과보다 회복 순서를 잘 세우는 것이 중요합니다. 학원에서는 조건 표시와 풀이 과정 확인을 반복하며, 아이가 스스로 이해한 내용을 문제에 적용할 수 있도록 지도하겠습니다. 조급하게 몰아가기보다 안정적인 학습 흐름을 만들겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서 확인된 부분들은 앞으로의 수업 방향을 잡는 데 중요한 기준이 됩니다. 아이가 수학에 대한 부담을 느끼지 않도록, 학원에서는 확인할 문항을 순서대로 나누어 다시 살펴보겠습니다. 먼저 조건을 정확히 읽고 풀이 과정을 정리하는 습관부터 잡은 뒤, 유사 문제로 적용력을 높이겠습니다. 아이가 작은 성공 경험을 통해 자신감을 회복할 수 있도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 리포트는 아이가 못한 부분을 강조하기보다, 앞으로 어떤 방향으로 좋아질 수 있는지 정리한 자료입니다. 확인할 문항이 여러 개 있었지만, 풀이 습관을 다시 잡고 조건을 차분히 정리하면 충분히 개선 흐름을 만들 수 있습니다. 학원에서는 수업 중 바로 피드백하며 막히는 지점을 확인하고, 유사 문제를 통해 다시 적용하는 연습을 진행하겠습니다. 학원에서 차분히 방향을 잡아가겠습니다.`
    ],
    middle: [
      `어머님, 안녕하세요.
이번 평가는 아이가 기본 흐름은 따라가고 있으나, 몇몇 문항에서 조건 확인과 풀이 과정 정리가 더 필요하다는 점을 확인한 자료입니다. 큰 방향은 잡혀 있기 때문에, 수업 안에서 풀이 습관을 조금 더 안정화하면 다음 평가에서 더 좋은 결과로 이어질 수 있습니다. 학원에서는 확인할 문항을 중심으로 풀이 과정을 다시 보고, 유사 문제로 같은 유형에서 흔들리지 않도록 관리하겠습니다. 현재의 흐름을 유지하면서 정확도를 높여가겠습니다.`,
      `어머님, 안녕하세요.
이번 평가를 통해 아이가 어느 부분에서 안정적으로 풀고, 어느 부분에서 한 번 더 확인이 필요한지 구분할 수 있었습니다. 지금은 개념을 새로 많이 늘리기보다, 알고 있는 내용을 문제에 정확히 적용하는 연습이 중요합니다. 학원에서는 조건 표시와 풀이 과정 설명을 병행하며, 문제 접근 순서를 더 분명하게 잡아가겠습니다. 아이가 자신감을 유지하면서 한 단계 더 안정적인 성취로 이어가도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 결과는 아이가 기본 학습 흐름을 유지하고 있음을 보여주면서도, 정확도를 조금 더 끌어올릴 지점을 함께 알려주고 있습니다. 특히 풀이 과정에서 조건을 놓치지 않고 끝까지 확인하는 습관이 자리 잡으면 성취가 더 안정될 수 있습니다. 학원에서는 확인할 문항을 중심으로 풀이 순서를 다시 정리하고, 비슷한 유형을 통해 적용력을 높이겠습니다. 현재의 흐름을 살리면서 실수를 줄이는 방향으로 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이의 학습 흐름에서 안정적인 부분과 조금 더 정리할 부분을 함께 확인할 수 있었던 자료입니다. 무리하게 난도를 올리기보다, 현재 알고 있는 개념을 정확하게 문제에 연결하는 습관을 만드는 것이 우선입니다. 학원에서는 조건 표시, 식 정리, 계산 검산 순서로 풀이 루틴을 다시 잡아가겠습니다. 아이가 지금의 성취를 바탕으로 더 안정적인 결과를 만들 수 있도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가를 통해 아이가 어떤 문항에서 조금 더 세밀한 확인이 필요한지 확인했습니다. 현재 흐름은 유지되고 있으므로, 핵심은 풀이 과정의 정확도를 높이는 데 있습니다. 학원에서는 확인할 문항을 다시 보며 조건을 어떻게 표시하고 식으로 옮기는지 함께 점검하겠습니다. 안정적인 학습 리듬을 유지하면서 다음 단계로 이어가겠습니다.`,
      `어머님, 안녕하세요.
이번 결과는 아이가 전체적인 흐름을 따라가고 있으나, 몇몇 문항에서 확인 순서를 조금 더 다듬으면 좋겠다는 점을 보여줍니다. 학원에서는 정답 여부만 보는 것이 아니라, 풀이 과정에서 어떤 지점이 흔들렸는지 함께 확인하겠습니다. 이후 유사 문제를 통해 같은 유형에서 더 안정적으로 접근하도록 지도하겠습니다. 아이가 자신감을 유지하며 정확도를 높일 수 있도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이가 잘 따라온 부분과 더 단단히 정리할 부분을 나누어 볼 수 있었던 자료입니다. 지금은 학습량을 무리하게 늘리기보다, 풀이 과정을 차분히 쓰고 검산하는 습관을 잡는 것이 효과적입니다. 학원에서는 확인할 문항을 중심으로 식 정리와 계산 과정을 다시 보고, 필요한 경우 유사 문제로 반복 확인하겠습니다. 현재의 성취가 더 안정적인 결과로 이어지도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서는 아이가 기본 흐름을 유지하면서도, 일부 문항에서 더 꼼꼼한 확인이 필요하다는 점을 확인했습니다. 이는 충분히 수업 안에서 잡아갈 수 있는 부분입니다. 학원에서는 조건 표시와 풀이 과정 점검을 통해 실수 가능성을 줄이고, 비슷한 유형에서 다시 적용해보는 연습을 진행하겠습니다. 안정적인 흐름을 이어갈 수 있도록 세심하게 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 리포트는 아이가 어느 정도까지 안정적으로 해결하고 있는지, 그리고 어디서 한 번 더 확인하면 좋을지를 정리한 자료입니다. 전반적인 학습 흐름은 유지되고 있으므로, 다음 단계는 풀이 과정의 세밀함을 높이는 것입니다. 학원에서는 확인할 문항을 다시 살펴보며 조건 해석과 식 정리를 함께 점검하겠습니다. 정확도를 높여 더 안정적인 성취로 이어가겠습니다.`,
      `어머님, 안녕하세요.
이번 평가를 통해 아이의 학습 흐름에서 보완하면 좋은 부분이 구체적으로 확인되었습니다. 현재는 방향을 크게 바꾸기보다, 풀이 루틴을 조금 더 정돈해 정확도를 높이는 것이 중요합니다. 학원에서는 조건 표시와 검산 습관을 중심으로 확인할 문항을 다시 보고, 유사 문제를 통해 같은 유형을 안정화하겠습니다. 지금의 흐름이 좋은 결과로 이어지도록 꾸준히 관리하겠습니다.`
    ],
    high: [
      `어머님, 안녕하세요.
이번 평가는 아이가 높은 정확도를 바탕으로 안정적인 성취를 보여준 결과였습니다. 기본 개념을 문제에 적용하는 흐름이 잘 유지되고 있으며, 이제는 난도 있는 문항에서 풀이 과정의 세밀함을 더해가면 좋겠습니다. 학원에서는 현재의 좋은 흐름을 유지하면서 심화 유형과 서술형 풀이까지 자연스럽게 확장하겠습니다. 지금의 성취가 더 깊은 사고력으로 이어지도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서 아이는 안정적인 정확도와 풀이 집중력을 보여주었습니다. 현재 학습 흐름이 잘 유지되고 있으므로, 다음 단계는 익숙한 유형을 넘어 조금 더 생각이 필요한 문제로 확장하는 것입니다. 학원에서는 심화 문항과 응용 유형을 병행하며, 풀이 과정의 완성도를 높이겠습니다. 좋은 흐름을 유지하면서 더 넓은 문제 해결력으로 이어가겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이가 학습한 내용을 안정적으로 소화하고 있음을 보여주는 결과였습니다. 점수뿐 아니라 풀이 흐름이 흔들리지 않았다는 점에서 긍정적으로 볼 수 있습니다. 학원에서는 현재의 정확도를 유지하면서, 난도 있는 문제와 서술형 문항에서 사고 과정을 더 정교하게 다듬겠습니다. 안정적인 성취가 심화 실력으로 이어지도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 결과는 아이가 현재 범위를 안정적으로 이해하고 있으며, 문제 풀이에서도 좋은 집중력을 유지하고 있음을 보여줍니다. 앞으로는 정확도를 유지하는 것과 함께, 낯선 유형에서도 차분히 접근하는 힘을 키우는 것이 중요합니다. 학원에서는 심화 유형을 조금씩 병행하며 풀이 과정의 논리성을 더 강화하겠습니다. 아이가 자신감을 바탕으로 더 깊은 단계까지 나아가도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서는 아이의 안정적인 성취와 꼼꼼한 풀이 흐름이 잘 드러났습니다. 현재는 기본기를 다시 점검하기보다, 좋은 정확도를 유지하면서 심화 응용으로 확장할 수 있는 단계입니다. 학원에서는 난도 있는 문제를 통해 사고력을 넓히고, 서술형 풀이에서 표현의 완성도까지 함께 관리하겠습니다. 지금의 좋은 흐름을 장기적인 실력으로 이어가겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이가 배운 내용을 정확하게 적용하고 있음을 확인할 수 있었던 결과였습니다. 전반적인 풀이 흐름이 안정되어 있어, 앞으로는 심화 유형에서 조건을 더 정교하게 해석하는 연습을 이어가면 좋겠습니다. 학원에서는 현재의 성취를 유지하면서 응용 문제와 서술형 풀이를 병행하겠습니다. 성취가 더 단단한 실력으로 이어지도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 결과를 통해 아이가 현재 학습 범위를 안정적으로 소화하고 있음을 확인했습니다. 좋은 점수도 의미 있지만, 풀이 과정이 흔들리지 않았다는 점이 더 중요한 성취입니다. 학원에서는 지금의 정확도를 유지하며 고난도 문항과 확장 문제를 통해 사고의 폭을 넓히겠습니다. 안정적인 성취가 더 높은 수준의 문제 해결력으로 이어질 수 있도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서는 아이가 높은 정확도와 안정적인 풀이 습관을 함께 보여주었습니다. 현재 흐름을 잘 유지하면 다음 단계의 심화 학습도 충분히 자연스럽게 이어갈 수 있습니다. 학원에서는 응용 유형과 서술형 정리를 병행하며, 풀이 과정의 논리성을 더 다듬겠습니다. 좋은 성취가 더 넓은 학습 역량으로 확장되도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이의 개념 이해와 문제 적용력이 안정적으로 이어지고 있음을 보여주었습니다. 앞으로는 맞힌 문제를 단순히 지나가기보다, 어떤 방식으로 해결했는지 풀이 과정을 정리하는 습관을 더해가면 좋겠습니다. 학원에서는 심화 유형과 변형 문제를 통해 사고력을 넓히고, 서술형에서도 논리적으로 표현할 수 있도록 지도하겠습니다. 지금의 자신감이 더 깊은 실력으로 이어지도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 결과는 아이가 학습 내용을 정확하게 이해하고 문제에 안정적으로 적용하고 있음을 보여줍니다. 이제는 현재의 성취를 유지하면서, 더 복잡한 조건이 포함된 문항에서도 풀이 순서를 차분히 세우는 연습이 필요합니다. 학원에서는 심화 문제와 서술형 정리를 함께 진행하며, 문제 해결 과정을 더 세밀하게 관리하겠습니다. 좋은 흐름이 장기적인 실력으로 이어지도록 지도하겠습니다.`
    ],
    perfect: [
      `어머님, 안녕하세요.
이번 평가는 아이가 모든 문항을 정확하게 해결하며 매우 안정적인 성취를 보여준 결과였습니다. 단순히 점수가 좋은 것을 넘어, 배운 개념을 문제에 흔들림 없이 적용했다는 점에서 의미가 큽니다. 학원에서는 현재의 정확도를 유지하면서 심화 응용과 서술형 풀이까지 자연스럽게 확장하겠습니다. 지금의 좋은 흐름이 더 깊은 수학적 사고로 이어지도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서 아이는 전 문항을 정확하게 해결하며 매우 탄탄한 학습 상태를 보여주었습니다. 현재 범위에 대한 이해와 집중력이 잘 유지되고 있기 때문에, 다음 단계는 심화 유형과 낯선 문제에 대한 적응력을 키우는 것입니다. 학원에서는 난도 있는 문항과 서술형 풀이를 병행하며 사고의 폭을 넓히겠습니다. 성취가 일시적인 결과에 머물지 않도록 꾸준히 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 결과는 아이가 배운 내용을 정확하게 이해하고, 문제 풀이에서도 흔들림 없이 적용하고 있음을 보여줍니다. 모든 문항을 해결했다는 점도 중요하지만, 난도 있는 문항까지 안정적으로 마무리했다는 점이 특히 긍정적입니다. 학원에서는 현재의 좋은 흐름을 유지하면서 심화 응용과 변형 문제까지 확장하겠습니다. 지금의 성취가 더 깊은 실력으로 이어질 수 있도록 지도하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가는 아이가 현재 학습 범위를 매우 안정적으로 소화하고 있음을 확인한 결과였습니다. 전 문항을 정확히 해결한 만큼, 앞으로는 같은 정확도를 유지하면서 더 복잡한 조건의 문제를 다루는 힘을 키워가면 좋겠습니다. 학원에서는 심화 유형, 서술형 정리, 다음 단원 연결을 균형 있게 진행하겠습니다. 좋은 흐름이 꾸준한 실력으로 자리 잡도록 관리하겠습니다.`,
      `어머님, 안녕하세요.
이번 평가에서는 아이의 정확도와 집중력이 모두 잘 드러났습니다. 모든 문항을 안정적으로 해결한 것은 꾸준히 쌓아온 학습 흐름이 잘 이어지고 있다는 뜻으로 볼 수 있습니다. 학원에서는 현재 성취에 머무르지 않고, 심화 문제와 사고력 문항을 통해 더 넓은 풀이 경험을 제공하겠습니다. 아이가 높은 자신감을 유지하면서도 더 깊이 있는 학습으로 나아갈 수 있도록 지도하겠습니다.`
    ]
  };
...
function buildReportAnalysisUserPrompt(payload) {
  ...
  return [
    ...
    '- 하위권 학생 parentMessage는 부족 지적보다 회복 순서와 학원 관리 책임 강조를 중심으로 작성하라.',
    ...
    '- 하위권 학생 parentMessage에도 가정 지도 요청을 넣지 말고, 오답 정리 완료와 학원 관리 순서를 중심으로 작성하라.',
    ...
    '- 가정 지도 요청은 쓰지 않는다. 조치와 관리는 모두 학원 주어로 쓴다.',
    ...
    '- parentCounselStyle: 상담 문체. 결과 해석과 ',
    ...
    '- parentMessage는 학습 점검 의미 → 학생 태도 또는 수업 안에서 다시 정리할 지점 → 학원 관리 방향 → ',
    '- parentMessage에서 수치 반복은 최소화하고, 학생을 비난하는 느낌을 주지 않는다.',
    '- 하위권 parentMessage 예시 톤: "어머님, 안녕하세요. 이번 평가는 단순히 점수를 확인하는 것이 아니라, 어떤 부분을 먼저 정리하면 좋을지 확인하는 자료로 보시면 좋겠습니다. 수업 안에서 다시 정리할 수 있는 지점이 보였고, 학원에서는 확인할 문항들을 차근차근 다시 풀어보며 조건 확인과 계산 검산 습관을 잡아가겠습니다. 학원에서 방향을 잡아가고 있으니 편안한 마음으로 지켜봐 주시면 감사하겠습니다." 이 톤을 따르되, 없는 태도 사실은 만들지 않는다.',
    '',
    '입력 데이터:',
...
function buildFallbackReportAnalysis(payload = {}) {
  ...
  if (draft && typeof draft === 'object') {
    ...
    return normalizeReportAnalysisResult({
      ...
      parentMessage: seededParentGuide && !mergedParentMessage.includes(seededParentGuide) ? `${mergedParentMessage} ` : mergedParentMessage,
...
감사합니다.`,
  ...
  const parentMessage = lowerScoreSupportMode
    ? `${positiveAnchor || '이번 평가는 단순히 점수를 확인하는 것이 아니라, 어떤 부분을 먼저 정리하면 좋을지 확인하는 자료로 보시면 좋겠습니다.'} ${studentName} 학생은 수업 안에서 다시 정리할 수 있는 지점을 차근차근 잡아가는 과정이 중요합니다. ${teacherCareMessage || '학원에서는 확인할 문항들을 차근차근 다시 풀어보며 조건 확인과 계산 검산 습관을 잡아가겠습니다.'} ${parentReassurance || '학원에서 방향을 잡아가고 있으니 편안한 마음으로 지켜봐 주시면 감사하겠습니다.'} `
    : `${positiveAnchor ? `${positiveAnchor} ` : ''}${studentName} 학생의 이번 평가는 점수뿐 아니라 확인할 문항의 단원을 함께 보는 것이 중요합니다. ${teacherCareMessage || '학원에서는 다음 수업에서 원인을 다시 확인하고, 같은 실수가 반복되지 않도록 풀이 순서와 검산 습관을 잡아가겠습니다.'} ${parentReassurance || '오답 정리는 수업에서 이미 마쳤고, 이후 관리도 학원에서 이어가겠습니다.'}`;
