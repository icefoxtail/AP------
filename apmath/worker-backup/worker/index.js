...
function buildReportAnalysisUserPrompt(payload) {
  ...
  return [
    ...
    '- "못했다", "낮은 수치", "아쉽게 느껴지실 수 있으나", "다소 아쉽게", "아쉬운 결과", "다소 아쉬운 결과", "부족합니다", "오답 처리", "실수 방지", "틀린 문항", "틀린 문제" 같은 표현은 parentMessage에 쓰지 말라.',
...
감사합니다.`,
  ...
  const parentMessage = lowerScoreSupportMode
    ? `${positiveAnchor || '이번 평가는 점수만으로 판단하기보다 앞으로 어떤 순서로 다시 정리해가야 할지를 확인하는 자료로 보시면 좋겠습니다.'} ${studentName} 학생은 현재 풀이 습관과 기본 개념의 흐름을 차근차근 다시 잡는 과정이 중요합니다. ${teacherCareMessage || '학원에서는 맞출 수 있었던 문항부터 먼저 회복하고 이후 난도 있는 문항으로 이어갈 수 있도록 수업 안에서 단계적으로 관리하겠습니다.'} ${parentReassurance || '학원에서 우선순위를 잡아 관리하겠습니다. 가정에서는 부담 없이 응원만 해주셔도 충분합니다.'} 가정에서는 ${parentGuideText}해 주시면 좋겠습니다.`
...
function isWeakAiReportResult(result) {
  ...
  if (/아쉽게 느껴지실 수 있으나|다소 아쉽게|아쉬운 결과|낮은 수치|부족합니다|오답 처리|틀린 문항|틀린 문제/.test(parentMessage)) return true;
...
async function callOpenAiReportAnalysis(env, payload) {
  ...
  const userPrompt = [
    ...
    '- "못했다", "낮은 수치", "아쉽게 느껴지실 수 있으나", "다소 아쉽게", "아쉬운 결과", "다소 아쉬운 결과", "부족합니다", "오답 처리", "실수 방지", "틀린 문항", "틀린 문제" 같은 표현은 parentMessage에 쓰지 않는다.',
