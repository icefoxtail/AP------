...
const sourceRoot = path.join(root, "archive", "textbook", "_비상교육_고등_대수_교과서", "generated");
const jsRoot = path.join(sourceRoot, "js");
const outDir = path.join(sourceRoot, "reports");
const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "").replace("T", "_");
const outJson = path.join(outDir, `visang_algebra_rules_audit_${stamp}.json`);
const outMd = path.join(outDir, `visang_algebra_rules_audit_${stamp}.md`);
...
function pathExistsFromImage(imagePath) {
  ...
  const candidates = [
    ...
    path.join(root, "archive", "textbook", "_비상교육_고등_대수_교과서", normalized),
...
function answerLooksMissing(answer) {
  const text = String(answer ?? "").trim();
  return !text || isPlaceholder(text);
}

function solutionLooksMissing(solution) {
...
for (const file of jsFiles) {
  ...
  bank.forEach((q, index) => {
    ...
    if (typeof q?.id === "number" && q.id !== index + 1) {
      addIssue(issues, file, id, "id", "id_sequence_gap", `expected ${index + 1}, got ${q.id}`);
    }
    ...
    if (!Array.isArray(choices)) {
      addIssue(issues, file, id, "choices", "choices_not_array", typeof choices);
    } else if (choices.length && choices.length !== 5) {
      addIssue(issues, file, id, "choices", "objective_choices_not_5", `choices length ${choices.length}`);
    }
    if (Array.isArray(choices)) {
    ...
    if (answerLooksMissing(answer)) addIssue(issues, file, id, "answer", "answer_missing", answer || "blank answer");
    if (hasMojibake(answer)) addIssue(issues, file, id, "answer", "answer_mojibake_suspected", answer);
    if (solutionLooksMissing(solution)) addIssue(issues, file, id, "solution", "solution_missing", solution || "blank solution");
    if (hasMojibake(solution)) addIssue(issues, file, id, "solution", "solution_mojibake_suspected", solution);

    if (q?.image && !pathExistsFromImage(q.image)) {
      addIssue(issues, file, id, "image", "image_path_missing", q.image);
    }
  });
...
const report = {
  ...
  scope: "비상 대수 generated/js rules audit",
...
if (issues.length) {
  ...
  for (const issue of issues.slice(0, 500)) {
    lines.push(`- ${issue.file} #${issue.id ?? "-"} ${issue.field}/${issue.type}: ${issue.evidence}`);
  }
  if (issues.length > 500) lines.push(`- ... ${issues.length - 500} more`);
}
