...
window.questionBank = [
  ...
  },
  {
    ...
    "standardUnit": "제곱근과 실수",
    "standardUnitOrder": 1,
    "solution": "* 선분 $AB$는 가로 $1$, 세로 $2$인 직사각형의 대각선이므로 $\\overline{AB} = \\sqrt{1^2+2^2} = \\sqrt{5}$입니다. 점 $A$의 좌표가 $-1$이므로 $P = -1+\\sqrt{5}$입니다.\\n* 선분 $CD$는 가로 $3$, 세로 $1$인 직사각형의 대각선이므로 $\\overline{CD} = \\sqrt{3^2+1^2} = \\sqrt{10}$입니다. (단, 그림의 $Q$ 위치가 $5$로 주어지는 경우를 가정함)\\n* 문제의 답안 근거에 따라 $Q$의 좌표가 $5$인 경우, $\\overline{PQ} = 5 - (-1+\\sqrt{5}) = 6-\\sqrt{5}$입니다.\\n* 정답: $6-\\sqrt{5}$",
    "level": "중"
  },
  {
    "id": "22",
