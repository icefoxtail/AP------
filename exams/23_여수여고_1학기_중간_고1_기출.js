...
},
{
  ...
  "answer": "③",
  "solution": "1. 조형물의 단면을 좌표평면 위에 나타내면, 지면 $AB$의 중점을 원점으로 하고 포물선의 꼭짓점을 $(0, 4)$로 잡을 수 있다. 폭이 $4\\text{m}$이므로 점 $A$는 $(-2, 0)$, 점 $B$는 $(2, 0)$이다.\n2. 포물선의 방정식을 $y = a(x+2)(x-2)$라 하면 꼭짓점 $(0, 4)$를 지나므로 $4 = a(2)(-2) \\Rightarrow a = -1$. 즉, 포물선의 방정식은 $y = -x^2 + 4$이다.\n3. 점 $B(2, 0)$에서 $0.5\\text{m}$ 떨어진 지점 $C$는 $(2.5, 0)$이다. 지점 $C$에서 쏜 레이저(직선)의 방정식을 $y = m(x - 2.5)$라 하자.\n4. 이 직선이 포물선 $y = -x^2 + 4$에 접하므로, 두 식을 연립한 $x^2 + mx - 2.5m - 4 = 0$의 판별식 $D$가 $0$이어야 한다.\n$D = m^2 - 4(-2.5m - 4) = m^2 + 10m + 16 = 0$\n$(m+2)(m+8) = 0 \\Rightarrow m = -2 \\text{ 또는 } m = -8$\n5. 그림의 상태에서 기울기는 음수이며, 접점의 위치를 고려할 때 $m = -2$가 적절하다. ($m = -8$인 경우는 접점의 $x$좌표가 $4$가 되어 조형물 밖의 지점이 된다.)\n6. $m = -2$일 때 접점의 $x$좌표는 중근인 $x = -\\frac{m}{2} = 1$이다.\n7. 접점의 높이 $h$는 $x = 1$일 때의 $y$값이므로 $h = -1^2 + 4 = 3(\\text{m})$이다.\n\n따라서 높이 $h$는 $3$이다."
},
  {
    "id": 18,
    "level": "상",
