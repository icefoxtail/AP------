...
const engineRoot = path.join(reviewDir, 'engine_source');
const archiveRoot = path.join(engineRoot, 'archive');
const chromeExe = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const viewport = { width: 1440, height: 1100, deviceScaleFactor: 1, mobile: false };
const exams = [
  {
    key: 'suncheon_before_reflow',
    expected: 23,
    title: '25_순천고_2학기_중간_고1_기출',
    data: 'exams/original/high/h1/2mid/25_순천고_2학기_중간_고1_기출.js',
  },
  {
    key: 'geumdang_after_reflow',
    expected: 22,
...
async function renderExam(cdp, serverPort, exam) {
  ...
  const metrics = await evaluate(cdp, sessionId, `(() => {
    ...
    return {title:state.title, mode:state.mode, questionCount:state.data.length,
      ...
      overflowElements, fullTextLength:visibleText.length,
  ...
  const pdfPath = path.join(schoolDir, `${exam.key}.pdf`);
  ...
  metrics.capture = {
    ...
    fullScreenshot: path.join(schoolDir, 'full-render.png'),
    pageScreenshots: pageDir, pdf: pdfPath,
